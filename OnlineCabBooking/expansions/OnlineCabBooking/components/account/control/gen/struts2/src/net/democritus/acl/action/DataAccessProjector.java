...
public class DataAccessProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(DataAccessProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "DataAccess.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "DataAccess.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.acl", "DataAccess");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    DataAccessAgent dataAccessAgent = createDataAccessAgent();
    crudsResult = dataAccessAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static DataAccessAgent createDataAccessAgent() {
    return DataAccessAgent.getDataAccessAgent(
