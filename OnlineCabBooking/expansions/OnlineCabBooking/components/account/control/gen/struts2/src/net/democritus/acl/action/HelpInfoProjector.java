...
public class HelpInfoProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(HelpInfoProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "HelpInfo.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "HelpInfo.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.acl", "HelpInfo");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    HelpInfoAgent helpInfoAgent = createHelpInfoAgent();
    crudsResult = helpInfoAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static HelpInfoAgent createHelpInfoAgent() {
    return HelpInfoAgent.getHelpInfoAgent(
