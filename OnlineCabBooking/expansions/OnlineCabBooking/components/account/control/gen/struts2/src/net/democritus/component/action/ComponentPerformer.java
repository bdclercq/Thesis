...
public class ComponentPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ComponentPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      ComponentDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ComponentPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(ComponentDetails details) throws Exception {
    Class<?> agentClass = Class.forName("net.democritus.component." + taskName + "Agent");
    ...
    Method performMethod = agentClass.getMethod("perform", ComponentDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(ComponentDetails details) throws Exception {
    Method setStatusMethod = ComponentDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    componentAgent.modify(details);
  }
