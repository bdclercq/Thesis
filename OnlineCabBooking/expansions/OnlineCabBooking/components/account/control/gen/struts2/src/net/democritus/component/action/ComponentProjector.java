...
public class ComponentProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ComponentProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Component.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Component.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.component", "Component");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    ComponentAgent componentAgent = createComponentAgent();
    crudsResult = componentAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static ComponentAgent createComponentAgent() {
    return ComponentAgent.getComponentAgent(
