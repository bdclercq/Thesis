...
public class AccountPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AccountPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      AccountDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "AccountPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(AccountDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", AccountDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(AccountDetails details) throws Exception {
    Method setStatusMethod = AccountDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    accountAgent.modify(details);
  }
