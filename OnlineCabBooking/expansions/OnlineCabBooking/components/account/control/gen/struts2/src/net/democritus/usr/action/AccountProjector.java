...
public class AccountProjector extends ActionSupport {
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Account.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Account.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.usr", "Account");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    AccountAgent accountAgent = createAccountAgent();
    crudsResult = accountAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static AccountAgent createAccountAgent() {
    return AccountAgent.getAccountAgent(
