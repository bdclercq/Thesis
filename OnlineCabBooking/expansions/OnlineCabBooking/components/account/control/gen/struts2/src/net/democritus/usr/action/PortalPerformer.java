...
public class PortalPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(PortalPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      PortalDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "PortalPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(PortalDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", PortalDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(PortalDetails details) throws Exception {
    Method setStatusMethod = PortalDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    portalAgent.modify(details);
  }
