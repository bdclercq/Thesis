...
public class PortalProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(PortalProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Portal.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Portal.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.usr", "Portal");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    PortalAgent portalAgent = createPortalAgent();
    crudsResult = portalAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static PortalAgent createPortalAgent() {
    return PortalAgent.getPortalAgent(
