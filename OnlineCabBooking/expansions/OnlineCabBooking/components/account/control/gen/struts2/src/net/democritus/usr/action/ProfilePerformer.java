...
public class ProfilePerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ProfilePerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      ProfileDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ProfilePerformer failed to perform task", e
        );
      }
  ...
  private void performTask(ProfileDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", ProfileDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(ProfileDetails details) throws Exception {
    Method setStatusMethod = ProfileDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    profileAgent.modify(details);
  }
