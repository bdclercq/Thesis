...
public class ProfileProjector extends ActionSupport {
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Profile.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Profile.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.usr", "Profile");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    ProfileAgent profileAgent = createProfileAgent();
    crudsResult = profileAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static ProfileAgent createProfileAgent() {
    return ProfileAgent.getProfileAgent(
