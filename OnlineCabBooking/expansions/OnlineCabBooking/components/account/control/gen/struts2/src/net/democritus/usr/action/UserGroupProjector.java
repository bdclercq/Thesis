...
public class UserGroupProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(UserGroupProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "UserGroup.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "UserGroup.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.usr", "UserGroup");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    UserGroupAgent userGroupAgent = createUserGroupAgent();
    crudsResult = userGroupAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static UserGroupAgent createUserGroupAgent() {
    return UserGroupAgent.getUserGroupAgent(
