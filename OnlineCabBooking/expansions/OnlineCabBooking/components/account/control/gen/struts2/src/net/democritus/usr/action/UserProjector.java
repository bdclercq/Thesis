...
public class UserProjector extends ActionSupport {
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "User.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "User.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.usr", "User");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    UserAgent userAgent = createUserAgent();
    crudsResult = userAgent.getProjection(projectionRef);
