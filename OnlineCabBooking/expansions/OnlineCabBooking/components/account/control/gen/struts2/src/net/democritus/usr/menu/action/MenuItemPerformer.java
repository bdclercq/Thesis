...
public class MenuItemPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(MenuItemPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      MenuItemDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "MenuItemPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(MenuItemDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", MenuItemDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(MenuItemDetails details) throws Exception {
    Method setStatusMethod = MenuItemDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    menuItemAgent.modify(details);
  }
