...
public class MenuItemProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(MenuItemProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "MenuItem.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "MenuItem.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.usr.menu", "MenuItem");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    MenuItemAgent menuItemAgent = createMenuItemAgent();
    crudsResult = menuItemAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static MenuItemAgent createMenuItemAgent() {
    return MenuItemAgent.getMenuItemAgent(
