...
public class MenuPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(MenuPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      MenuDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "MenuPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(MenuDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", MenuDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(MenuDetails details) throws Exception {
    Method setStatusMethod = MenuDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    menuAgent.modify(details);
  }
