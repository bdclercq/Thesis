...
public class MenuProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(MenuProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Menu.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Menu.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.usr.menu", "Menu");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    MenuAgent menuAgent = createMenuAgent();
    crudsResult = menuAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static MenuAgent createMenuAgent() {
    return MenuAgent.getMenuAgent(
