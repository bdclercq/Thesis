...
public class ScreenPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ScreenPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      ScreenDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ScreenPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(ScreenDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", ScreenDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(ScreenDetails details) throws Exception {
    Method setStatusMethod = ScreenDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    screenAgent.modify(details);
  }
