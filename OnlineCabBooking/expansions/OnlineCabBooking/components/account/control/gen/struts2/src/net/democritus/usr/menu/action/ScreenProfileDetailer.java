...
public class ScreenProfileDetailer extends ActionSupport implements Preparable {

  private String parentOid = "";
  private String parentField = "";
  private CrudsResult<ScreenProfileDetails> crudsResult;
  ...
  public CrudsResult<ScreenProfileDetails> getCrudsResult() {
    return crudsResult;
  }

  public JsonResult<ScreenProfileDetails> getJsonResult() {
  ...
  public String execute() throws Exception {
    ...
    if (screenProfileOid.length() > 0) {
      if (shouldClone(screenProfileOid)) {
        screenProfileId = new Long(screenProfileOid.substring(1));
        screenProfileOid = "";
      } else {
        screenProfileId = new Long(screenProfileOid);
      }

      crudsResult = screenProfileAgent.getDetails(screenProfileId);

      if (crudsResult.isError()) {
        actionResult = Action.INPUT;
      } else {
        screenProfileDetails = crudsResult.getValue();
      ...
      if (actionResult.equals(Action.SUCCESS)) {
        crudsResult = CrudsResult.success(screenProfileDetails);
      } else {
          Diagnostic diagnostic = Diagnostic.error("account", "ScreenProfile", parentField, "cannotSetParentField");
  ...
  private boolean shouldClone(String screenProfileOid) {
    return screenProfileOid.startsWith("c");
  }
  ...
  private String setParent(Long parentOid) {
    try {
        ...
        Method getParentDataRefMethod = ScreenProfileAgent.class.getMethod(getParentDataRefMethodName, new Class[] {Long.class});

        DataRef parentRef = (DataRef) getParentDataRefMethod.invoke(screenProfileAgent, new Object[] {parentOid});

        String setParentMethodName = "set"+parentField;
        Method setParentMethod = ScreenProfileDetails.class.getMethod(setParentMethodName, new Class[] {DataRef.class});

        setParentMethod.invoke(screenProfileDetails, new Object[] {parentRef});
