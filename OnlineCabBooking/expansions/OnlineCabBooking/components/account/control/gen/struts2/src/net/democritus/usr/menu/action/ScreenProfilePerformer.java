...
public class ScreenProfilePerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ScreenProfilePerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      ScreenProfileDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ScreenProfilePerformer failed to perform task", e
        );
      }
  ...
  private void performTask(ScreenProfileDetails details) throws Exception {
    Class<?> agentClass = Class.forName("net.democritus.usr.menu." + taskName + "Agent");
    ...
    Method performMethod = agentClass.getMethod("perform", ScreenProfileDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(ScreenProfileDetails details) throws Exception {
    Method setStatusMethod = ScreenProfileDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    screenProfileAgent.modify(details);
  }
