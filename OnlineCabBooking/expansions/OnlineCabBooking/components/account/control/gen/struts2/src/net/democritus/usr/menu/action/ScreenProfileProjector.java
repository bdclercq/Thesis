...
public class ScreenProfileProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ScreenProfileProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "ScreenProfile.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "ScreenProfile.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.usr.menu", "ScreenProfile");
    ...
    crudsResult = screenProfileAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static ScreenProfileAgent createScreenProfileAgent() {
    return ScreenProfileAgent.getScreenProfileAgent(
