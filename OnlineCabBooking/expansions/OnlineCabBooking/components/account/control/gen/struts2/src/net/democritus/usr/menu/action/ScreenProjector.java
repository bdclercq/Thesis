...
public class ScreenProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ScreenProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Screen.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Screen.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "account", "net.democritus.usr.menu", "Screen");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    ScreenAgent screenAgent = createScreenAgent();
    crudsResult = screenAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static ScreenAgent createScreenAgent() {
    return ScreenAgent.getScreenAgent(
