...
public class DataAccessDataRefProjector implements IDataElementProjector<DataAccessData, DataRef>, IDataProjectorRequired<DataRef> {
  ...
  public DataAccessDataRefProjector(DataAccessCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  // anchor:project:start
  public DataRef project(ParameterContext<DataAccessData> dataParameter) {

    DataRef projection = DataRef.withId(0L);
    projection.setComponentName("account");
    projection.setPackName("net.democritus.acl");
    projection.setClassName("DataAccess");
  ...
  // anchor:toData:start
  public void toData(DataAccessData data, ParameterContext<DataRef> projectionParameter) {
