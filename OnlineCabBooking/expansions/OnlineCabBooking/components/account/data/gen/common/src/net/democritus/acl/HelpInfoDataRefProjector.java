...
public class HelpInfoDataRefProjector implements IDataElementProjector<HelpInfoData, DataRef>, IDataProjectorRequired<DataRef> {
  ...
  public HelpInfoDataRefProjector(HelpInfoCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  // anchor:project:start
  public DataRef project(ParameterContext<HelpInfoData> dataParameter) {

    DataRef projection = DataRef.withId(0L);
    projection.setComponentName("account");
    projection.setPackName("net.democritus.acl");
    projection.setClassName("HelpInfo");
  ...
  // anchor:toData:start
  public void toData(HelpInfoData data, ParameterContext<DataRef> projectionParameter) {
