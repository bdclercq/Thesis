...
public class ProfileDetailsProjector implements IDataElementProjector<ProfileData, ProfileDetails>, IDataProjectorRequired<ProfileDetails> {
  ...
  public ProfileDetailsProjector(ProfileCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  public ProfileDetails project(ParameterContext<ProfileData> dataParameter) {

    ProfileDetails projection = new ProfileDetails();
    ...
    if (data != null) {
      ...
      projection.setUserGroup(crudsInternal.getUserGroup(dataParameter.construct(data.getUserGroup())).getValueOrElse(EMPTY_DATA_REF));
  ...
  public void toData(ProfileData data, ParameterContext<ProfileDetails> projectionParameter) {
    // don't set the data id
    ProfileDetails projection = projectionParameter.getValue();
    ...
    crudsInternal.setUserGroup(data, projectionParameter.construct(projection.getUserGroup()));
  ...
  public DataRef getDataRef(ProfileDetails projection) {
  ...
  public Class<ProfileDetails> getProjectionClass() {
  ...
  public CrudsResult<Void> checkRequired(ParameterContext<ProfileDetails> projectionParameter) {
