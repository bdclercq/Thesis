...
public class UserDetailsWithoutRefsProjector implements IDataElementProjector<UserData, UserDetailsWithoutRefs>, IDataProjectorRequired<UserDetailsWithoutRefs> {
  ...
  public UserDetailsWithoutRefsProjector(UserCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  public UserDetailsWithoutRefs project(ParameterContext<UserData> dataParameter) {

    UserDetailsWithoutRefs projection = new UserDetailsWithoutRefs();
    ...
    if (data != null) {
      ...
      // @anchor:project-setters:start
      projection.setAccountId(data.getAccount());
      projection.setProfileId(data.getProfile());
      // @anchor:project-setters:end
  ...
  public void toData(UserData data, ParameterContext<UserDetailsWithoutRefs> projectionParameter) {
    // don't set the data id
    UserDetailsWithoutRefs projection = projectionParameter.getValue();
    ...
    // @anchor:toData-setters:start
    crudsInternal.setAccount(data, projectionParameter.construct(DataRef.withId(projection.getAccountId())));
    crudsInternal.setProfile(data, projectionParameter.construct(DataRef.withId(projection.getProfileId())));
    // @anchor:toData-setters:end
    // anchor:custom-toData-setters:start
    // anchor:custom-toData-setters:end
  }

  public DataRef getDataRef(UserDetailsWithoutRefs projection) {
  ...
  public Class<UserDetailsWithoutRefs> getProjectionClass() {
    return UserDetailsWithoutRefs.class;
  }
  ...
  public CrudsResult<Void> checkRequired(ParameterContext<UserDetailsWithoutRefs> projectionParameter) {
