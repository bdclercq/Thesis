...
@Local({ComponentCrudsLocal.class, ComponentCrudsInternal.class})
// @anchor:annotations:end
public class ComponentCruds /*@anchor:interfaces:start@*/implements ComponentCrudsLocal, ComponentCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("account", "Component");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "account.component.alreadyDisabled";
  ...
  private ComponentFinderLocal componentFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(ComponentCruds.class);
  ...
  private ComponentDataRefProjector dataRefProjector;
  private ComponentDetailsProjector detailsProjector;
  private ComponentInfoProjector infoProjector;

  private DataElementProjectorManager<ComponentData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    ComponentCrudsInternal componentCrudsInternal = sessionContext.getBusinessObject(ComponentCrudsInternal.class);

    detailsProjector = new ComponentDetailsProjector(componentCrudsInternal);
    infoProjector = new ComponentInfoProjector(componentCrudsInternal);
    dataRefProjector = new ComponentDataRefProjector(componentCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<ComponentData>();
  ...
  private void initFinder() {
    componentFinder = new ComponentFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<ComponentDetails> detailsParameter) {
    ...
    ComponentDetails details = detailsParameter.getValue();
    ...
    try {
      ...
      detailsProjector.toData(componentData, detailsParameter);
    ...
    try {
      entityManager.persist(componentData);
      ...
      if (componentData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created ComponentCruds with { id : " + componentData.getId() + ", name: " + componentData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(componentData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<ComponentDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    ComponentDetails componentDetails = detailsParameter.getValue();
    DataRef dataRef = componentDetails.getDataRef();
    ...
    try {
      CrudsResult<ComponentData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      ComponentData componentData = dataResult.getValue();
      ...
      if (componentData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified ComponentCruds with { id : " + componentData.getId() + ", name: " + componentData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ComponentCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof ComponentDetails) {
      ComponentDetails details = (ComponentDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(componentData);
      // @anchor:delete:end
      ...
      if (componentData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted ComponentCruds with { id : " + componentData.getId() + ", name: " + componentData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<ComponentData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      ComponentData componentData = entityManager.find(ComponentData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(componentData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<ComponentData> getDataById(ParameterContext<Long> idParameter) {
    ComponentData data = entityManager.find(ComponentData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<ComponentData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<ComponentData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      ComponentFindByNameEqDetails findByNameEqDetails = new ComponentFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<ComponentFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<ComponentFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<ComponentData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<ComponentData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<ComponentData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<ComponentData> result;
  ...
  private CrudsResult<ComponentData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<ComponentData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends ComponentFindDetails> SearchResult<ComponentData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return componentFinder.find(searchParameter);
  }

  public <S extends ComponentFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<ComponentData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<ComponentData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<ComponentData> fromItems, IDataElementProjector<ComponentData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<ComponentData> fromItems, IDataElementProjector<ComponentData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<ComponentData>> listParameter, String projection) {
    IDataElementProjector<ComponentData, P> elementProjector = getElementProjector(projection);
    List<ComponentData> list = listParameter.getValue();
  ...
  private <P> IDataElementProjector<ComponentData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<ComponentData> dataParameter) {
    ...
    ComponentData data = dataParameter.getValue();
    String displayName;
  ...
  private Option<String> getCustomDisplayName(ParameterContext<ComponentData> dataParameter) {
  ...
  private <T> IDataElementProjector<ComponentData, T> getElementProjector(String projectionName) {
    IDataElementProjector<ComponentData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("account", "Component"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<ComponentData> dataResult = getData(projectionRefParameter.construct(dataRef));
    ComponentData componentData;
    if (dataResult.isSuccess()) {
      componentData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<ComponentData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(componentData));

    if (componentData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved ComponentCruds:" + projectionRef.getProjection() + " { id : " + componentData.getId() + ", name: " + componentData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<ComponentDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<ComponentData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    ComponentData componentData = result.getValue();
    ComponentDetails componentDetails = detailsProjector.project(dataRefParameter.construct(componentData));

    return CrudsResult.success(componentDetails);
  }

  public CrudsResult<List<ComponentDetails>> getDetailsListFromData(ParameterContext<Collection<ComponentData>> dataListParameter) {
    Collection<ComponentData> dataList = dataListParameter.getValue();
    List<ComponentDetails> list = new ArrayList<>(dataList.size());

    for (ComponentData componentData: dataList) {
      ComponentDetails componentDetails = detailsProjector.project(dataListParameter.construct(componentData));
      list.add(componentDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<ComponentData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    ComponentData data = dataResult.getValue();
