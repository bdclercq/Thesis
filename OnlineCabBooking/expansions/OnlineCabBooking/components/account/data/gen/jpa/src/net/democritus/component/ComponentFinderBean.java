...
public class ComponentFinderBean implements ComponentFinderLocal {

  private EntityManager entityManager;

  @SuppressWarnings("rawtypes")
  private static final Map<Class<? extends ComponentFindDetails>,
      BiFunction<ComponentFinderBean, ParameterContext, SearchResult<ComponentData>>> finderMapping = generateFinderMapping();

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(ComponentFinderBean.class);
  // @anchor:variables:end

  // anchor:custom-variables:start
  // anchor:custom-variables:end

  public ComponentFinderBean(EntityManager entityManager) {
    this.entityManager = entityManager;
  }

  @SuppressWarnings("rawtypes")
  private static Map<Class<? extends ComponentFindDetails>, BiFunction<ComponentFinderBean, ParameterContext, SearchResult<ComponentData>>> generateFinderMapping() {
    Map<Class<? extends ComponentFindDetails>, BiFunction<ComponentFinderBean, ParameterContext, SearchResult<ComponentData>>> finderMapping = new HashMap<>();
    // anchor:register-finders:start
    finderMapping.put(ComponentFindAllComponentsDetails.class, ComponentFinderBean::findAllComponents);
    finderMapping.put(ComponentFindByNameEqDetails.class, ComponentFinderBean::findByNameEq);
  ...
  public SearchResult<ComponentData> findAllComponents(ParameterContext<SearchDetails<ComponentFindAllComponentsDetails>> searchParameter) {
    SearchDetails<ComponentFindAllComponentsDetails> searchDetails = searchParameter.getValue();
  ...
  @SuppressWarnings({"unchecked", "rawtypes"})
  public SearchResult<ComponentData> findByNameEq(ParameterContext<SearchDetails<ComponentFindByNameEqDetails>> searchParameter) {
    SearchDetails<ComponentFindByNameEqDetails> searchDetails = searchParameter.getValue();

    ComponentFindByNameEqDetails details = searchDetails.getDetails();
  ...
  private SearchResult<ComponentData> fetchData(ParameterContext<SearchDetails<?>> searchParameter, QueryBuilder queryBuilder) {
  ...
  private SearchResult<ComponentData> fetchData(SearchDetails<?> searchDetails, Query countQuery, Query dataQuery) {
    Long total = 0L;
    if (!searchDetails.getSkipCount()) {
      ...
      if (total == 0 || searchDetails.getPaging().getRowsPerPage() == 0) {
        return SearchResult.success(Collections.<ComponentData>emptyList(), total.intValue());
      }
    ...
    List<ComponentData> resultDatas = (List<ComponentData>) dataQuery.getResultList();
  ...
  @Override
  public <S extends ComponentFindDetails> SearchResult<ComponentData> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<ComponentData> searchResult;
    ...
    Class<? extends ComponentFindDetails> findDetailsClass = searchParameter.getValue().getDetails().getClass();
    try {
      if (finderMapping.containsKey(findDetailsClass)) {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
              findDetailsClass.getName() + " does not match a finder in ComponentFinder.find()"
          );
        }

        searchResult = SearchResult.error(Diagnostic.error("account", "component", "invalidFinderException"));
      }
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Exception when processing " + findDetailsClass.getName() + " in ComponentFinder.find() ", e
        );
      }

      searchResult = SearchResult.error(Diagnostic.error("account", "component", "finderException"));
    }
  ...
  private JPAQueryBuilder createQueryBuilder() {
    return new JPAQueryBuilder(ComponentData.ENTITY_NAME, "name");
  }
