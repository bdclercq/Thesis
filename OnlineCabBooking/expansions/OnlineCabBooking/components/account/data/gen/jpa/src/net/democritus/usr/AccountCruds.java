...
@Local({AccountCrudsLocal.class, AccountCrudsInternal.class})
// @anchor:annotations:end
public class AccountCruds /*@anchor:interfaces:start@*/implements AccountCrudsLocal, AccountCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("account", "Account");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "account.account.alreadyDisabled";
  ...
  private AccountFinderLocal accountFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(AccountCruds.class);
  ...
  private AccountDataRefProjector dataRefProjector;
  private AccountDetailsProjector detailsProjector;
  private AccountInfoProjector infoProjector;

  private DataElementProjectorManager<AccountData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    AccountCrudsInternal accountCrudsInternal = sessionContext.getBusinessObject(AccountCrudsInternal.class);

    detailsProjector = new AccountDetailsProjector(accountCrudsInternal);
    infoProjector = new AccountInfoProjector(accountCrudsInternal);
    dataRefProjector = new AccountDataRefProjector(accountCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<AccountData>();
  ...
  private void initFinder() {
    accountFinder = new AccountFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<AccountDetails> detailsParameter) {
    ...
    AccountDetails details = detailsParameter.getValue();
    ...
    try {
      ...
      detailsProjector.toData(accountData, detailsParameter);
    ...
    try {
      ...
      if (accountData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created AccountCruds with { id : " + accountData.getId() + ", name: " + accountData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(accountData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<AccountDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    AccountDetails accountDetails = detailsParameter.getValue();
    DataRef dataRef = accountDetails.getDataRef();
    ...
    try {
      CrudsResult<AccountData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      AccountData accountData = dataResult.getValue();
      ...
      if (accountData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified AccountCruds with { id : " + accountData.getId() + ", name: " + accountData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "AccountCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof AccountDetails) {
      AccountDetails details = (AccountDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(accountData);
      // @anchor:delete:end
      ...
      if (accountData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted AccountCruds with { id : " + accountData.getId() + ", name: " + accountData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<AccountData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      AccountData accountData = entityManager.find(AccountData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(accountData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<AccountData> getDataById(ParameterContext<Long> idParameter) {
    AccountData data = entityManager.find(AccountData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<AccountData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<AccountData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      AccountFindByNameEqDetails findByNameEqDetails = new AccountFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<AccountFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<AccountFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<AccountData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<AccountData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<AccountData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<AccountData> result;
  ...
  private CrudsResult<AccountData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<AccountData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends AccountFindDetails> SearchResult<AccountData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return accountFinder.find(searchParameter);
  }

  public <S extends AccountFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<AccountData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<AccountData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<AccountData> fromItems, IDataElementProjector<AccountData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<AccountData> fromItems, IDataElementProjector<AccountData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<AccountData>> listParameter, String projection) {
    IDataElementProjector<AccountData, P> elementProjector = getElementProjector(projection);
    List<AccountData> list = listParameter.getValue();
  ...
  private <P> IDataElementProjector<AccountData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<AccountData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<AccountData> dataParameter) {
  ...
  private <T> IDataElementProjector<AccountData, T> getElementProjector(String projectionName) {
    IDataElementProjector<AccountData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("account", "Account"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<AccountData> dataResult = getData(projectionRefParameter.construct(dataRef));
    AccountData accountData;
    if (dataResult.isSuccess()) {
      accountData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<AccountData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(accountData));

    if (accountData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved AccountCruds:" + projectionRef.getProjection() + " { id : " + accountData.getId() + ", name: " + accountData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<AccountDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<AccountData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    AccountData accountData = result.getValue();
    AccountDetails accountDetails = detailsProjector.project(dataRefParameter.construct(accountData));

    return CrudsResult.success(accountDetails);
  }

  public CrudsResult<List<AccountDetails>> getDetailsListFromData(ParameterContext<Collection<AccountData>> dataListParameter) {
    Collection<AccountData> dataList = dataListParameter.getValue();
    List<AccountDetails> list = new ArrayList<>(dataList.size());

    for (AccountData accountData: dataList) {
      AccountDetails accountDetails = detailsProjector.project(dataListParameter.construct(accountData));
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<AccountData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    AccountData data = dataResult.getValue();
  ...
  public CrudsResult<Void> compareAndSetStatus(ParameterContext<StateUpdate> parameter) {
    ...
    try {
      Query query = entityManager.createNamedQuery(AccountData.QUERY_COMPARE_SET_STATUS);
      ...
      if (nbUpdatedEntities == 1) {
        ...
        CrudsResult<AccountDetails> detailsResult = getDetails(parameter.construct(target));
        String actualStatus = detailsResult.isSuccess() ? detailsResult.getValue().getStatus() : "???";
        if (logger.isDebugEnabled()) {
          logger.debug(
              "compareAndSetStatus failed for Account with id=" + target.getId() + ", expectedStatus='" + expectedStatus + "', actualStatus='" + actualStatus + "', targetStatus='" + targetStatus + "'"
          );
        }
