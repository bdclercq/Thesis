...
@Local({PortalCrudsLocal.class, PortalCrudsInternal.class})
// @anchor:annotations:end
public class PortalCruds /*@anchor:interfaces:start@*/implements PortalCrudsLocal, PortalCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("account", "Portal");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "account.portal.alreadyDisabled";
  ...
  private PortalFinderLocal portalFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(PortalCruds.class);
  ...
  private PortalDataRefProjector dataRefProjector;
  private PortalDetailsProjector detailsProjector;
  private PortalInfoProjector infoProjector;

  private DataElementProjectorManager<PortalData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    PortalCrudsInternal portalCrudsInternal = sessionContext.getBusinessObject(PortalCrudsInternal.class);

    detailsProjector = new PortalDetailsProjector(portalCrudsInternal);
    infoProjector = new PortalInfoProjector(portalCrudsInternal);
    dataRefProjector = new PortalDataRefProjector(portalCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<PortalData>();
  ...
  private void initFinder() {
    portalFinder = new PortalFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<PortalDetails> detailsParameter) {
    ...
    PortalDetails details = detailsParameter.getValue();
    ...
    try {
      ...
      detailsProjector.toData(portalData, detailsParameter);
    ...
    try {
      entityManager.persist(portalData);
      ...
      if (portalData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created PortalCruds with { id : " + portalData.getId() + ", name: " + portalData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(portalData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<PortalDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    PortalDetails portalDetails = detailsParameter.getValue();
    DataRef dataRef = portalDetails.getDataRef();
    ...
    try {
      CrudsResult<PortalData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      PortalData portalData = dataResult.getValue();
      ...
      if (portalData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified PortalCruds with { id : " + portalData.getId() + ", name: " + portalData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "PortalCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof PortalDetails) {
      PortalDetails details = (PortalDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(portalData);
      // @anchor:delete:end
      ...
      if (portalData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted PortalCruds with { id : " + portalData.getId() + ", name: " + portalData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<PortalData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      PortalData portalData = entityManager.find(PortalData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(portalData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<PortalData> getDataById(ParameterContext<Long> idParameter) {
    PortalData data = entityManager.find(PortalData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<PortalData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<PortalData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      PortalFindByNameEqDetails findByNameEqDetails = new PortalFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<PortalFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<PortalFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<PortalData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<PortalData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<PortalData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  private CrudsResult<PortalData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<PortalData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends PortalFindDetails> SearchResult<PortalData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return portalFinder.find(searchParameter);
  }

  public <S extends PortalFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<PortalData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<PortalData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<PortalData> fromItems, IDataElementProjector<PortalData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<PortalData> fromItems, IDataElementProjector<PortalData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<PortalData>> listParameter, String projection) {
    IDataElementProjector<PortalData, P> elementProjector = getElementProjector(projection);
    List<PortalData> list = listParameter.getValue();
  ...
  private <P> IDataElementProjector<PortalData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<PortalData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<PortalData> dataParameter) {
  ...
  private <T> IDataElementProjector<PortalData, T> getElementProjector(String projectionName) {
    IDataElementProjector<PortalData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("account", "Portal"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<PortalData> dataResult = getData(projectionRefParameter.construct(dataRef));
    PortalData portalData;
    if (dataResult.isSuccess()) {
      portalData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<PortalData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(portalData));

    if (portalData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved PortalCruds:" + projectionRef.getProjection() + " { id : " + portalData.getId() + ", name: " + portalData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<PortalDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<PortalData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    PortalData portalData = result.getValue();
    PortalDetails portalDetails = detailsProjector.project(dataRefParameter.construct(portalData));

    return CrudsResult.success(portalDetails);
  }

  public CrudsResult<List<PortalDetails>> getDetailsListFromData(ParameterContext<Collection<PortalData>> dataListParameter) {
    Collection<PortalData> dataList = dataListParameter.getValue();
    List<PortalDetails> list = new ArrayList<>(dataList.size());

    for (PortalData portalData: dataList) {
      PortalDetails portalDetails = detailsProjector.project(dataListParameter.construct(portalData));
      list.add(portalDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<PortalData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    PortalData data = dataResult.getValue();
