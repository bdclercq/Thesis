...
import net.democritus.usr.menu.MenuItemFindByMenuItemEqDetails;
// anchor:imports:end
...
@Local({MenuItemCrudsLocal.class, MenuItemCrudsInternal.class})
// @anchor:annotations:end
public class MenuItemCruds /*@anchor:interfaces:start@*/implements MenuItemCrudsLocal, MenuItemCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("account", "MenuItem");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "account.menuItem.alreadyDisabled";
  ...
  private MenuItemFinderLocal menuItemFinder;

  /* linked bean variables */
  // anchor:link-variables:start
  @EJB
  private MenuCrudsInternal menuLocal;
  @EJB
  ...
  private static final Logger logger = LoggerFactory.getLogger(MenuItemCruds.class);
  ...
  private MenuItemDataRefProjector dataRefProjector;
  private MenuItemDetailsProjector detailsProjector;
  private MenuItemInfoProjector infoProjector;

  private DataElementProjectorManager<MenuItemData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    MenuItemCrudsInternal menuItemCrudsInternal = sessionContext.getBusinessObject(MenuItemCrudsInternal.class);

    detailsProjector = new MenuItemDetailsProjector(menuItemCrudsInternal);
    infoProjector = new MenuItemInfoProjector(menuItemCrudsInternal);
    dataRefProjector = new MenuItemDataRefProjector(menuItemCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<MenuItemData>();
  ...
  private void initFinder() {
    menuItemFinder = new MenuItemFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<MenuItemDetails> detailsParameter) {
    ...
    MenuItemDetails details = detailsParameter.getValue();
    ...
    try {
      ...
      detailsProjector.toData(menuItemData, detailsParameter);
    ...
    try {
      entityManager.persist(menuItemData);

      // @anchor:implicitNameFieldOnly-afterPersist:start
      if (identity == null) {
        identity = "M-" + menuItemData.getId().toString();
      }
      menuItemData.setName(identity);
      entityManager.flush();
      ...
      if (menuItemData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created MenuItemCruds with { id : " + menuItemData.getId() + ", name: " + menuItemData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(menuItemData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<MenuItemDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    MenuItemDetails menuItemDetails = detailsParameter.getValue();
    DataRef dataRef = menuItemDetails.getDataRef();
    ...
    try {
      CrudsResult<MenuItemData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      MenuItemData menuItemData = dataResult.getValue();
      ...
      if (menuItemData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified MenuItemCruds with { id : " + menuItemData.getId() + ", name: " + menuItemData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "MenuItemCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof MenuItemDetails) {
      MenuItemDetails details = (MenuItemDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(menuItemData);
      // @anchor:delete:end
      ...
      if (menuItemData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted MenuItemCruds with { id : " + menuItemData.getId() + ", name: " + menuItemData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<MenuItemData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      MenuItemData menuItemData = entityManager.find(MenuItemData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(menuItemData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<MenuItemData> getDataById(ParameterContext<Long> idParameter) {
    MenuItemData data = entityManager.find(MenuItemData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<MenuItemData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<MenuItemData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      MenuItemFindByNameEqDetails findByNameEqDetails = new MenuItemFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<MenuItemFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<MenuItemFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<MenuItemData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<MenuItemData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<MenuItemData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<MenuItemData> result;
  ...
  private CrudsResult<MenuItemData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<MenuItemData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends MenuItemFindDetails> SearchResult<MenuItemData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return menuItemFinder.find(searchParameter);
  }

  public <S extends MenuItemFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<MenuItemData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<MenuItemData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<MenuItemData> fromItems, IDataElementProjector<MenuItemData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<MenuItemData> fromItems, IDataElementProjector<MenuItemData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<MenuItemData>> listParameter, String projection) {
    IDataElementProjector<MenuItemData, P> elementProjector = getElementProjector(projection);
    List<MenuItemData> list = listParameter.getValue();
  ...
  public CrudsResult<DataRef> getMenu(ParameterContext<Long> idParameter) {
    try {
      return menuLocal.getDataRefFromId(idParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  @Override
  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<DataRef> getScreen(ParameterContext<Long> idParameter) {
    try {
      return screenLocal.getDataRefFromId(idParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Cannot dereference specified data bean link", e
  ...
  @Override
  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<DataRef> getMenuItem(ParameterContext<Long> idParameter) {
    try {
      return menuItemLocal.getDataRefFromId(idParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Cannot dereference specified data bean link", e
  ...
  @Override
  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<List<DataRef>> getSubmenuItems(ParameterContext<MenuItemData> menuItemDataParameter) {
    List<DataRef> dataRefVec = new ArrayList<DataRef>();
    MenuItemData menuItemData = menuItemDataParameter.getValue();
    try {
      MenuItemFindByMenuItemEqDetails details = new MenuItemFindByMenuItemEqDetails();
      details.setMenuItem(new DataRef(menuItemData.getId()));

      SearchDetails<MenuItemFindByMenuItemEqDetails> searchDetails = new SearchDetails<MenuItemFindByMenuItemEqDetails>(details);
      searchDetails.setProjection("dataRef");

      ParameterContext<SearchDetails<MenuItemFindByMenuItemEqDetails>> searchParameter = menuItemDataParameter.construct(searchDetails);
  ...
  public CrudsResult<MenuDetails> getMenuDetails(ParameterContext<DataRef> dataRefParameter) {
    try {
      return menuLocal.getDetails(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  public CrudsResult<ScreenDetails> getScreenDetails(ParameterContext<DataRef> dataRefParameter) {
    try {
      return screenLocal.getDetails(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Cannot dereference specified data bean link for details", e
  ...
  public CrudsResult<MenuItemDetails> getMenuItemDetails(ParameterContext<DataRef> dataRefParameter) {
    try {
      return menuItemLocal.getDetails(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Cannot dereference specified data bean link for details", e
  ...
  @Override
  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<List<MenuItemDetails>> getSubmenuItemsDetails(ParameterContext<MenuItemData> dataParameter) {
    MenuItemData menuItemData = dataParameter.getValue();

    List<MenuItemDetails> detailsVec = new ArrayList<>();
    try {
      MenuItemFindByMenuItemEqDetails findDetails = new MenuItemFindByMenuItemEqDetails();

      findDetails.setMenuItem(DataRef.withId(menuItemData.getId()));
      SearchDetails<MenuItemFindByMenuItemEqDetails> searchDetails = new SearchDetails<>(findDetails);
      searchDetails.setProjection("details");

      ParameterContext<SearchDetails<MenuItemFindByMenuItemEqDetails>> searchParameter = dataParameter.construct(searchDetails);
  ...
  @Override
  public void setMenu(MenuItemData menuItemData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      menuItemData.setMenu(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
      result = menuLocal.resolveDataRef(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
    ...
    if (result.isSuccess()) {
      menuItemData.setMenu(result.getValue().getId());
    } else {
      throw new IllegalStateException("Failed to find Menu instance '" + dataRefParameter.getValue() + "'");
    }
  }

  @Override
  public void setScreen(MenuItemData menuItemData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      menuItemData.setScreen(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
      result = screenLocal.resolveDataRef(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Cannot get specified data bean and add the link", e
    ...
    if (result.isSuccess()) {
      menuItemData.setScreen(result.getValue().getId());
    } else {
      throw new IllegalStateException("Failed to find Screen instance '" + dataRefParameter.getValue() + "'");
    }
  }

  @Override
  public void setMenuItem(MenuItemData menuItemData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      menuItemData.setMenuItem(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
      result = menuItemLocal.resolveDataRef(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Cannot get specified data bean and add the link", e
    ...
    if (result.isSuccess()) {
      menuItemData.setMenuItem(result.getValue().getId());
    } else {
      throw new IllegalStateException("Failed to find MenuItem instance '" + dataRefParameter.getValue() + "'");
    }
  }
  ...
  private <P> IDataElementProjector<MenuItemData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<MenuItemData> dataParameter) {
    ...
    displayName = StringUtil.join(":",
      nameFromDataRef(getMenu(dataParameter.construct(data.getMenu()))),
      nameFromDataRef(getScreen(dataParameter.construct(data.getScreen())))
    );

    return displayName;
  }

  private Option<String> getCustomDisplayName(ParameterContext<MenuItemData> dataParameter) {
  ...
  private <T> IDataElementProjector<MenuItemData, T> getElementProjector(String projectionName) {
    IDataElementProjector<MenuItemData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("account", "MenuItem"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<MenuItemData> dataResult = getData(projectionRefParameter.construct(dataRef));
    MenuItemData menuItemData;
    if (dataResult.isSuccess()) {
      menuItemData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<MenuItemData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(menuItemData));

    if (menuItemData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved MenuItemCruds:" + projectionRef.getProjection() + " { id : " + menuItemData.getId() + ", name: " + menuItemData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<MenuItemDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<MenuItemData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    MenuItemData menuItemData = result.getValue();
    MenuItemDetails menuItemDetails = detailsProjector.project(dataRefParameter.construct(menuItemData));

    return CrudsResult.success(menuItemDetails);
  }

  public CrudsResult<List<MenuItemDetails>> getDetailsListFromData(ParameterContext<Collection<MenuItemData>> dataListParameter) {
    Collection<MenuItemData> dataList = dataListParameter.getValue();
    List<MenuItemDetails> list = new ArrayList<>(dataList.size());

    for (MenuItemData menuItemData: dataList) {
      MenuItemDetails menuItemDetails = detailsProjector.project(dataListParameter.construct(menuItemData));
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<MenuItemData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    MenuItemData data = dataResult.getValue();
