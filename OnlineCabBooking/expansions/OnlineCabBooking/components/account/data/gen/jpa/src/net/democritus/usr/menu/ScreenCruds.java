...
@Local({ScreenCrudsLocal.class, ScreenCrudsInternal.class})
// @anchor:annotations:end
public class ScreenCruds /*@anchor:interfaces:start@*/implements ScreenCrudsLocal, ScreenCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("account", "Screen");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "account.screen.alreadyDisabled";
  ...
  private ScreenFinderLocal screenFinder;

  /* linked bean variables */
  // anchor:link-variables:start
  @EJB
  private ComponentCrudsInternal componentLocal;
  // anchor:link-variables:end

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(ScreenCruds.class);
  ...
  private ScreenDataRefProjector dataRefProjector;
  private ScreenDetailsProjector detailsProjector;
  private ScreenInfoProjector infoProjector;

  private DataElementProjectorManager<ScreenData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    ScreenCrudsInternal screenCrudsInternal = sessionContext.getBusinessObject(ScreenCrudsInternal.class);

    detailsProjector = new ScreenDetailsProjector(screenCrudsInternal);
    infoProjector = new ScreenInfoProjector(screenCrudsInternal);
    dataRefProjector = new ScreenDataRefProjector(screenCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<ScreenData>();
  ...
  private void initFinder() {
    screenFinder = new ScreenFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<ScreenDetails> detailsParameter) {
    ...
    ScreenDetails details = detailsParameter.getValue();
    ...
    try {
      ...
      detailsProjector.toData(screenData, detailsParameter);
    ...
    try {
      entityManager.persist(screenData);
      ...
      if (screenData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created ScreenCruds with { id : " + screenData.getId() + ", name: " + screenData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(screenData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<ScreenDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    ScreenDetails screenDetails = detailsParameter.getValue();
    DataRef dataRef = screenDetails.getDataRef();
    ...
    try {
      CrudsResult<ScreenData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      ScreenData screenData = dataResult.getValue();
      ...
      if (screenData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified ScreenCruds with { id : " + screenData.getId() + ", name: " + screenData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ScreenCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof ScreenDetails) {
      ScreenDetails details = (ScreenDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(screenData);
      // @anchor:delete:end
      ...
      if (screenData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted ScreenCruds with { id : " + screenData.getId() + ", name: " + screenData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<ScreenData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      ScreenData screenData = entityManager.find(ScreenData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(screenData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<ScreenData> getDataById(ParameterContext<Long> idParameter) {
    ScreenData data = entityManager.find(ScreenData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<ScreenData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<ScreenData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      ScreenFindByNameEqDetails findByNameEqDetails = new ScreenFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<ScreenFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<ScreenFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<ScreenData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<ScreenData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<ScreenData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  private CrudsResult<ScreenData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<ScreenData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends ScreenFindDetails> SearchResult<ScreenData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return screenFinder.find(searchParameter);
  }

  public <S extends ScreenFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<ScreenData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<ScreenData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<ScreenData> fromItems, IDataElementProjector<ScreenData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<ScreenData> fromItems, IDataElementProjector<ScreenData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<ScreenData>> listParameter, String projection) {
    IDataElementProjector<ScreenData, P> elementProjector = getElementProjector(projection);
    List<ScreenData> list = listParameter.getValue();
  ...
  public CrudsResult<DataRef> getComponent(ParameterContext<Long> idParameter) {
    try {
      return componentLocal.getDataRefFromId(idParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  public CrudsResult<ComponentDetails> getComponentDetails(ParameterContext<DataRef> dataRefParameter) {
    try {
      return componentLocal.getDetails(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  @Override
  public void setComponent(ScreenData screenData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      screenData.setComponent(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
      result = componentLocal.resolveDataRef(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
    ...
    if (result.isSuccess()) {
      screenData.setComponent(result.getValue().getId());
    } else {
      throw new IllegalStateException("Failed to find Component instance '" + dataRefParameter.getValue() + "'");
    }
  }
  ...
  private <P> IDataElementProjector<ScreenData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<ScreenData> dataParameter) {
    ...
    ScreenData data = dataParameter.getValue();
    String displayName;
  ...
  private Option<String> getCustomDisplayName(ParameterContext<ScreenData> dataParameter) {
  ...
  private <T> IDataElementProjector<ScreenData, T> getElementProjector(String projectionName) {
    IDataElementProjector<ScreenData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("account", "Screen"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<ScreenData> dataResult = getData(projectionRefParameter.construct(dataRef));
    ScreenData screenData;
    if (dataResult.isSuccess()) {
      screenData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<ScreenData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(screenData));

    if (screenData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved ScreenCruds:" + projectionRef.getProjection() + " { id : " + screenData.getId() + ", name: " + screenData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<ScreenDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<ScreenData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    ScreenData screenData = result.getValue();
    ScreenDetails screenDetails = detailsProjector.project(dataRefParameter.construct(screenData));

    return CrudsResult.success(screenDetails);
  }

  public CrudsResult<List<ScreenDetails>> getDetailsListFromData(ParameterContext<Collection<ScreenData>> dataListParameter) {
    Collection<ScreenData> dataList = dataListParameter.getValue();
    List<ScreenDetails> list = new ArrayList<>(dataList.size());

    for (ScreenData screenData: dataList) {
      ScreenDetails screenDetails = detailsProjector.project(dataListParameter.construct(screenData));
      list.add(screenDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<ScreenData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    ScreenData data = dataResult.getValue();
