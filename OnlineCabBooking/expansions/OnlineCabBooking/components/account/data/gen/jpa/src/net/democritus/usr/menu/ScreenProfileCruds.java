...
@Local({ScreenProfileCrudsLocal.class, ScreenProfileCrudsInternal.class})
// @anchor:annotations:end
public class ScreenProfileCruds /*@anchor:interfaces:start@*/implements ScreenProfileCrudsLocal, ScreenProfileCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("account", "ScreenProfile");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "account.screenProfile.alreadyDisabled";
  ...
  private ScreenProfileFinderLocal screenProfileFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(ScreenProfileCruds.class);
  ...
  private ScreenProfileDataRefProjector dataRefProjector;
  private ScreenProfileDetailsProjector detailsProjector;
  private ScreenProfileInfoProjector infoProjector;

  private DataElementProjectorManager<ScreenProfileData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    ScreenProfileCrudsInternal screenProfileCrudsInternal = sessionContext.getBusinessObject(ScreenProfileCrudsInternal.class);

    detailsProjector = new ScreenProfileDetailsProjector(screenProfileCrudsInternal);
    infoProjector = new ScreenProfileInfoProjector(screenProfileCrudsInternal);
    dataRefProjector = new ScreenProfileDataRefProjector(screenProfileCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<ScreenProfileData>();
  ...
  private void initFinder() {
    screenProfileFinder = new ScreenProfileFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<ScreenProfileDetails> detailsParameter) {
    ...
    ScreenProfileDetails details = detailsParameter.getValue();
    ...
    ScreenProfileData screenProfileData = new ScreenProfileData();
    try {
      ...
      detailsProjector.toData(screenProfileData, detailsParameter);
    ...
    try {
      entityManager.persist(screenProfileData);

      // @anchor:implicitNameFieldOnly-afterPersist:start
      // @anchor:implicitNameFieldOnly-afterPersist:end

      if (screenProfileData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created ScreenProfileCruds with { id : " + screenProfileData.getId() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(screenProfileData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<ScreenProfileDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    ScreenProfileDetails screenProfileDetails = detailsParameter.getValue();
    DataRef dataRef = screenProfileDetails.getDataRef();
    ...
    try {
      CrudsResult<ScreenProfileData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      ScreenProfileData screenProfileData = dataResult.getValue();
      ...
      if (screenProfileData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified ScreenProfileCruds with { id : " + screenProfileData.getId() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ScreenProfileCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof ScreenProfileDetails) {
      ScreenProfileDetails details = (ScreenProfileDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(screenProfileData);
      // @anchor:delete:end
      ...
      if (screenProfileData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted ScreenProfileCruds with { id : " + screenProfileData.getId() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<ScreenProfileData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      ScreenProfileData screenProfileData = entityManager.find(ScreenProfileData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(screenProfileData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<ScreenProfileData> getDataById(ParameterContext<Long> idParameter) {
    ScreenProfileData data = entityManager.find(ScreenProfileData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<ScreenProfileData> getDataByName(ParameterContext<String> nameParameter) {
    return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  }

  public CrudsResult<ScreenProfileData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<ScreenProfileData> result;
  ...
  private CrudsResult<ScreenProfileData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<ScreenProfileData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends ScreenProfileFindDetails> SearchResult<ScreenProfileData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return screenProfileFinder.find(searchParameter);
  }

  public <S extends ScreenProfileFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<ScreenProfileData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<ScreenProfileData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<ScreenProfileData> fromItems, IDataElementProjector<ScreenProfileData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<ScreenProfileData> fromItems, IDataElementProjector<ScreenProfileData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<ScreenProfileData>> listParameter, String projection) {
    IDataElementProjector<ScreenProfileData, P> elementProjector = getElementProjector(projection);
    List<ScreenProfileData> list = listParameter.getValue();
  ...
  @Override
  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<List<DataRef>> getScreens(ParameterContext<ScreenProfileData> screenProfileDataParameter) {
    ScreenProfileData screenProfileData = screenProfileDataParameter.getValue();
    try {
      List<DataRef> dataRefVec = new ArrayList<DataRef>();
      Collection<ScreenData> screenDatas = screenProfileData.getScreens();

      if (screenDatas != null) {
        for (ScreenData data: screenDatas) {
          ParameterContext<ScreenData> dataParameter = screenProfileDataParameter.construct(data);
  ...
  @Override
  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<List<ScreenDetails>> getScreensDetails(ParameterContext<ScreenProfileData> dataParameter) {
    ScreenProfileData screenProfileData = dataParameter.getValue();
    List<ScreenDetails> detailsVec = new ArrayList<ScreenDetails>();

    try {
      CrudsResult<List<ScreenDetails>> detailsListResult =
        screenLocal.getDetailsListFromData(dataParameter.construct(screenProfileData.getScreens()));
  ...
  @Override
  public void setScreen(ScreenProfileData screenProfileData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      screenProfileData.setScreen(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
    ...
    if (result.isSuccess()) {
      screenProfileData.setScreen(result.getValue().getId());
    } else {
  ...
  @Override
  public void setProfile(ScreenProfileData screenProfileData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      screenProfileData.setProfile(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
    ...
    if (result.isSuccess()) {
      screenProfileData.setProfile(result.getValue().getId());
    } else {
  ...
  @Override
  public void setScreens(ScreenProfileData screenProfileData, ParameterContext<List<DataRef>> dataRefListParameter) {
    ...
    screenProfileData.setScreens(aArrayList);
  }
  ...
  private <P> IDataElementProjector<ScreenProfileData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<ScreenProfileData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<ScreenProfileData> dataParameter) {
  ...
  private <T> IDataElementProjector<ScreenProfileData, T> getElementProjector(String projectionName) {
    IDataElementProjector<ScreenProfileData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("account", "ScreenProfile"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<ScreenProfileData> dataResult = getData(projectionRefParameter.construct(dataRef));
    ScreenProfileData screenProfileData;
    if (dataResult.isSuccess()) {
      screenProfileData = dataResult.getValue();
    } else {
      screenProfileData = new ScreenProfileData();
    }

    // anchor:custom-preProject:start
    // anchor:custom-preProject:end

    IDataElementProjector<ScreenProfileData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(screenProfileData));

    if (screenProfileData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved ScreenProfileCruds:" + projectionRef.getProjection() + " { id : " + screenProfileData.getId() + " }"
        );
      }
    }
  ...
  public CrudsResult<ScreenProfileDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<ScreenProfileData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    ScreenProfileData screenProfileData = result.getValue();
    ScreenProfileDetails screenProfileDetails = detailsProjector.project(dataRefParameter.construct(screenProfileData));

    return CrudsResult.success(screenProfileDetails);
  }

  public CrudsResult<List<ScreenProfileDetails>> getDetailsListFromData(ParameterContext<Collection<ScreenProfileData>> dataListParameter) {
    Collection<ScreenProfileData> dataList = dataListParameter.getValue();
    List<ScreenProfileDetails> list = new ArrayList<>(dataList.size());

    for (ScreenProfileData screenProfileData: dataList) {
      ScreenProfileDetails screenProfileDetails = detailsProjector.project(dataListParameter.construct(screenProfileData));
      list.add(screenProfileDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<ScreenProfileData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    ScreenProfileData data = dataResult.getValue();
