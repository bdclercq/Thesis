...
public class HelpInfoCsvImporterImpl implements IImporter {
  private static final Logger logger = LoggerFactory.getLogger(HelpInfoCsvImporterImpl.class);

  private HelpInfoMapper helpInfoMapper = new HelpInfoMapper();
  ...
  private void importInstance(ParameterContext<ImportLine> importLineParam, ImportResult importResult) {
    ...
    Result<HelpInfoDetails> mappingResult = helpInfoMapper.convertToDetails(importLineParam.construct(line.getValues()));
    if (mappingResult.isError()) {
      importResult.addError(new ImportError(line.getIndex(), mappingResult.getMessage()));
      return;
    }
    HelpInfoDetails helpInfoDetails = mappingResult.getValue();
    ...
    CrudsResult<DataRef> crudsResult = persistHelpInfo(importLineParam.construct(helpInfoDetails));
  ...
  private CrudsResult<DataRef> persistHelpInfo(ParameterContext<HelpInfoDetails> detailsParameter) {
    HelpInfoLocalAgent helpInfoAgent = HelpInfoLocalAgent.getHelpInfoAgent(detailsParameter.getContext());
    ...
    return helpInfoAgent.createOrModify(helpInfoDetails);
  }
