...
public class AccountCsvImporterImpl implements IImporter {
  private static final Logger logger = LoggerFactory.getLogger(AccountCsvImporterImpl.class);
  ...
  private void importInstance(ParameterContext<ImportLine> importLineParam, ImportResult importResult) {
    ...
    Result<AccountDetails> mappingResult = accountMapper.convertToDetails(importLineParam.construct(line.getValues()));
    if (mappingResult.isError()) {
      importResult.addError(new ImportError(line.getIndex(), mappingResult.getMessage()));
      return;
    }
    AccountDetails accountDetails = mappingResult.getValue();
    ...
    CrudsResult<DataRef> crudsResult = persistAccount(importLineParam.construct(accountDetails));
  ...
  private CrudsResult<DataRef> persistAccount(ParameterContext<AccountDetails> detailsParameter) {
    AccountLocalAgent accountAgent = AccountLocalAgent.getAccountAgent(detailsParameter.getContext());
    ...
    return accountAgent.createOrModify(accountDetails);
  }
