...
public class ProfileCsvImporterImpl implements IImporter {
  private static final Logger logger = LoggerFactory.getLogger(ProfileCsvImporterImpl.class);
  ...
  private void importInstance(ParameterContext<ImportLine> importLineParam, ImportResult importResult) {
    ...
    Result<ProfileDetails> mappingResult = profileMapper.convertToDetails(importLineParam.construct(line.getValues()));
    if (mappingResult.isError()) {
      importResult.addError(new ImportError(line.getIndex(), mappingResult.getMessage()));
      return;
    }
    ProfileDetails profileDetails = mappingResult.getValue();
    ...
    CrudsResult<DataRef> crudsResult = persistProfile(importLineParam.construct(profileDetails));
  ...
  private CrudsResult<DataRef> persistProfile(ParameterContext<ProfileDetails> detailsParameter) {
    ProfileLocalAgent profileAgent = ProfileLocalAgent.getProfileAgent(detailsParameter.getContext());
    ...
    return profileAgent.createOrModify(profileDetails);
  }
