...
public class UserCsvImporterImpl implements IImporter {
  private static final Logger logger = LoggerFactory.getLogger(UserCsvImporterImpl.class);
  ...
  private void importInstance(ParameterContext<ImportLine> importLineParam, ImportResult importResult) {
    ...
    Result<UserDetails> mappingResult = userMapper.convertToDetails(importLineParam.construct(line.getValues()));
    if (mappingResult.isError()) {
      importResult.addError(new ImportError(line.getIndex(), mappingResult.getMessage()));
      return;
    }
    UserDetails userDetails = mappingResult.getValue();
    ...
    CrudsResult<DataRef> crudsResult = persistUser(importLineParam.construct(userDetails));
  ...
  private CrudsResult<DataRef> persistUser(ParameterContext<UserDetails> detailsParameter) {
    UserLocalAgent userAgent = UserLocalAgent.getUserAgent(detailsParameter.getContext());
    ...
    return userAgent.createOrModify(userDetails);
  }
