...
public class UserGroupCsvImporterImpl implements IImporter {
  private static final Logger logger = LoggerFactory.getLogger(UserGroupCsvImporterImpl.class);

  private UserGroupMapper userGroupMapper = new UserGroupMapper();
  ...
  private void importInstance(ParameterContext<ImportLine> importLineParam, ImportResult importResult) {
    ...
    Result<UserGroupDetails> mappingResult = userGroupMapper.convertToDetails(importLineParam.construct(line.getValues()));
    if (mappingResult.isError()) {
      importResult.addError(new ImportError(line.getIndex(), mappingResult.getMessage()));
      return;
    }
    UserGroupDetails userGroupDetails = mappingResult.getValue();
    ...
    CrudsResult<DataRef> crudsResult = persistUserGroup(importLineParam.construct(userGroupDetails));
  ...
  private CrudsResult<DataRef> persistUserGroup(ParameterContext<UserGroupDetails> detailsParameter) {
    UserGroupLocalAgent userGroupAgent = UserGroupLocalAgent.getUserGroupAgent(detailsParameter.getContext());
    ...
    return userGroupAgent.createOrModify(userGroupDetails);
  }
