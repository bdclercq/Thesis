...
/**
 * Stateless session bean encapsulating the implementation of the task TaskAuthorization.
 */

@Stateless
@Remote(TaskAuthorizationRemote.class)
@Local(TaskAuthorizationLocal.class)
...
public class TaskAuthorizationBean implements TaskAuthorizationRemote, TaskAuthorizationLocal {

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(TaskAuthorizationBean.class);
  ...
  public TaskResult<TaskAccessRights> performOnTarget(ParameterContext<DataRef> targetParameter) {
  ...
  public TaskResult<TaskAccessRights> perform(ParameterContext<DataAccessQuery> targetParameter) {
    ...
    final TaskAuthorization implementation;
    try {
      String implName = targetParameter.getProcessingContext().getImplementation();
      implementation = (TaskAuthorization) Class.forName(implName).newInstance();
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
      ...
      return TaskResult.error(Diagnostic.error("account", "TaskAuthorization", "NO_VALID_IMPLEMENTATION_CLASS"));
    }
    ...
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "Failed to perform TaskAuthorization on DataAccessQuery", e
        );
      }
    }
