...
/**
 * Local interface for the entity bean Account,
 * representing a known Account
 */
public interface AccountLocal {
  ...
  AccountDetails getDetails(DataRef dataRef);
  ...
  CrudsResult<AccountInfo> getInfo(ParameterContext<Long> idParameter);
  CrudsResult<AccountDetails> getDetails(ParameterContext<Long> idParameter);
  ...
  CrudsResult<AccountDetails> getDetailsFromDataRef(ParameterContext<DataRef> dataRef);
  ...
  <S extends AccountFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter);
