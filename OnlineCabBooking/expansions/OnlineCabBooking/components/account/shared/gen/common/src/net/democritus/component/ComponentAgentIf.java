...
/**
 * interface for the entity bean ComponentBean,
 * representing a known Component
 */
public interface ComponentAgentIf {
  ...
  CrudsResult<DataRef> create(ComponentDetails componentDetails);
  CrudsResult<DataRef> modify(ComponentDetails componentDetails);
  ...
  CrudsResult<ComponentDetails> getDetails(DataRef dataRef);
  ...
  <S extends ComponentFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<ComponentDetails> getDetails(Long componentId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<ComponentInfo> findAllInfos();
