...
public class ComponentMapper implements IDataElementMapper<ComponentDetails> {
  private static final Logger logger = LoggerFactory.getLogger(ComponentMapper.class);
  ...
  public Result<Map<String, String>> convertToMap(ParameterContext<ComponentDetails> parameter) {
    ComponentDetails componentDetails = parameter.getValue();
    try {
      Map<String, String> map = new HashMap<String, String>();

      // anchor:convert-fields-to-map:start
      if (componentDetails.getName() != null) {
        map.put("name", componentDetails.getName());
      }
      ...
      if (logger.isErrorEnabled()) {
          logger.error(
          "Unexpected error while mapping instance " + componentDetails.getId() + " to map", e
          );
      }
  ...
  public Result<ComponentDetails> convertToDetails(ParameterContext<Map<String, String>> parameter) {
    try {
      ...
      if (nameValueFromMap != null && !nameValueFromMap.isEmpty()) {
        componentDetails.setName(nameValueFromMap);
      }
      ...
      return Result.success(componentDetails);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
          logger.error(
