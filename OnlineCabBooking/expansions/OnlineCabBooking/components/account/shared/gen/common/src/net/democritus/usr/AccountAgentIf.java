...
/**
 * interface for the entity bean AccountBean,
 * representing a known Account
 */
public interface AccountAgentIf {
  ...
  CrudsResult<DataRef> create(AccountDetails accountDetails);
  CrudsResult<DataRef> modify(AccountDetails accountDetails);
  ...
  CrudsResult<AccountDetails> getDetails(DataRef dataRef);
  ...
  <S extends AccountFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<AccountDetails> getDetails(Long accountId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<AccountInfo> findAllInfos();
