...
public class PortalMapper implements IDataElementMapper<PortalDetails> {
  private static final Logger logger = LoggerFactory.getLogger(PortalMapper.class);
  ...
  public Result<Map<String, String>> convertToMap(ParameterContext<PortalDetails> parameter) {
    PortalDetails portalDetails = parameter.getValue();
    try {
      Map<String, String> map = new HashMap<String, String>();

      // anchor:convert-fields-to-map:start
      if (portalDetails.getName() != null) {
        map.put("name", portalDetails.getName());
      }

      if (portalDetails.getVersion() != null) {
        map.put("version", portalDetails.getVersion());
      }

      if (portalDetails.getDescription() != null) {
        map.put("description", portalDetails.getDescription());
      }
      ...
      if (logger.isErrorEnabled()) {
          logger.error(
          "Unexpected error while mapping instance " + portalDetails.getId() + " to map", e
          );
      }
  ...
  public Result<PortalDetails> convertToDetails(ParameterContext<Map<String, String>> parameter) {
    try {
      ...
      if (nameValueFromMap != null && !nameValueFromMap.isEmpty()) {
        portalDetails.setName(nameValueFromMap);
      }

      String versionValueFromMap = map.get("version");
      if (versionValueFromMap != null && !versionValueFromMap.isEmpty()) {
        portalDetails.setVersion(versionValueFromMap);
      }

      String descriptionValueFromMap = map.get("description");
      if (descriptionValueFromMap != null && !descriptionValueFromMap.isEmpty()) {
        portalDetails.setDescription(descriptionValueFromMap);
      }
      ...
      return Result.success(portalDetails);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
          logger.error(
