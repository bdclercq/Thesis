...
/**
 * interface for the entity bean ScreenBean,
 * representing a known Screen
 */
public interface ScreenAgentIf {
  ...
  CrudsResult<DataRef> create(ScreenDetails screenDetails);
  CrudsResult<DataRef> modify(ScreenDetails screenDetails);
  ...
  CrudsResult<ScreenDetails> getDetails(DataRef dataRef);
  ...
  <S extends ScreenFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<ScreenDetails> getDetails(Long screenId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<ScreenInfo> findAllInfos();
