define(function (require) {
  ...
  const MenuEvents = require('account/menu/menu-events');
  const MenuElement = require('nsx/nsx-application').getElement('account', 'menu');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.menu.create"}),
      updateEvent: MenuEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(MenuElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.menu.edit"}),
      updateEvent: MenuEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(MenuElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.menu.delete"}),
      updateEvent: MenuEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(MenuElement, "delete-json"),
          data: JSON.stringify({
            "menuOid": target.id
          }),
          dataType: 'json',
