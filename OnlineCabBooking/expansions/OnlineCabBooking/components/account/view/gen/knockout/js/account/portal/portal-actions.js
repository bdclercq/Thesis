define(function (require) {
  ...
  const PortalEvents = require('account/portal/portal-events');
  const PortalElement = require('nsx/nsx-application').getElement('account', 'portal');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.portal.create"}),
      updateEvent: PortalEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(PortalElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.portal.edit"}),
      updateEvent: PortalEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(PortalElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.portal.delete"}),
      updateEvent: PortalEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(PortalElement, "delete-json"),
          data: JSON.stringify({
            "portalOid": target.id
          }),
          dataType: 'json',
