...
define(function (require) {
  ...
  var PortalProjector = require('account/portal/portal-projector');
  var PortalForm = require('account/portal/portal-instance-form');
  ...
  function defineEditDialog(options) {
    ...
    when(editRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to edit a(n) Portal element!")
    });

    function render() {
      var editIntent = PortalEvents.defineEditIntentListener();
      var showDialog = Trigger.defineTrigger();

      var constants = ko.observable();
      var selection = ko.observable();
      var formView = "account/portal-form";
      var title = "Edit Portal";
      var layoutConfig = initLayoutConfig(constants)
      var formOptions = {
        resetOn: editIntent,
        action: PortalActions.editAction()
      };

      var detailsProjection = PortalProjector.defineProjector(selection, {
        projection: "details"
      });
      ...
      var form = PortalForm.defineForm({
  ...
  function initLayoutConfig(constants) {
    return {
      ...
      version: {
          disabled: ko.pureComputed(function () {
            return utils.isDefined(constants().version)
          })
      },
      description: {
          disabled: ko.pureComputed(function () {
