define(function (require) {
  ...
  const UserGroupEvents = require('account/userGroup/userGroup-events');
  const UserGroupElement = require('nsx/nsx-application').getElement('account', 'userGroup');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.userGroup.create"}),
      updateEvent: UserGroupEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(UserGroupElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.userGroup.edit"}),
      updateEvent: UserGroupEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(UserGroupElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.userGroup.delete"}),
      updateEvent: UserGroupEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(UserGroupElement, "delete-json"),
          data: JSON.stringify({
            "userGroupOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
