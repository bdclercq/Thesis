...
public class AssetPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AssetPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      AssetDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "AssetPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(AssetDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", AssetDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(AssetDetails details) throws Exception {
    Method setStatusMethod = AssetDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    assetAgent.modify(details);
  }
