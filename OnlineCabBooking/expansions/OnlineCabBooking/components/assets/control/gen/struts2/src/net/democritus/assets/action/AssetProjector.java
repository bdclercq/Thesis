...
public class AssetProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AssetProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Asset.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Asset.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "assets", "net.democritus.assets", "Asset");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    AssetAgent assetAgent = createAssetAgent();
    crudsResult = assetAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static AssetAgent createAssetAgent() {
    return AssetAgent.getAssetAgent(
