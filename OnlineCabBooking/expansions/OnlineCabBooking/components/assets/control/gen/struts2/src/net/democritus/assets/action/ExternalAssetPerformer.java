...
public class ExternalAssetPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ExternalAssetPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      ExternalAssetDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ExternalAssetPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(ExternalAssetDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", ExternalAssetDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(ExternalAssetDetails details) throws Exception {
    Method setStatusMethod = ExternalAssetDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    externalAssetAgent.modify(details);
  }
