...
public class ExternalAssetProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ExternalAssetProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "ExternalAsset.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "ExternalAsset.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "assets", "net.democritus.assets", "ExternalAsset");
    ...
    crudsResult = externalAssetAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static ExternalAssetAgent createExternalAssetAgent() {
    return ExternalAssetAgent.getExternalAssetAgent(
