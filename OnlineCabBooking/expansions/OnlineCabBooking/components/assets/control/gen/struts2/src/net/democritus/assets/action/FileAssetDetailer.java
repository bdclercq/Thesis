...
public class FileAssetDetailer extends ActionSupport implements Preparable {

  private String parentOid = "";
  private String parentField = "";
  private CrudsResult<FileAssetDetails> crudsResult;

  // @anchor:variables:start
  private Long fileAssetId = null;
  ...
  public CrudsResult<FileAssetDetails> getCrudsResult() {
    return crudsResult;
  }

  public JsonResult<FileAssetDetails> getJsonResult() {
  ...
  public String execute() throws Exception {
    ...
    if (fileAssetOid.length() > 0) {
      if (shouldClone(fileAssetOid)) {
        fileAssetId = new Long(fileAssetOid.substring(1));
        fileAssetOid = "";
      } else {
        fileAssetId = new Long(fileAssetOid);
      }

      crudsResult = fileAssetAgent.getDetails(fileAssetId);

      if (crudsResult.isError()) {
        actionResult = Action.INPUT;
      } else {
        fileAssetDetails = crudsResult.getValue();
        fileAssetName = fileAssetDetails.getName();
      ...
      if (actionResult.equals(Action.SUCCESS)) {
        crudsResult = CrudsResult.success(fileAssetDetails);
      } else {
          Diagnostic diagnostic = Diagnostic.error("assets", "FileAsset", parentField, "cannotSetParentField");
  ...
  private boolean shouldClone(String fileAssetOid) {
    return fileAssetOid.startsWith("c");
  }
  ...
  private String setParent(Long parentOid) {
    try {
        ...
        Method getParentDataRefMethod = FileAssetAgent.class.getMethod(getParentDataRefMethodName, new Class[] {Long.class});

        DataRef parentRef = (DataRef) getParentDataRefMethod.invoke(fileAssetAgent, new Object[] {parentOid});

        String setParentMethodName = "set"+parentField;
        Method setParentMethod = FileAssetDetails.class.getMethod(setParentMethodName, new Class[] {DataRef.class});

        setParentMethod.invoke(fileAssetDetails, new Object[] {parentRef});
