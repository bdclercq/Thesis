...
public class FileAssetPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(FileAssetPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      FileAssetDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "FileAssetPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(FileAssetDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", FileAssetDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(FileAssetDetails details) throws Exception {
    Method setStatusMethod = FileAssetDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    fileAssetAgent.modify(details);
  }
