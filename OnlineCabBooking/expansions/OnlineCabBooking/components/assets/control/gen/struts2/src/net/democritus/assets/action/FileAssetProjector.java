...
public class FileAssetProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(FileAssetProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "FileAsset.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "FileAsset.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "assets", "net.democritus.assets", "FileAsset");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    FileAssetAgent fileAssetAgent = createFileAssetAgent();
    crudsResult = fileAssetAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static FileAssetAgent createFileAssetAgent() {
    return FileAssetAgent.getFileAssetAgent(
