...
public class InternalAssetChunkDetailer extends ActionSupport implements Preparable {

  private String parentOid = "";
  private String parentField = "";
  private CrudsResult<InternalAssetChunkDetails> crudsResult;
  ...
  public CrudsResult<InternalAssetChunkDetails> getCrudsResult() {
    return crudsResult;
  }

  public JsonResult<InternalAssetChunkDetails> getJsonResult() {
  ...
  public String execute() throws Exception {
    ...
    if (internalAssetChunkOid.length() > 0) {
      if (shouldClone(internalAssetChunkOid)) {
        internalAssetChunkId = new Long(internalAssetChunkOid.substring(1));
        internalAssetChunkOid = "";
      } else {
        internalAssetChunkId = new Long(internalAssetChunkOid);
      }

      crudsResult = internalAssetChunkAgent.getDetails(internalAssetChunkId);

      if (crudsResult.isError()) {
        actionResult = Action.INPUT;
      } else {
        internalAssetChunkDetails = crudsResult.getValue();
        internalAssetChunkName = internalAssetChunkDetails.getName();

        actionResult = Action.SUCCESS;
      }

    } else {
      internalAssetChunkName = "";
      ...
      if (actionResult.equals(Action.SUCCESS)) {
        crudsResult = CrudsResult.success(internalAssetChunkDetails);
      } else {
          Diagnostic diagnostic = Diagnostic.error("assets", "InternalAssetChunk", parentField, "cannotSetParentField");
  ...
  private boolean shouldClone(String internalAssetChunkOid) {
    return internalAssetChunkOid.startsWith("c");
  }
  ...
  private String setParent(Long parentOid) {
    try {
        ...
        Method getParentDataRefMethod = InternalAssetChunkAgent.class.getMethod(getParentDataRefMethodName, new Class[] {Long.class});

        DataRef parentRef = (DataRef) getParentDataRefMethod.invoke(internalAssetChunkAgent, new Object[] {parentOid});

        String setParentMethodName = "set"+parentField;
        Method setParentMethod = InternalAssetChunkDetails.class.getMethod(setParentMethodName, new Class[] {DataRef.class});

        setParentMethod.invoke(internalAssetChunkDetails, new Object[] {parentRef});
