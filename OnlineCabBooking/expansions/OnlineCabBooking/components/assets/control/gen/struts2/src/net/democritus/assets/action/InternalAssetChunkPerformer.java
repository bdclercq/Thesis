...
public class InternalAssetChunkPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(InternalAssetChunkPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      InternalAssetChunkDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "InternalAssetChunkPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(InternalAssetChunkDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", InternalAssetChunkDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(InternalAssetChunkDetails details) throws Exception {
    Method setStatusMethod = InternalAssetChunkDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    internalAssetChunkAgent.modify(details);
  }
