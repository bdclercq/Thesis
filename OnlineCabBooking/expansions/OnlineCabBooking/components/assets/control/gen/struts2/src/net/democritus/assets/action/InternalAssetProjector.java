...
public class InternalAssetProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(InternalAssetProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "InternalAsset.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "InternalAsset.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "assets", "net.democritus.assets", "InternalAsset");
    ...
    crudsResult = internalAssetAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static InternalAssetAgent createInternalAssetAgent() {
    return InternalAssetAgent.getInternalAssetAgent(
