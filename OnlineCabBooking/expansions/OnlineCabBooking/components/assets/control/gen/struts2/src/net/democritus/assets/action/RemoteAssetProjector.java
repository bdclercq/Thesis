...
public class RemoteAssetProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(RemoteAssetProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "RemoteAsset.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "RemoteAsset.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "assets", "net.democritus.assets", "RemoteAsset");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    RemoteAssetAgent remoteAssetAgent = createRemoteAssetAgent();
    crudsResult = remoteAssetAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static RemoteAssetAgent createRemoteAssetAgent() {
    return RemoteAssetAgent.getRemoteAssetAgent(
