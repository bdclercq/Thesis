...
public class ExternalAssetFinderBean implements ExternalAssetFinderLocal {

  private EntityManager entityManager;

  @SuppressWarnings("rawtypes")
  private static final Map<Class<? extends ExternalAssetFindDetails>,
      BiFunction<ExternalAssetFinderBean, ParameterContext, SearchResult<ExternalAssetData>>> finderMapping = generateFinderMapping();

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(ExternalAssetFinderBean.class);
  // @anchor:variables:end

  // anchor:custom-variables:start
  // anchor:custom-variables:end

  public ExternalAssetFinderBean(EntityManager entityManager) {
    this.entityManager = entityManager;
  }

  @SuppressWarnings("rawtypes")
  private static Map<Class<? extends ExternalAssetFindDetails>, BiFunction<ExternalAssetFinderBean, ParameterContext, SearchResult<ExternalAssetData>>> generateFinderMapping() {
    Map<Class<? extends ExternalAssetFindDetails>, BiFunction<ExternalAssetFinderBean, ParameterContext, SearchResult<ExternalAssetData>>> finderMapping = new HashMap<>();
    // anchor:register-finders:start
    finderMapping.put(ExternalAssetFindAllExternalAssetsDetails.class, ExternalAssetFinderBean::findAllExternalAssets);
    finderMapping.put(ExternalAssetFindByNameEqDetails.class, ExternalAssetFinderBean::findByNameEq);
  ...
  public SearchResult<ExternalAssetData> findAllExternalAssets(ParameterContext<SearchDetails<ExternalAssetFindAllExternalAssetsDetails>> searchParameter) {
    SearchDetails<ExternalAssetFindAllExternalAssetsDetails> searchDetails = searchParameter.getValue();
  ...
  @SuppressWarnings({"unchecked", "rawtypes"})
  public SearchResult<ExternalAssetData> findByNameEq(ParameterContext<SearchDetails<ExternalAssetFindByNameEqDetails>> searchParameter) {
    SearchDetails<ExternalAssetFindByNameEqDetails> searchDetails = searchParameter.getValue();

    ExternalAssetFindByNameEqDetails details = searchDetails.getDetails();
  ...
  private SearchResult<ExternalAssetData> fetchData(ParameterContext<SearchDetails<?>> searchParameter, QueryBuilder queryBuilder) {
  ...
  private SearchResult<ExternalAssetData> fetchData(SearchDetails<?> searchDetails, Query countQuery, Query dataQuery) {
    Long total = 0L;
    if (!searchDetails.getSkipCount()) {
      ...
      if (total == 0 || searchDetails.getPaging().getRowsPerPage() == 0) {
        return SearchResult.success(Collections.<ExternalAssetData>emptyList(), total.intValue());
      }
    ...
    List<ExternalAssetData> resultDatas = (List<ExternalAssetData>) dataQuery.getResultList();
  ...
  @Override
  public <S extends ExternalAssetFindDetails> SearchResult<ExternalAssetData> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<ExternalAssetData> searchResult;
    ...
    Class<? extends ExternalAssetFindDetails> findDetailsClass = searchParameter.getValue().getDetails().getClass();
    try {
      if (finderMapping.containsKey(findDetailsClass)) {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
              findDetailsClass.getName() + " does not match a finder in ExternalAssetFinder.find()"
          );
        }

        searchResult = SearchResult.error(Diagnostic.error("assets", "externalAsset", "invalidFinderException"));
      }
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Exception when processing " + findDetailsClass.getName() + " in ExternalAssetFinder.find() ", e
        );
      }

      searchResult = SearchResult.error(Diagnostic.error("assets", "externalAsset", "finderException"));
    }
  ...
  private JPAQueryBuilder createQueryBuilder() {
    return new JPAQueryBuilder(ExternalAssetData.ENTITY_NAME, "name");
  }
