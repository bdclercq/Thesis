...
public class InternalAssetFinderBean implements InternalAssetFinderLocal {

  private EntityManager entityManager;

  @SuppressWarnings("rawtypes")
  private static final Map<Class<? extends InternalAssetFindDetails>,
      BiFunction<InternalAssetFinderBean, ParameterContext, SearchResult<InternalAssetData>>> finderMapping = generateFinderMapping();

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(InternalAssetFinderBean.class);
  // @anchor:variables:end

  // anchor:custom-variables:start
  // anchor:custom-variables:end

  public InternalAssetFinderBean(EntityManager entityManager) {
    this.entityManager = entityManager;
  }

  @SuppressWarnings("rawtypes")
  private static Map<Class<? extends InternalAssetFindDetails>, BiFunction<InternalAssetFinderBean, ParameterContext, SearchResult<InternalAssetData>>> generateFinderMapping() {
    Map<Class<? extends InternalAssetFindDetails>, BiFunction<InternalAssetFinderBean, ParameterContext, SearchResult<InternalAssetData>>> finderMapping = new HashMap<>();
    // anchor:register-finders:start
    finderMapping.put(InternalAssetFindAllInternalAssetsDetails.class, InternalAssetFinderBean::findAllInternalAssets);
    finderMapping.put(InternalAssetFindByNameEqDetails.class, InternalAssetFinderBean::findByNameEq);
  ...
  public SearchResult<InternalAssetData> findAllInternalAssets(ParameterContext<SearchDetails<InternalAssetFindAllInternalAssetsDetails>> searchParameter) {
    SearchDetails<InternalAssetFindAllInternalAssetsDetails> searchDetails = searchParameter.getValue();
  ...
  @SuppressWarnings({"unchecked", "rawtypes"})
  public SearchResult<InternalAssetData> findByNameEq(ParameterContext<SearchDetails<InternalAssetFindByNameEqDetails>> searchParameter) {
    SearchDetails<InternalAssetFindByNameEqDetails> searchDetails = searchParameter.getValue();

    InternalAssetFindByNameEqDetails details = searchDetails.getDetails();
  ...
  private SearchResult<InternalAssetData> fetchData(ParameterContext<SearchDetails<?>> searchParameter, QueryBuilder queryBuilder) {
  ...
  private SearchResult<InternalAssetData> fetchData(SearchDetails<?> searchDetails, Query countQuery, Query dataQuery) {
    Long total = 0L;
    if (!searchDetails.getSkipCount()) {
      ...
      if (total == 0 || searchDetails.getPaging().getRowsPerPage() == 0) {
        return SearchResult.success(Collections.<InternalAssetData>emptyList(), total.intValue());
      }
    ...
    List<InternalAssetData> resultDatas = (List<InternalAssetData>) dataQuery.getResultList();
  ...
  @Override
  public <S extends InternalAssetFindDetails> SearchResult<InternalAssetData> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<InternalAssetData> searchResult;
    ...
    Class<? extends InternalAssetFindDetails> findDetailsClass = searchParameter.getValue().getDetails().getClass();
    try {
      if (finderMapping.containsKey(findDetailsClass)) {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
              findDetailsClass.getName() + " does not match a finder in InternalAssetFinder.find()"
          );
        }

        searchResult = SearchResult.error(Diagnostic.error("assets", "internalAsset", "invalidFinderException"));
      }
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Exception when processing " + findDetailsClass.getName() + " in InternalAssetFinder.find() ", e
        );
      }

      searchResult = SearchResult.error(Diagnostic.error("assets", "internalAsset", "finderException"));
    }
  ...
  private JPAQueryBuilder createQueryBuilder() {
    return new JPAQueryBuilder(InternalAssetData.ENTITY_NAME, "name");
  }
