...
public class FileAssetMapper implements IDataElementMapper<FileAssetDetails> {
  private static final Logger logger = LoggerFactory.getLogger(FileAssetMapper.class);
  ...
  public Result<Map<String, String>> convertToMap(ParameterContext<FileAssetDetails> parameter) {
    FileAssetDetails fileAssetDetails = parameter.getValue();
    try {
      Map<String, String> map = new HashMap<String, String>();

      // anchor:convert-fields-to-map:start
      if (fileAssetDetails.getName() != null) {
        map.put("name", fileAssetDetails.getName());
      }

      if (fileAssetDetails.getUploadUri() != null) {
        map.put("uploadUri", fileAssetDetails.getUploadUri());
      }
      ...
      if (logger.isErrorEnabled()) {
          logger.error(
          "Unexpected error while mapping instance " + fileAssetDetails.getId() + " to map", e
          );
      }
  ...
  public Result<FileAssetDetails> convertToDetails(ParameterContext<Map<String, String>> parameter) {
    try {
      ...
      if (nameValueFromMap != null && !nameValueFromMap.isEmpty()) {
        fileAssetDetails.setName(nameValueFromMap);
      }

      String uploadUriValueFromMap = map.get("uploadUri");
      if (uploadUriValueFromMap != null && !uploadUriValueFromMap.isEmpty()) {
        fileAssetDetails.setUploadUri(uploadUriValueFromMap);
      }
      ...
      return Result.success(fileAssetDetails);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
          logger.error(
