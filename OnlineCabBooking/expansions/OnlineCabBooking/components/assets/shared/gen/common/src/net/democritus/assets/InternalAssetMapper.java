...
public class InternalAssetMapper implements IDataElementMapper<InternalAssetDetails> {
  private static final Logger logger = LoggerFactory.getLogger(InternalAssetMapper.class);
  ...
  public Result<Map<String, String>> convertToMap(ParameterContext<InternalAssetDetails> parameter) {
    InternalAssetDetails internalAssetDetails = parameter.getValue();
    try {
      Map<String, String> map = new HashMap<String, String>();

      // anchor:convert-fields-to-map:start
      if (internalAssetDetails.getName() != null) {
        map.put("name", internalAssetDetails.getName());
      }
      ...
      if (logger.isErrorEnabled()) {
          logger.error(
          "Unexpected error while mapping instance " + internalAssetDetails.getId() + " to map", e
          );
      }
  ...
  public Result<InternalAssetDetails> convertToDetails(ParameterContext<Map<String, String>> parameter) {
    try {
      ...
      if (nameValueFromMap != null && !nameValueFromMap.isEmpty()) {
        internalAssetDetails.setName(nameValueFromMap);
      }
      ...
      return Result.success(internalAssetDetails);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
          logger.error(
