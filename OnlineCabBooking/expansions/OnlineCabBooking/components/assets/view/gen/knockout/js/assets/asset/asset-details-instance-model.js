  ...
  function defineInstanceView(input, options) {
    ...
    function initViewmodel() {
      return {
        ...
        name: ko.observable(),
        type: ko.observable(),
        fileAsset: defineLink(FileAssetEvents),
        internalAsset: defineLink(InternalAssetEvents),
        contentType: ko.observable(),
        byteSize: ko.observable(),
        fileId: ko.observable(),
        complete: ko.observable(),
        externalAsset: defineLink(ExternalAssetEvents)
    ...
    function updateViewmodel(assetDetails) {
      if (!!assetDetails) {
        viewmodel.id(assetDetails.id);
        // anchor:update-viewModel:start
        viewmodel.name(assetDetails.name);
        viewmodel.type(assetDetails.type);
        viewmodel.fileAsset.value(assetDetails.fileAsset);
        viewmodel.internalAsset.value(assetDetails.internalAsset);
        viewmodel.contentType(assetDetails.contentType);
        viewmodel.byteSize(assetDetails.byteSize);
        viewmodel.fileId(assetDetails.fileId);
        viewmodel.complete(assetDetails.complete);
        viewmodel.externalAsset.value(assetDetails.externalAsset);
