...
define(function (require) {
  ...
  var FileAssetForm = require('assets/fileAsset/fileAsset-instance-form');
  var FileAssetActions = require('assets/fileAsset/fileAsset-actions');
  ...
  function defineCreateDialog(options) {
    ...
    when(createRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to create a(n) FileAsset element!")
    });

    function render() {
      var createIntent = FileAssetEvents.defineCreateIntentListener();

      var constants = ko.observable();
      var values = ko.observable();

      var formView = "assets/fileAsset-form";
      var title = "FileAsset";
      var layoutConfig = initLayoutConfig(constants);
      var defaultValues = { id: 0 };
      var formOptions = {
        resetOn: createIntent,
        action: FileAssetActions.createAction()
      };
      ...
      var form = FileAssetForm.defineForm({
        data: formData,
  ...
  function initLayoutConfig(constants) {
    return {
      ...
      uploadUri: {
          disabled: ko.pureComputed(function () {
            return utils.isDefined(constants().uploadUri)
          })
      }
