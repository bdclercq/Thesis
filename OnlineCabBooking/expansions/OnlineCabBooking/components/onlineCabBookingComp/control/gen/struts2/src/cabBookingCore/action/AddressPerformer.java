...
public class AddressPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AddressPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      AddressDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "AddressPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(AddressDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", AddressDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(AddressDetails details) throws Exception {
    Method setStatusMethod = AddressDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    addressAgent.modify(details);
  }
