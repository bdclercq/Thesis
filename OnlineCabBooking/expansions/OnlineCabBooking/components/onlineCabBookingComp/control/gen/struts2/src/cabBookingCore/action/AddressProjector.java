...
public class AddressProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AddressProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Address.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Address.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "onlineCabBookingComp", "cabBookingCore", "Address");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    AddressAgent addressAgent = createAddressAgent();
    crudsResult = addressAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static AddressAgent createAddressAgent() {
    return AddressAgent.getAddressAgent(
