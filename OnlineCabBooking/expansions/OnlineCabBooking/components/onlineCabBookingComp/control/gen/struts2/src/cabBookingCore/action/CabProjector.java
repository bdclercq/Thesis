...
public class CabProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(CabProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Cab.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Cab.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "onlineCabBookingComp", "cabBookingCore", "Cab");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    CabAgent cabAgent = createCabAgent();
    crudsResult = cabAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static CabAgent createCabAgent() {
    return CabAgent.getCabAgent(
