...
public class CarTypeProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(CarTypeProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "CarType.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "CarType.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "onlineCabBookingComp", "cabBookingCore", "CarType");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    CarTypeAgent carTypeAgent = createCarTypeAgent();
    crudsResult = carTypeAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static CarTypeAgent createCarTypeAgent() {
    return CarTypeAgent.getCarTypeAgent(
