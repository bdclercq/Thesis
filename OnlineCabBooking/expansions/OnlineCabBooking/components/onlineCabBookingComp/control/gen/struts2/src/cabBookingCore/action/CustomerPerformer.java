...
public class CustomerPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(CustomerPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      CustomerDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "CustomerPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(CustomerDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", CustomerDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(CustomerDetails details) throws Exception {
    Method setStatusMethod = CustomerDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    customerAgent.modify(details);
  }
