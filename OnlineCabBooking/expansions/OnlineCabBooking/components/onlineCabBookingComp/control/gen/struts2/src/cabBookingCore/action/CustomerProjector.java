...
public class CustomerProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(CustomerProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Customer.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Customer.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "onlineCabBookingComp", "cabBookingCore", "Customer");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    CustomerAgent customerAgent = createCustomerAgent();
    crudsResult = customerAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static CustomerAgent createCustomerAgent() {
    return CustomerAgent.getCustomerAgent(
