...
public class DriverPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(DriverPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      DriverDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "DriverPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(DriverDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", DriverDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(DriverDetails details) throws Exception {
    Method setStatusMethod = DriverDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    driverAgent.modify(details);
  }
