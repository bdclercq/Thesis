...
public class DriverProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(DriverProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Driver.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Driver.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "onlineCabBookingComp", "cabBookingCore", "Driver");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    DriverAgent driverAgent = createDriverAgent();
    crudsResult = driverAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static DriverAgent createDriverAgent() {
    return DriverAgent.getDriverAgent(
