...
public class InvoicePerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(InvoicePerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      InvoiceDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "InvoicePerformer failed to perform task", e
        );
      }
  ...
  private void performTask(InvoiceDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", InvoiceDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(InvoiceDetails details) throws Exception {
    Method setStatusMethod = InvoiceDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    invoiceAgent.modify(details);
  }
