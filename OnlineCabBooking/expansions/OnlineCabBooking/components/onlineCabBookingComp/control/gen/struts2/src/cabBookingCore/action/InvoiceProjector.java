...
public class InvoiceProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(InvoiceProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Invoice.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Invoice.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "onlineCabBookingComp", "cabBookingCore", "Invoice");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    InvoiceAgent invoiceAgent = createInvoiceAgent();
    crudsResult = invoiceAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static InvoiceAgent createInvoiceAgent() {
    return InvoiceAgent.getInvoiceAgent(
