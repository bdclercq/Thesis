...
public class PaymentPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(PaymentPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      PaymentDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "PaymentPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(PaymentDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", PaymentDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(PaymentDetails details) throws Exception {
    Method setStatusMethod = PaymentDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    paymentAgent.modify(details);
  }
