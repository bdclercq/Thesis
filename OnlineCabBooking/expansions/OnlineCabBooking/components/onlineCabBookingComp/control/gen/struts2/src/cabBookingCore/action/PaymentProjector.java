...
public class PaymentProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(PaymentProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Payment.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Payment.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "onlineCabBookingComp", "cabBookingCore", "Payment");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    PaymentAgent paymentAgent = createPaymentAgent();
    crudsResult = paymentAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static PaymentAgent createPaymentAgent() {
    return PaymentAgent.getPaymentAgent(
