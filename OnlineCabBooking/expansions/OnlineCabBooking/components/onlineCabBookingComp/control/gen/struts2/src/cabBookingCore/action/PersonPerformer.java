...
public class PersonPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(PersonPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      PersonDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "PersonPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(PersonDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", PersonDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(PersonDetails details) throws Exception {
    Method setStatusMethod = PersonDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    personAgent.modify(details);
  }
