...
public class PersonProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(PersonProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Person.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Person.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "onlineCabBookingComp", "cabBookingCore", "Person");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    PersonAgent personAgent = createPersonAgent();
    crudsResult = personAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static PersonAgent createPersonAgent() {
    return PersonAgent.getPersonAgent(
