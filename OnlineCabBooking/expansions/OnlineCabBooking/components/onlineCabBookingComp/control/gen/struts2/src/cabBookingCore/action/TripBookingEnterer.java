...
public class TripBookingEnterer extends ActionSupport implements Preparable {

  private String tripBookingOid = "";
  private String tripBookingName = null;
  ...
  private static final Logger logger = LoggerFactory.getLogger(TripBookingEnterer.class);
  ...
  public TripBookingDetails getTripBooking() {
    return getTripBookingDetails();
  }
  ...
  public String execute() throws Exception {
    ...
    if (tripBookingName != null) {
      tripBookingDetails.setName(tripBookingName);
    }

    // @anchor:execute-fileUploadOnly-before:start
    // @anchor:execute-fileUploadOnly-before:end

    crudsResult = saveDetails(tripBookingDetails);
    if (crudsResult.isSuccess()) {
      DataRef dataRef = crudsResult.getValue();

      tripBookingName = dataRef.getName();
      tripBookingOid = dataRef.getId().toString();
  ...
  public TripBookingDetails getJsonRoot() {
    return tripBookingDetails;
  }

  private boolean hastripBookingOid() {
    return !(tripBookingOid.equals("") || tripBookingOid.equals("0"));
  }

  private CrudsResult<DataRef> saveDetails(TripBookingDetails tripBookingDetails) {
    boolean isNew;

    if (hastripBookingOid()) {
      tripBookingDetails.setId(new Long(tripBookingOid));
    }

    Long id = tripBookingDetails.getId();
    isNew = id == null || id == 0L;

    if (isNew) {
      return tripBookingAgent.create(tripBookingDetails);
    } else {
      return tripBookingAgent.modify(tripBookingDetails);
    }
  }
