...
public class TripBookingProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(TripBookingProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "TripBooking.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "TripBooking.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "onlineCabBookingComp", "cabBookingCore", "TripBooking");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    TripBookingAgent tripBookingAgent = createTripBookingAgent();
    crudsResult = tripBookingAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static TripBookingAgent createTripBookingAgent() {
    return TripBookingAgent.getTripBookingAgent(
