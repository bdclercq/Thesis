...
public class AddressDetailsProjector implements IDataElementProjector<AddressData, AddressDetails>, IDataProjectorRequired<AddressDetails> {

  private static final List<DataRef> EMPTY_DATA_REF_COLLECTION = new ArrayList<>();

  private final AddressCrudsInternal crudsInternal;

  // anchor:custom-variables:start
  // anchor:custom-variables:end

  public AddressDetailsProjector(AddressCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  public AddressDetails project(ParameterContext<AddressData> dataParameter) {

    AddressDetails projection = new AddressDetails();
  ...
  public void toData(AddressData data, ParameterContext<AddressDetails> projectionParameter) {
    // don't set the data id
    AddressDetails projection = projectionParameter.getValue();

    // anchor:toData-setters:start
    data.setState(projection.getState());
  ...
  public DataRef getDataRef(AddressDetails projection) {
  ...
  public Class<AddressDetails> getProjectionClass() {
  ...
  public CrudsResult<Void> checkRequired(ParameterContext<AddressDetails> projectionParameter) {
