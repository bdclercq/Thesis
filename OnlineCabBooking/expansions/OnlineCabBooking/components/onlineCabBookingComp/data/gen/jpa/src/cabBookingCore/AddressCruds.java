...
@Local({AddressCrudsLocal.class, AddressCrudsInternal.class})
// @anchor:annotations:end
public class AddressCruds /*@anchor:interfaces:start@*/implements AddressCrudsLocal, AddressCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("onlineCabBookingComp", "Address");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "onlineCabBookingComp.address.alreadyDisabled";
  ...
  private AddressFinderLocal addressFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(AddressCruds.class);
  ...
  private AddressDataRefProjector dataRefProjector;
  private AddressDetailsProjector detailsProjector;
  private AddressInfoProjector infoProjector;

  private DataElementProjectorManager<AddressData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    AddressCrudsInternal addressCrudsInternal = sessionContext.getBusinessObject(AddressCrudsInternal.class);

    detailsProjector = new AddressDetailsProjector(addressCrudsInternal);
    infoProjector = new AddressInfoProjector(addressCrudsInternal);
    dataRefProjector = new AddressDataRefProjector(addressCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<AddressData>();
  ...
  private void initFinder() {
    addressFinder = new AddressFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<AddressDetails> detailsParameter) {
    ...
    AddressDetails details = detailsParameter.getValue();
    ...
    try {
      ...
      detailsProjector.toData(addressData, detailsParameter);
    ...
    try {
      ...
      if (addressData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created AddressCruds with { id : " + addressData.getId() + ", name: " + addressData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(addressData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<AddressDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    AddressDetails addressDetails = detailsParameter.getValue();
    DataRef dataRef = addressDetails.getDataRef();
    ...
    try {
      CrudsResult<AddressData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      AddressData addressData = dataResult.getValue();
      ...
      if (addressData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified AddressCruds with { id : " + addressData.getId() + ", name: " + addressData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "AddressCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof AddressDetails) {
      AddressDetails details = (AddressDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(addressData);
      // @anchor:delete:end
      ...
      if (addressData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted AddressCruds with { id : " + addressData.getId() + ", name: " + addressData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<AddressData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      AddressData addressData = entityManager.find(AddressData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(addressData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<AddressData> getDataById(ParameterContext<Long> idParameter) {
    AddressData data = entityManager.find(AddressData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<AddressData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<AddressData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      AddressFindByNameEqDetails findByNameEqDetails = new AddressFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<AddressFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<AddressFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<AddressData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<AddressData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<AddressData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<AddressData> result;
  ...
  private CrudsResult<AddressData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<AddressData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends AddressFindDetails> SearchResult<AddressData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return addressFinder.find(searchParameter);
  }

  public <S extends AddressFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<AddressData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<AddressData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<AddressData> fromItems, IDataElementProjector<AddressData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<AddressData> fromItems, IDataElementProjector<AddressData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<AddressData>> listParameter, String projection) {
    IDataElementProjector<AddressData, P> elementProjector = getElementProjector(projection);
    List<AddressData> list = listParameter.getValue();
  ...
  private <P> IDataElementProjector<AddressData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<AddressData> dataParameter) {
    ...
    AddressData data = dataParameter.getValue();
    String displayName;
  ...
  private Option<String> getCustomDisplayName(ParameterContext<AddressData> dataParameter) {
  ...
  private <T> IDataElementProjector<AddressData, T> getElementProjector(String projectionName) {
    IDataElementProjector<AddressData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("onlineCabBookingComp", "Address"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<AddressData> dataResult = getData(projectionRefParameter.construct(dataRef));
    AddressData addressData;
    if (dataResult.isSuccess()) {
      addressData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<AddressData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(addressData));

    if (addressData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved AddressCruds:" + projectionRef.getProjection() + " { id : " + addressData.getId() + ", name: " + addressData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<AddressDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<AddressData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    AddressData addressData = result.getValue();
    AddressDetails addressDetails = detailsProjector.project(dataRefParameter.construct(addressData));

    return CrudsResult.success(addressDetails);
  }

  public CrudsResult<List<AddressDetails>> getDetailsListFromData(ParameterContext<Collection<AddressData>> dataListParameter) {
    Collection<AddressData> dataList = dataListParameter.getValue();
    List<AddressDetails> list = new ArrayList<>(dataList.size());

    for (AddressData addressData: dataList) {
      AddressDetails addressDetails = detailsProjector.project(dataListParameter.construct(addressData));
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<AddressData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    AddressData data = dataResult.getValue();
