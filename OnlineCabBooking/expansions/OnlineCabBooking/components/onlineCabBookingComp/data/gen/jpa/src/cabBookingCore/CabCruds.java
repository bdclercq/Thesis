...
@Local({CabCrudsLocal.class, CabCrudsInternal.class})
// @anchor:annotations:end
public class CabCruds /*@anchor:interfaces:start@*/implements CabCrudsLocal, CabCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("onlineCabBookingComp", "Cab");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "onlineCabBookingComp.cab.alreadyDisabled";
  ...
  private CabFinderLocal cabFinder;

  /* linked bean variables */
  // anchor:link-variables:start
  @EJB
  private CarTypeCrudsInternal carTypeLocal;
  @EJB
  private DriverCrudsInternal driverLocal;
  // anchor:link-variables:end

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(CabCruds.class);
  ...
  private CabDataRefProjector dataRefProjector;
  private CabDetailsProjector detailsProjector;
  private CabInfoProjector infoProjector;

  private DataElementProjectorManager<CabData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    CabCrudsInternal cabCrudsInternal = sessionContext.getBusinessObject(CabCrudsInternal.class);

    detailsProjector = new CabDetailsProjector(cabCrudsInternal);
    infoProjector = new CabInfoProjector(cabCrudsInternal);
    dataRefProjector = new CabDataRefProjector(cabCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<CabData>();
  ...
  private void initFinder() {
    cabFinder = new CabFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<CabDetails> detailsParameter) {
    ...
    CabDetails details = detailsParameter.getValue();
    ...
    try {
      ...
      detailsProjector.toData(cabData, detailsParameter);
    ...
    try {
      ...
      if (cabData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created CabCruds with { id : " + cabData.getId() + ", name: " + cabData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(cabData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<CabDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    CabDetails cabDetails = detailsParameter.getValue();
    DataRef dataRef = cabDetails.getDataRef();
    ...
    try {
      CrudsResult<CabData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      CabData cabData = dataResult.getValue();
      ...
      if (cabData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified CabCruds with { id : " + cabData.getId() + ", name: " + cabData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "CabCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof CabDetails) {
      CabDetails details = (CabDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      if (cabData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted CabCruds with { id : " + cabData.getId() + ", name: " + cabData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<CabData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      CabData cabData = entityManager.find(CabData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(cabData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<CabData> getDataById(ParameterContext<Long> idParameter) {
    CabData data = entityManager.find(CabData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<CabData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<CabData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      CabFindByNameEqDetails findByNameEqDetails = new CabFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<CabFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<CabFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<CabData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<CabData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<CabData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  private CrudsResult<CabData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<CabData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends CabFindDetails> SearchResult<CabData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return cabFinder.find(searchParameter);
  }

  public <S extends CabFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<CabData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<CabData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<CabData> fromItems, IDataElementProjector<CabData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<CabData> fromItems, IDataElementProjector<CabData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<CabData>> listParameter, String projection) {
    IDataElementProjector<CabData, P> elementProjector = getElementProjector(projection);
    List<CabData> list = listParameter.getValue();
  ...
  public CrudsResult<DataRef> getCarType(ParameterContext<Long> idParameter) {
    try {
      return carTypeLocal.getDataRefFromId(idParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  public CrudsResult<CarTypeDetails> getCarTypeDetails(ParameterContext<DataRef> dataRefParameter) {
    try {
      return carTypeLocal.getDetails(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  @Override
  public void setCarType(CabData cabData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      cabData.setCarType(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
      result = carTypeLocal.resolveDataRef(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
    ...
    if (result.isSuccess()) {
      cabData.setCarType(result.getValue().getId());
    } else {
      throw new IllegalStateException("Failed to find CarType instance '" + dataRefParameter.getValue() + "'");
    }
  }

  @Override
  public void setDriver(CabData cabData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      cabData.setDriver(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
    ...
    if (result.isSuccess()) {
      cabData.setDriver(result.getValue().getId());
    } else {
  ...
  private <P> IDataElementProjector<CabData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<CabData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<CabData> dataParameter) {
  ...
  private <T> IDataElementProjector<CabData, T> getElementProjector(String projectionName) {
    IDataElementProjector<CabData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("onlineCabBookingComp", "Cab"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<CabData> dataResult = getData(projectionRefParameter.construct(dataRef));
    CabData cabData;
    if (dataResult.isSuccess()) {
      cabData = dataResult.getValue();
    } else {
      cabData = new CabData();
    }
    ...
    IDataElementProjector<CabData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(cabData));

    if (cabData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved CabCruds:" + projectionRef.getProjection() + " { id : " + cabData.getId() + ", name: " + cabData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<CabDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<CabData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    CabData cabData = result.getValue();
    CabDetails cabDetails = detailsProjector.project(dataRefParameter.construct(cabData));

    return CrudsResult.success(cabDetails);
  }

  public CrudsResult<List<CabDetails>> getDetailsListFromData(ParameterContext<Collection<CabData>> dataListParameter) {
    Collection<CabData> dataList = dataListParameter.getValue();
    List<CabDetails> list = new ArrayList<>(dataList.size());

    for (CabData cabData: dataList) {
      CabDetails cabDetails = detailsProjector.project(dataListParameter.construct(cabData));
      list.add(cabDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<CabData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    CabData data = dataResult.getValue();
