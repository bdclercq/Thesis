...
// anchor:imports:start
import cabBookingCore.PersonData;
import cabBookingCore.PersonDetails;
import cabBookingCore.PersonCrudsInternal;
// anchor:imports:end
...
@Local({CustomerCrudsLocal.class, CustomerCrudsInternal.class})
// @anchor:annotations:end
public class CustomerCruds /*@anchor:interfaces:start@*/implements CustomerCrudsLocal, CustomerCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("onlineCabBookingComp", "Customer");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "onlineCabBookingComp.customer.alreadyDisabled";
  ...
  private CustomerFinderLocal customerFinder;

  /* linked bean variables */
  // anchor:link-variables:start
  @EJB
  private PersonCrudsInternal personLocal;
  // anchor:link-variables:end

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(CustomerCruds.class);
  ...
  private CustomerDataRefProjector dataRefProjector;
  private CustomerDetailsProjector detailsProjector;
  private CustomerInfoProjector infoProjector;

  private DataElementProjectorManager<CustomerData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    CustomerCrudsInternal customerCrudsInternal = sessionContext.getBusinessObject(CustomerCrudsInternal.class);

    detailsProjector = new CustomerDetailsProjector(customerCrudsInternal);
    infoProjector = new CustomerInfoProjector(customerCrudsInternal);
    dataRefProjector = new CustomerDataRefProjector(customerCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<CustomerData>();
  ...
  private void initFinder() {
    customerFinder = new CustomerFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<CustomerDetails> detailsParameter) {
    ...
    CustomerDetails details = detailsParameter.getValue();
    ...
    try {
      ...
      detailsProjector.toData(customerData, detailsParameter);
    ...
    try {
      ...
      if (identity == null) {
        identity = "C-" + customerData.getId().toString();
      }
      customerData.setName(identity);
      entityManager.flush();
      ...
      if (customerData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created CustomerCruds with { id : " + customerData.getId() + ", name: " + customerData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(customerData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<CustomerDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    CustomerDetails customerDetails = detailsParameter.getValue();
    DataRef dataRef = customerDetails.getDataRef();
    ...
    try {
      CrudsResult<CustomerData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      CustomerData customerData = dataResult.getValue();
      ...
      if (customerData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified CustomerCruds with { id : " + customerData.getId() + ", name: " + customerData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "CustomerCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof CustomerDetails) {
      CustomerDetails details = (CustomerDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      if (customerData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted CustomerCruds with { id : " + customerData.getId() + ", name: " + customerData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<CustomerData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      CustomerData customerData = entityManager.find(CustomerData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(customerData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<CustomerData> getDataById(ParameterContext<Long> idParameter) {
    CustomerData data = entityManager.find(CustomerData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<CustomerData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<CustomerData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      CustomerFindByNameEqDetails findByNameEqDetails = new CustomerFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<CustomerFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<CustomerFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<CustomerData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<CustomerData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<CustomerData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<CustomerData> result;
  ...
  private CrudsResult<CustomerData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<CustomerData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends CustomerFindDetails> SearchResult<CustomerData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return customerFinder.find(searchParameter);
  }

  public <S extends CustomerFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<CustomerData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<CustomerData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<CustomerData> fromItems, IDataElementProjector<CustomerData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<CustomerData> fromItems, IDataElementProjector<CustomerData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<CustomerData>> listParameter, String projection) {
    IDataElementProjector<CustomerData, P> elementProjector = getElementProjector(projection);
    List<CustomerData> list = listParameter.getValue();
  ...
  public CrudsResult<DataRef> getPerson(ParameterContext<Long> idParameter) {
    try {
      return personLocal.getDataRefFromId(idParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  public CrudsResult<PersonDetails> getPersonDetails(ParameterContext<DataRef> dataRefParameter) {
    try {
      return personLocal.getDetails(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  @Override
  public void setPerson(CustomerData customerData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      customerData.setPerson(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
      result = personLocal.resolveDataRef(dataRefParameter);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
    ...
    if (result.isSuccess()) {
      customerData.setPerson(result.getValue().getId());
    } else {
      throw new IllegalStateException("Failed to find Person instance '" + dataRefParameter.getValue() + "'");
    }
  }
  ...
  private <P> IDataElementProjector<CustomerData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<CustomerData> dataParameter) {
    ...
    CustomerData data = dataParameter.getValue();
    String displayName;
  ...
  private Option<String> getCustomDisplayName(ParameterContext<CustomerData> dataParameter) {
  ...
  private <T> IDataElementProjector<CustomerData, T> getElementProjector(String projectionName) {
    IDataElementProjector<CustomerData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("onlineCabBookingComp", "Customer"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<CustomerData> dataResult = getData(projectionRefParameter.construct(dataRef));
    CustomerData customerData;
    if (dataResult.isSuccess()) {
      customerData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<CustomerData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(customerData));

    if (customerData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved CustomerCruds:" + projectionRef.getProjection() + " { id : " + customerData.getId() + ", name: " + customerData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<CustomerDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<CustomerData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    CustomerData customerData = result.getValue();
    CustomerDetails customerDetails = detailsProjector.project(dataRefParameter.construct(customerData));

    return CrudsResult.success(customerDetails);
  }

  public CrudsResult<List<CustomerDetails>> getDetailsListFromData(ParameterContext<Collection<CustomerData>> dataListParameter) {
    Collection<CustomerData> dataList = dataListParameter.getValue();
    List<CustomerDetails> list = new ArrayList<>(dataList.size());

    for (CustomerData customerData: dataList) {
      CustomerDetails customerDetails = detailsProjector.project(dataListParameter.construct(customerData));
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<CustomerData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    CustomerData data = dataResult.getValue();
