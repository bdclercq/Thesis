...
@Local({PaymentCrudsLocal.class, PaymentCrudsInternal.class})
// @anchor:annotations:end
public class PaymentCruds /*@anchor:interfaces:start@*/implements PaymentCrudsLocal, PaymentCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("onlineCabBookingComp", "Payment");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "onlineCabBookingComp.payment.alreadyDisabled";
  ...
  private PaymentFinderLocal paymentFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(PaymentCruds.class);
  ...
  private PaymentDataRefProjector dataRefProjector;
  private PaymentDetailsProjector detailsProjector;
  private PaymentInfoProjector infoProjector;

  private DataElementProjectorManager<PaymentData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    PaymentCrudsInternal paymentCrudsInternal = sessionContext.getBusinessObject(PaymentCrudsInternal.class);

    detailsProjector = new PaymentDetailsProjector(paymentCrudsInternal);
    infoProjector = new PaymentInfoProjector(paymentCrudsInternal);
    dataRefProjector = new PaymentDataRefProjector(paymentCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<PaymentData>();
  ...
  private void initFinder() {
    paymentFinder = new PaymentFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<PaymentDetails> detailsParameter) {
    ...
    PaymentDetails details = detailsParameter.getValue();
    ...
    try {
      ...
      detailsProjector.toData(paymentData, detailsParameter);
    ...
    try {
      entityManager.persist(paymentData);

      // @anchor:implicitNameFieldOnly-afterPersist:start
      if (identity == null) {
        identity = "P-" + paymentData.getId().toString();
      }
      paymentData.setName(identity);
      entityManager.flush();
      ...
      if (paymentData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created PaymentCruds with { id : " + paymentData.getId() + ", name: " + paymentData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(paymentData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<PaymentDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    PaymentDetails paymentDetails = detailsParameter.getValue();
    DataRef dataRef = paymentDetails.getDataRef();
    ...
    try {
      CrudsResult<PaymentData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      PaymentData paymentData = dataResult.getValue();
      ...
      if (paymentData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified PaymentCruds with { id : " + paymentData.getId() + ", name: " + paymentData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "PaymentCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof PaymentDetails) {
      PaymentDetails details = (PaymentDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(paymentData);
      // @anchor:delete:end
      ...
      if (paymentData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted PaymentCruds with { id : " + paymentData.getId() + ", name: " + paymentData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<PaymentData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      PaymentData paymentData = entityManager.find(PaymentData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(paymentData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<PaymentData> getDataById(ParameterContext<Long> idParameter) {
    PaymentData data = entityManager.find(PaymentData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<PaymentData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<PaymentData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      PaymentFindByNameEqDetails findByNameEqDetails = new PaymentFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<PaymentFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<PaymentFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<PaymentData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<PaymentData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<PaymentData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<PaymentData> result;
  ...
  private CrudsResult<PaymentData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<PaymentData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends PaymentFindDetails> SearchResult<PaymentData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return paymentFinder.find(searchParameter);
  }

  public <S extends PaymentFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<PaymentData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<PaymentData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<PaymentData> fromItems, IDataElementProjector<PaymentData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<PaymentData> fromItems, IDataElementProjector<PaymentData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<PaymentData>> listParameter, String projection) {
    IDataElementProjector<PaymentData, P> elementProjector = getElementProjector(projection);
    List<PaymentData> list = listParameter.getValue();
  ...
  private <P> IDataElementProjector<PaymentData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<PaymentData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<PaymentData> dataParameter) {
  ...
  private <T> IDataElementProjector<PaymentData, T> getElementProjector(String projectionName) {
    IDataElementProjector<PaymentData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("onlineCabBookingComp", "Payment"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<PaymentData> dataResult = getData(projectionRefParameter.construct(dataRef));
    PaymentData paymentData;
    if (dataResult.isSuccess()) {
      paymentData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<PaymentData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(paymentData));

    if (paymentData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved PaymentCruds:" + projectionRef.getProjection() + " { id : " + paymentData.getId() + ", name: " + paymentData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<PaymentDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<PaymentData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    PaymentData paymentData = result.getValue();
    PaymentDetails paymentDetails = detailsProjector.project(dataRefParameter.construct(paymentData));

    return CrudsResult.success(paymentDetails);
  }

  public CrudsResult<List<PaymentDetails>> getDetailsListFromData(ParameterContext<Collection<PaymentData>> dataListParameter) {
    Collection<PaymentData> dataList = dataListParameter.getValue();
    List<PaymentDetails> list = new ArrayList<>(dataList.size());

    for (PaymentData paymentData: dataList) {
      PaymentDetails paymentDetails = detailsProjector.project(dataListParameter.construct(paymentData));
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<PaymentData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    PaymentData data = dataResult.getValue();
