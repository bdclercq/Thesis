...
public class PaymentFinderBean implements PaymentFinderLocal {

  private EntityManager entityManager;

  @SuppressWarnings("rawtypes")
  private static final Map<Class<? extends PaymentFindDetails>,
      BiFunction<PaymentFinderBean, ParameterContext, SearchResult<PaymentData>>> finderMapping = generateFinderMapping();

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(PaymentFinderBean.class);
  // @anchor:variables:end

  // anchor:custom-variables:start
  // anchor:custom-variables:end

  public PaymentFinderBean(EntityManager entityManager) {
    this.entityManager = entityManager;
  }

  @SuppressWarnings("rawtypes")
  private static Map<Class<? extends PaymentFindDetails>, BiFunction<PaymentFinderBean, ParameterContext, SearchResult<PaymentData>>> generateFinderMapping() {
    Map<Class<? extends PaymentFindDetails>, BiFunction<PaymentFinderBean, ParameterContext, SearchResult<PaymentData>>> finderMapping = new HashMap<>();
    // anchor:register-finders:start
    finderMapping.put(PaymentFindAllPaymentsDetails.class, PaymentFinderBean::findAllPayments);
    finderMapping.put(PaymentFindByNameEqDetails.class, PaymentFinderBean::findByNameEq);
  ...
  public SearchResult<PaymentData> findAllPayments(ParameterContext<SearchDetails<PaymentFindAllPaymentsDetails>> searchParameter) {
    SearchDetails<PaymentFindAllPaymentsDetails> searchDetails = searchParameter.getValue();
  ...
  @SuppressWarnings({"unchecked", "rawtypes"})
  public SearchResult<PaymentData> findByNameEq(ParameterContext<SearchDetails<PaymentFindByNameEqDetails>> searchParameter) {
    SearchDetails<PaymentFindByNameEqDetails> searchDetails = searchParameter.getValue();

    PaymentFindByNameEqDetails details = searchDetails.getDetails();
  ...
  private SearchResult<PaymentData> fetchData(ParameterContext<SearchDetails<?>> searchParameter, QueryBuilder queryBuilder) {
  ...
  private SearchResult<PaymentData> fetchData(SearchDetails<?> searchDetails, Query countQuery, Query dataQuery) {
    Long total = 0L;
    if (!searchDetails.getSkipCount()) {
      ...
      if (total == 0 || searchDetails.getPaging().getRowsPerPage() == 0) {
        return SearchResult.success(Collections.<PaymentData>emptyList(), total.intValue());
      }
    ...
    List<PaymentData> resultDatas = (List<PaymentData>) dataQuery.getResultList();
  ...
  @Override
  public <S extends PaymentFindDetails> SearchResult<PaymentData> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<PaymentData> searchResult;
    ...
    Class<? extends PaymentFindDetails> findDetailsClass = searchParameter.getValue().getDetails().getClass();
    try {
      if (finderMapping.containsKey(findDetailsClass)) {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
              findDetailsClass.getName() + " does not match a finder in PaymentFinder.find()"
          );
        }

        searchResult = SearchResult.error(Diagnostic.error("onlineCabBookingComp", "payment", "invalidFinderException"));
      }
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Exception when processing " + findDetailsClass.getName() + " in PaymentFinder.find() ", e
        );
      }

      searchResult = SearchResult.error(Diagnostic.error("onlineCabBookingComp", "payment", "finderException"));
    }
  ...
  private JPAQueryBuilder createQueryBuilder() {
    return new JPAQueryBuilder(PaymentData.ENTITY_NAME, "id");
  }
