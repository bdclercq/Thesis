...
/**
 * Example implementation delegate for the task element MakeTripBookingInvoice.
 */
public class MakeTripBookingInvoiceImpl implements MakeTripBookingInvoice {

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(MakeTripBookingInvoiceImpl.class);
  ...
  public TaskResult<Void> perform(ParameterContext<TripBookingDetails> targetParameter) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Performing the MakeTripBookingInvoiceImpl implementation"
      );
    }
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "Exception in MakeTripBookingInvoiceImpl implementation", e
        );
      }
    }
    return taskResult;
