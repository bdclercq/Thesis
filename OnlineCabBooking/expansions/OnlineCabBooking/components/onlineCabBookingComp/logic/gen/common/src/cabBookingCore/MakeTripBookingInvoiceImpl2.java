...
/**
 * Example2 implementation delegate for the task element MakeTripBookingInvoice.
 */
public class MakeTripBookingInvoiceImpl2 implements MakeTripBookingInvoice {

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(MakeTripBookingInvoiceImpl2.class);
  ...
  public TaskResult<Void> perform(ParameterContext<TripBookingDetails> targetParameter) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Performing the MakeTripBookingInvoiceImpl2 implementation"
      );
    }
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "Exception in MakeTripBookingInvoiceImpl2 implementation", e
        );
      }
    }
    return taskResult;
