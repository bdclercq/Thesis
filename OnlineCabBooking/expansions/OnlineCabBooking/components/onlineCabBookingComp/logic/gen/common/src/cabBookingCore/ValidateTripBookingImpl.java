...
/**
 * Example implementation delegate for the task element ValidateTripBooking.
 */
public class ValidateTripBookingImpl implements ValidateTripBooking {

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(ValidateTripBookingImpl.class);
  ...
  public TaskResult<Void> perform(ParameterContext<TripBookingDetails> targetParameter) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Performing the ValidateTripBookingImpl implementation"
      );
    }
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "Exception in ValidateTripBookingImpl implementation", e
        );
      }
    }
    return taskResult;
