...
// anchor:imports:start
import cabBookingCore.CustomerLocal;
import cabBookingCore.CustomerDetails;
import cabBookingCore.DriverLocal;
import cabBookingCore.DriverDetails;
import cabBookingCore.AddressLocal;
import cabBookingCore.AddressDetails;
import cabBookingCore.PaymentLocal;
...
@Remote(TripBookingRemote.class)
@Local(TripBookingLocal.class)
// @anchor:annotations:end
@DeclareRoles({
// anchor:custom-declare-roles:start
// anchor:custom-declare-roles:end
})
public class TripBookingBean /*@anchor:interfaces:start@*/implements TripBookingRemote, TripBookingLocal/*@anchor:interfaces:end@*/{
  ...
  @EJB private CustomerLocal customerLocal;

  @EJB private DriverLocal driverLocal;

  @EJB private AddressLocal addressLocal;

  @EJB private PaymentLocal paymentLocal;

  // anchor:link-variables:end

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(TripBookingBean.class);
  @Deprecated
  @PersistenceContext(unitName="OnlineCabBooking_onlineCabBookingComp")
  private EntityManager entityManager;

  @EJB
  TripBookingCrudsLocal tripBookingCrudsLocal;
  ...
  public CrudsResult<DataRef> create(ParameterContext<TripBookingDetails> detailsParameter) {
    ...
    CrudsResult<DataRef> result = tripBookingCrudsLocal.create(context.withParameter(details));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<TripBookingDetails> detailsParameter) {
    ...
    CrudsResult<DataRef> result = tripBookingCrudsLocal.modify(context.withParameter(details));
    if (result.isError()) {
      return result;
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    CrudsResult<DataRef> result = tripBookingCrudsLocal.createOrModify(projectionParameter);
    if (result.isError()) {
      return result;
    }
  ...
  public CrudsResult<Void> delete(ParameterContext<Long> idParameter) {
    ...
    try {
      ...
      CrudsResult<Void> result = tripBookingCrudsLocal.delete(context.withParameter(dataRef));
  ...
  public CrudsResult<DataRef> getId(ParameterContext<String> nameParameter) {
    return tripBookingCrudsLocal.getDataRefFromName(nameParameter);
  }

  public CrudsResult<String> getName(ParameterContext<Long> idParameter) {
    // @anchor:before-getName:start
    // @anchor:before-getName:end
    CrudsResult<DataRef> dataRefResult = tripBookingCrudsLocal.getDataRefFromId(idParameter);
  ...
  public DataRef getDataRef(Long id) {
    return tripBookingCrudsLocal.getDataRefFromId(ParameterContext.create(null, id)).getValue();
  }

  @Deprecated
  public TripBookingInfo getInfo(Long id) {
    return getInfo(ParameterContext.create(null, id)).getValue();
  }

  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public TripBookingDetails getDetails(Long id) {
    return getDetailsFromDataRef(ParameterContext.create(null, idToDataRef(id))).getValue();
  }

  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public TripBookingDetails getDetails(DataRef dataRef) {
  ...
  public CrudsResult<DataRef> getDataRef(ParameterContext<Long> idParameter) {
    return tripBookingCrudsLocal.getDataRefFromId(idParameter);
  }

  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    return tripBookingCrudsLocal.resolveDataRef(dataRefParameter);
  }

  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<TripBookingInfo> getInfo(ParameterContext<Long> idParameter) {
    DataRef dataRef = idToDataRef(idParameter.getValue());
  ...
  public CrudsResult<TripBookingDetails> getDetails(ParameterContext<Long> idParameter) {
    ...
    CrudsResult<TripBookingDetails> result = getDetailsFromDataRef(idParameter.construct(dataRef));
  ...
  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<TripBookingDetails> getDetailsFromDataRef(ParameterContext<DataRef> dataRefParameter) {
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    result = tripBookingCrudsLocal.getProjection(projectionRefParameter);
  ...
  public <S extends TripBookingFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    ...
    result = tripBookingCrudsLocal.find(searchParameter);
