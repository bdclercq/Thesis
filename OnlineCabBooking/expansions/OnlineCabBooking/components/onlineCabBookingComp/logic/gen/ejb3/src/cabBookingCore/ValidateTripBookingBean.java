...
/**
 * Stateless session bean encapsulating the implementation of the task ValidateTripBooking.
 */

@Stateless
@Remote(ValidateTripBookingRemote.class)
@Local(ValidateTripBookingLocal.class)
...
public class ValidateTripBookingBean implements ValidateTripBookingRemote, ValidateTripBookingLocal {

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(ValidateTripBookingBean.class);
  ...
  public TaskResult<Void> perform(ParameterContext<TripBookingDetails> targetParameter) {
    ...
    final ValidateTripBooking implementation;
    try {
      String implName = targetParameter.getProcessingContext().getImplementation();
      implementation = (ValidateTripBooking) Class.forName(implName).newInstance();
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
      ...
      return TaskResult.error(Diagnostic.error("onlineCabBookingComp", "ValidateTripBooking", "NO_VALID_IMPLEMENTATION_CLASS"));
    }
    ...
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "Failed to perform ValidateTripBooking on TripBookingDetails", e
        );
      }
    }
