...
/**
 * interface for the entity bean AddressBean,
 * representing a known Address
 */
public interface AddressAgentIf {
  ...
  CrudsResult<DataRef> create(AddressDetails addressDetails);
  CrudsResult<DataRef> modify(AddressDetails addressDetails);
  ...
  CrudsResult<AddressDetails> getDetails(DataRef dataRef);
  ...
  <S extends AddressFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<AddressDetails> getDetails(Long addressId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<AddressInfo> findAllInfos();
