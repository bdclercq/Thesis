...
/**
 * interface for the entity bean CabBean,
 * representing a known Cab
 */
public interface CabAgentIf {
  ...
  CrudsResult<DataRef> create(CabDetails cabDetails);
  CrudsResult<DataRef> modify(CabDetails cabDetails);
  ...
  CrudsResult<CabDetails> getDetails(DataRef dataRef);
  ...
  <S extends CabFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<CabDetails> getDetails(Long cabId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<CabInfo> findAllInfos();
