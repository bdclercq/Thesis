...
/**
 * interface for the entity bean CustomerBean,
 * representing a known Customer
 */
public interface CustomerAgentIf {
  ...
  CrudsResult<DataRef> create(CustomerDetails customerDetails);
  CrudsResult<DataRef> modify(CustomerDetails customerDetails);
  ...
  CrudsResult<CustomerDetails> getDetails(DataRef dataRef);
  ...
  <S extends CustomerFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<CustomerDetails> getDetails(Long customerId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<CustomerInfo> findAllInfos();
