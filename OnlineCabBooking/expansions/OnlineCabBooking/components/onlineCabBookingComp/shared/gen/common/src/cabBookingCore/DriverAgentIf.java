...
/**
 * interface for the entity bean DriverBean,
 * representing a known Driver
 */
public interface DriverAgentIf {
  ...
  CrudsResult<DataRef> create(DriverDetails driverDetails);
  CrudsResult<DataRef> modify(DriverDetails driverDetails);
  ...
  CrudsResult<DriverDetails> getDetails(DataRef dataRef);
  ...
  <S extends DriverFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<DriverDetails> getDetails(Long driverId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<DriverInfo> findAllInfos();
