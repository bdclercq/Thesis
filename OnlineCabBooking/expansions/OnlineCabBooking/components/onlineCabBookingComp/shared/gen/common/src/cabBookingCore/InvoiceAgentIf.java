...
/**
 * interface for the entity bean InvoiceBean,
 * representing a known Invoice
 */
public interface InvoiceAgentIf {
  ...
  CrudsResult<DataRef> create(InvoiceDetails invoiceDetails);
  CrudsResult<DataRef> modify(InvoiceDetails invoiceDetails);
  ...
  CrudsResult<InvoiceDetails> getDetails(DataRef dataRef);
  ...
  <S extends InvoiceFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<InvoiceDetails> getDetails(Long invoiceId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<InvoiceInfo> findAllInfos();
