...
/**
 * interface for the entity bean PaymentBean,
 * representing a known Payment
 */
public interface PaymentAgentIf {
  ...
  CrudsResult<DataRef> create(PaymentDetails paymentDetails);
  CrudsResult<DataRef> modify(PaymentDetails paymentDetails);
  ...
  CrudsResult<PaymentDetails> getDetails(DataRef dataRef);
  ...
  <S extends PaymentFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<PaymentDetails> getDetails(Long paymentId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<PaymentInfo> findAllInfos();
