...
/**
 * interface for the entity bean PersonBean,
 * representing a known Person
 */
public interface PersonAgentIf {
  ...
  CrudsResult<DataRef> create(PersonDetails personDetails);
  CrudsResult<DataRef> modify(PersonDetails personDetails);
  ...
  CrudsResult<PersonDetails> getDetails(DataRef dataRef);
  ...
  <S extends PersonFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<PersonDetails> getDetails(Long personId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<PersonInfo> findAllInfos();
