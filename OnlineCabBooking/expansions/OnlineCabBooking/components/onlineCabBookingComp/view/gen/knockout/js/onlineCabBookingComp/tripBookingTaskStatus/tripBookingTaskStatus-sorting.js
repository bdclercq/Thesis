...
define(function (require) {
  ...
  function defineSorting(input, options) {
    ...
    viewModel.startedAt = SortableColumn.defineSortableColumn({
      fieldName: "startedAt"
    }, {
      resetOn: Trigger.defineFilter({
        trigger: selectSorting,
        condition: function (sortField) {
          return sortField === null || sortField.fieldName !== "startedAt";
        }
      })
    });

    viewModel.finishedAt = SortableColumn.defineSortableColumn({
      fieldName: "finishedAt"
    }, {
      resetOn: Trigger.defineFilter({
        trigger: selectSorting,
        condition: function (sortField) {
          return sortField === null || sortField.fieldName !== "finishedAt";
        }
      })
    });
    ...
    selectSorting.addTrigger(viewModel.startedAt.select);
    selectSorting.addTrigger(viewModel.finishedAt.select);
