...
public class ThumbnailPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ThumbnailPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      ThumbnailDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ThumbnailPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(ThumbnailDetails details) throws Exception {
    Class<?> agentClass = Class.forName("net.democritus.gui." + taskName + "Agent");
    ...
    Method performMethod = agentClass.getMethod("perform", ThumbnailDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(ThumbnailDetails details) throws Exception {
    Method setStatusMethod = ThumbnailDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    thumbnailAgent.modify(details);
  }
