...
public class ThumbnailProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ThumbnailProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Thumbnail.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Thumbnail.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "utils", "net.democritus.gui", "Thumbnail");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    ThumbnailAgent thumbnailAgent = createThumbnailAgent();
    crudsResult = thumbnailAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static ThumbnailAgent createThumbnailAgent() {
    return ThumbnailAgent.getThumbnailAgent(
