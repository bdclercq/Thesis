...
public class ExecutionDetailer extends ActionSupport implements Preparable {
  ...
  public CrudsResult<ExecutionDetails> getCrudsResult() {
    return crudsResult;
  }

  public JsonResult<ExecutionDetails> getJsonResult() {
  ...
  public String execute() throws Exception {
    ...
    if (executionOid.length() > 0) {
      if (shouldClone(executionOid)) {
        executionId = new Long(executionOid.substring(1));
        executionOid = "";
      } else {
      ...
      if (crudsResult.isError()) {
        ...
        executionDetails = crudsResult.getValue();
        executionName = executionDetails.getName();
      ...
      if (actionResult.equals(Action.SUCCESS)) {
        crudsResult = CrudsResult.success(executionDetails);
      } else {
          Diagnostic diagnostic = Diagnostic.error("utils", "Execution", parentField, "cannotSetParentField");
  ...
  private boolean shouldClone(String executionOid) {
    return executionOid.startsWith("c");
  }
  ...
  private String setParent(Long parentOid) {
    try {
        ...
        Method getParentDataRefMethod = ExecutionAgent.class.getMethod(getParentDataRefMethodName, new Class[] {Long.class});

        DataRef parentRef = (DataRef) getParentDataRefMethod.invoke(executionAgent, new Object[] {parentOid});

        String setParentMethodName = "set"+parentField;
        Method setParentMethod = ExecutionDetails.class.getMethod(setParentMethodName, new Class[] {DataRef.class});

        setParentMethod.invoke(executionDetails, new Object[] {parentRef});
