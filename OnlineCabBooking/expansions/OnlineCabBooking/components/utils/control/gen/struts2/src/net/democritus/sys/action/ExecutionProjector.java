...
public class ExecutionProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ExecutionProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Execution.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Execution.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "utils", "net.democritus.sys", "Execution");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    ExecutionAgent executionAgent = createExecutionAgent();
    crudsResult = executionAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static ExecutionAgent createExecutionAgent() {
    return ExecutionAgent.getExecutionAgent(
