...
public class IdCounterPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(IdCounterPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      IdCounterDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "IdCounterPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(IdCounterDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", IdCounterDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(IdCounterDetails details) throws Exception {
    Method setStatusMethod = IdCounterDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    idCounterAgent.modify(details);
  }
