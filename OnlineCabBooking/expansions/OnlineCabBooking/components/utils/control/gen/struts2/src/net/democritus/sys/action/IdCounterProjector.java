...
public class IdCounterProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(IdCounterProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "IdCounter.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "IdCounter.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "utils", "net.democritus.sys", "IdCounter");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    IdCounterAgent idCounterAgent = createIdCounterAgent();
    crudsResult = idCounterAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static IdCounterAgent createIdCounterAgent() {
    return IdCounterAgent.getIdCounterAgent(
