...
public class ParamTargetValuePerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ParamTargetValuePerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      ParamTargetValueDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ParamTargetValuePerformer failed to perform task", e
        );
      }
  ...
  private void performTask(ParamTargetValueDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", ParamTargetValueDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(ParamTargetValueDetails details) throws Exception {
    Method setStatusMethod = ParamTargetValueDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    paramTargetValueAgent.modify(details);
  }
