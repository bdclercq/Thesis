...
public class ParamTargetValueProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ParamTargetValueProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "ParamTargetValue.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "ParamTargetValue.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "utils", "net.democritus.sys", "ParamTargetValue");
    ...
    crudsResult = paramTargetValueAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static ParamTargetValueAgent createParamTargetValueAgent() {
    return ParamTargetValueAgent.getParamTargetValueAgent(
