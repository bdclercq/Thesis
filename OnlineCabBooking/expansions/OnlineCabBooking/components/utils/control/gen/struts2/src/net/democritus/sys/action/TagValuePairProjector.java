...
public class TagValuePairProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(TagValuePairProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "TagValuePair.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "TagValuePair.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "utils", "net.democritus.sys", "TagValuePair");
    ...
    crudsResult = tagValuePairAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static TagValuePairAgent createTagValuePairAgent() {
    return TagValuePairAgent.getTagValuePairAgent(
