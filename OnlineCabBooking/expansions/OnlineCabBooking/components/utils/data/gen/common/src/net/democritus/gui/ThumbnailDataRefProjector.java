...
public class ThumbnailDataRefProjector implements IDataElementProjector<ThumbnailData, DataRef>, IDataProjectorRequired<DataRef> {
  ...
  public ThumbnailDataRefProjector(ThumbnailCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  // anchor:project:start
  public DataRef project(ParameterContext<ThumbnailData> dataParameter) {

    DataRef projection = DataRef.withId(0L);
    projection.setComponentName("utils");
    projection.setPackName("net.democritus.gui");
    projection.setClassName("Thumbnail");
  ...
  // anchor:toData:start
  public void toData(ThumbnailData data, ParameterContext<DataRef> projectionParameter) {
