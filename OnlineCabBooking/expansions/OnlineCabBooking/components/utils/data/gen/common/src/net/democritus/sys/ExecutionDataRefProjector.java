...
public class ExecutionDataRefProjector implements IDataElementProjector<ExecutionData, DataRef>, IDataProjectorRequired<DataRef> {
  ...
  public ExecutionDataRefProjector(ExecutionCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  // anchor:project:start
  public DataRef project(ParameterContext<ExecutionData> dataParameter) {

    DataRef projection = DataRef.withId(0L);
    projection.setComponentName("utils");
    projection.setPackName("net.democritus.sys");
    projection.setClassName("Execution");
  ...
  // anchor:toData:start
  public void toData(ExecutionData data, ParameterContext<DataRef> projectionParameter) {
