...
public class IdCounterDataRefProjector implements IDataElementProjector<IdCounterData, DataRef>, IDataProjectorRequired<DataRef> {
  ...
  public IdCounterDataRefProjector(IdCounterCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  // anchor:project:start
  public DataRef project(ParameterContext<IdCounterData> dataParameter) {

    DataRef projection = DataRef.withId(0L);
    projection.setComponentName("utils");
    projection.setPackName("net.democritus.sys");
    projection.setClassName("IdCounter");
  ...
  // anchor:toData:start
  public void toData(IdCounterData data, ParameterContext<DataRef> projectionParameter) {
