...
@Local({ThumbnailCrudsLocal.class, ThumbnailCrudsInternal.class})
// @anchor:annotations:end
public class ThumbnailCruds /*@anchor:interfaces:start@*/implements ThumbnailCrudsLocal, ThumbnailCrudsInternal/*@anchor:interfaces:end@*/ {
  ...
  public static final String DISABLED_ERROR_MSG_KEY = "utils.thumbnail.alreadyDisabled";
  ...
  private ThumbnailFinderLocal thumbnailFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(ThumbnailCruds.class);
  ...
  private ThumbnailDataRefProjector dataRefProjector;
  private ThumbnailDetailsProjector detailsProjector;
  private ThumbnailInfoProjector infoProjector;

  private DataElementProjectorManager<ThumbnailData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    ThumbnailCrudsInternal thumbnailCrudsInternal = sessionContext.getBusinessObject(ThumbnailCrudsInternal.class);

    detailsProjector = new ThumbnailDetailsProjector(thumbnailCrudsInternal);
    infoProjector = new ThumbnailInfoProjector(thumbnailCrudsInternal);
    dataRefProjector = new ThumbnailDataRefProjector(thumbnailCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<ThumbnailData>();
  ...
  private void initFinder() {
    thumbnailFinder = new ThumbnailFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<ThumbnailDetails> detailsParameter) {
    ...
    try {
      ...
      detailsProjector.toData(thumbnailData, detailsParameter);
    ...
    try {
      entityManager.persist(thumbnailData);
      ...
      if (thumbnailData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created ThumbnailCruds with { id : " + thumbnailData.getId() + ", name: " + thumbnailData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(thumbnailData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<ThumbnailDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    ThumbnailDetails thumbnailDetails = detailsParameter.getValue();
    DataRef dataRef = thumbnailDetails.getDataRef();
    ...
    try {
      CrudsResult<ThumbnailData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      ThumbnailData thumbnailData = dataResult.getValue();
      ...
      if (thumbnailData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified ThumbnailCruds with { id : " + thumbnailData.getId() + ", name: " + thumbnailData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ThumbnailCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof ThumbnailDetails) {
      ThumbnailDetails details = (ThumbnailDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(thumbnailData);
      // @anchor:delete:end
      ...
      if (thumbnailData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted ThumbnailCruds with { id : " + thumbnailData.getId() + ", name: " + thumbnailData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<ThumbnailData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      ThumbnailData thumbnailData = entityManager.find(ThumbnailData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(thumbnailData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<ThumbnailData> getDataById(ParameterContext<Long> idParameter) {
    ThumbnailData data = entityManager.find(ThumbnailData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<ThumbnailData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<ThumbnailData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      ThumbnailFindByNameEqDetails findByNameEqDetails = new ThumbnailFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<ThumbnailFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<ThumbnailFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<ThumbnailData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<ThumbnailData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<ThumbnailData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<ThumbnailData> result;
  ...
  private CrudsResult<ThumbnailData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<ThumbnailData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends ThumbnailFindDetails> SearchResult<ThumbnailData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return thumbnailFinder.find(searchParameter);
  }

  public <S extends ThumbnailFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<ThumbnailData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<ThumbnailData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<ThumbnailData> fromItems, IDataElementProjector<ThumbnailData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<ThumbnailData> fromItems, IDataElementProjector<ThumbnailData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<ThumbnailData>> listParameter, String projection) {
    IDataElementProjector<ThumbnailData, P> elementProjector = getElementProjector(projection);
    List<ThumbnailData> list = listParameter.getValue();
  ...
  private <P> IDataElementProjector<ThumbnailData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<ThumbnailData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<ThumbnailData> dataParameter) {
  ...
  private <T> IDataElementProjector<ThumbnailData, T> getElementProjector(String projectionName) {
    IDataElementProjector<ThumbnailData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("utils", "Thumbnail"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<ThumbnailData> dataResult = getData(projectionRefParameter.construct(dataRef));
    ThumbnailData thumbnailData;
    if (dataResult.isSuccess()) {
      thumbnailData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<ThumbnailData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(thumbnailData));

    if (thumbnailData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved ThumbnailCruds:" + projectionRef.getProjection() + " { id : " + thumbnailData.getId() + ", name: " + thumbnailData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<ThumbnailDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<ThumbnailData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    ThumbnailData thumbnailData = result.getValue();
    ThumbnailDetails thumbnailDetails = detailsProjector.project(dataRefParameter.construct(thumbnailData));

    return CrudsResult.success(thumbnailDetails);
  }

  public CrudsResult<List<ThumbnailDetails>> getDetailsListFromData(ParameterContext<Collection<ThumbnailData>> dataListParameter) {
    Collection<ThumbnailData> dataList = dataListParameter.getValue();
    List<ThumbnailDetails> list = new ArrayList<>(dataList.size());

    for (ThumbnailData thumbnailData: dataList) {
      ThumbnailDetails thumbnailDetails = detailsProjector.project(dataListParameter.construct(thumbnailData));
      list.add(thumbnailDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<ThumbnailData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    ThumbnailData data = dataResult.getValue();
