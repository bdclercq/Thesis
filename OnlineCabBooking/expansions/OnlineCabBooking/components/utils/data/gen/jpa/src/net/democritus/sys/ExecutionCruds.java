...
@Local({ExecutionCrudsLocal.class, ExecutionCrudsInternal.class})
// @anchor:annotations:end
public class ExecutionCruds /*@anchor:interfaces:start@*/implements ExecutionCrudsLocal, ExecutionCrudsInternal/*@anchor:interfaces:end@*/ {
  ...
  public static final String DISABLED_ERROR_MSG_KEY = "utils.execution.alreadyDisabled";
  ...
  private ExecutionFinderLocal executionFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(ExecutionCruds.class);
  ...
  private ExecutionDataRefProjector dataRefProjector;
  private ExecutionDetailsProjector detailsProjector;
  private ExecutionInfoProjector infoProjector;

  private DataElementProjectorManager<ExecutionData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    ExecutionCrudsInternal executionCrudsInternal = sessionContext.getBusinessObject(ExecutionCrudsInternal.class);

    detailsProjector = new ExecutionDetailsProjector(executionCrudsInternal);
    infoProjector = new ExecutionInfoProjector(executionCrudsInternal);
    dataRefProjector = new ExecutionDataRefProjector(executionCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<ExecutionData>();
  ...
  private void initFinder() {
    executionFinder = new ExecutionFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<ExecutionDetails> detailsParameter) {
    ...
    try {
      ...
      detailsProjector.toData(executionData, detailsParameter);
    ...
    try {
      entityManager.persist(executionData);
      ...
      if (executionData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created ExecutionCruds with { id : " + executionData.getId() + ", name: " + executionData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(executionData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<ExecutionDetails> detailsParameter) {
    ...
    DataRef dataRef = executionDetails.getDataRef();
    ...
    try {
      CrudsResult<ExecutionData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      ExecutionData executionData = dataResult.getValue();
      ...
      if (executionData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified ExecutionCruds with { id : " + executionData.getId() + ", name: " + executionData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ExecutionCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof ExecutionDetails) {
      ExecutionDetails details = (ExecutionDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(executionData);
      // @anchor:delete:end
      ...
      if (executionData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted ExecutionCruds with { id : " + executionData.getId() + ", name: " + executionData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<ExecutionData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      ExecutionData executionData = entityManager.find(ExecutionData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(executionData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<ExecutionData> getDataById(ParameterContext<Long> idParameter) {
    ExecutionData data = entityManager.find(ExecutionData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<ExecutionData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<ExecutionData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      ExecutionFindByNameEqDetails findByNameEqDetails = new ExecutionFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<ExecutionFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<ExecutionFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<ExecutionData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<ExecutionData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<ExecutionData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<ExecutionData> result;
  ...
  private CrudsResult<ExecutionData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<ExecutionData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends ExecutionFindDetails> SearchResult<ExecutionData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return executionFinder.find(searchParameter);
  }

  public <S extends ExecutionFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<ExecutionData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<ExecutionData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<ExecutionData> fromItems, IDataElementProjector<ExecutionData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<ExecutionData> fromItems, IDataElementProjector<ExecutionData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<ExecutionData>> listParameter, String projection) {
    IDataElementProjector<ExecutionData, P> elementProjector = getElementProjector(projection);
    List<ExecutionData> list = listParameter.getValue();
  ...
  private <P> IDataElementProjector<ExecutionData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<ExecutionData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<ExecutionData> dataParameter) {
  ...
  private <T> IDataElementProjector<ExecutionData, T> getElementProjector(String projectionName) {
    IDataElementProjector<ExecutionData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("utils", "Execution"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<ExecutionData> dataResult = getData(projectionRefParameter.construct(dataRef));
    ExecutionData executionData;
    if (dataResult.isSuccess()) {
      executionData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<ExecutionData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(executionData));

    if (executionData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved ExecutionCruds:" + projectionRef.getProjection() + " { id : " + executionData.getId() + ", name: " + executionData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<ExecutionDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<ExecutionData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    ExecutionData executionData = result.getValue();
    ExecutionDetails executionDetails = detailsProjector.project(dataRefParameter.construct(executionData));

    return CrudsResult.success(executionDetails);
  }

  public CrudsResult<List<ExecutionDetails>> getDetailsListFromData(ParameterContext<Collection<ExecutionData>> dataListParameter) {
    Collection<ExecutionData> dataList = dataListParameter.getValue();
    List<ExecutionDetails> list = new ArrayList<>(dataList.size());

    for (ExecutionData executionData: dataList) {
      ExecutionDetails executionDetails = detailsProjector.project(dataListParameter.construct(executionData));
      list.add(executionDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<ExecutionData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    ExecutionData data = dataResult.getValue();
