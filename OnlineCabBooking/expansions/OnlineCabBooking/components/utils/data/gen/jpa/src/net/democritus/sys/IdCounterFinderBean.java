...
public class IdCounterFinderBean implements IdCounterFinderLocal {

  private EntityManager entityManager;

  @SuppressWarnings("rawtypes")
  private static final Map<Class<? extends IdCounterFindDetails>,
      BiFunction<IdCounterFinderBean, ParameterContext, SearchResult<IdCounterData>>> finderMapping = generateFinderMapping();

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(IdCounterFinderBean.class);
  // @anchor:variables:end

  // anchor:custom-variables:start
  // anchor:custom-variables:end

  public IdCounterFinderBean(EntityManager entityManager) {
    this.entityManager = entityManager;
  }

  @SuppressWarnings("rawtypes")
  private static Map<Class<? extends IdCounterFindDetails>, BiFunction<IdCounterFinderBean, ParameterContext, SearchResult<IdCounterData>>> generateFinderMapping() {
    Map<Class<? extends IdCounterFindDetails>, BiFunction<IdCounterFinderBean, ParameterContext, SearchResult<IdCounterData>>> finderMapping = new HashMap<>();
    // anchor:register-finders:start
    finderMapping.put(IdCounterFindAllIdCountersDetails.class, IdCounterFinderBean::findAllIdCounters);
    finderMapping.put(IdCounterFindByNameEqDetails.class, IdCounterFinderBean::findByNameEq);
  ...
  public SearchResult<IdCounterData> findAllIdCounters(ParameterContext<SearchDetails<IdCounterFindAllIdCountersDetails>> searchParameter) {
    SearchDetails<IdCounterFindAllIdCountersDetails> searchDetails = searchParameter.getValue();
  ...
  @SuppressWarnings({"unchecked", "rawtypes"})
  public SearchResult<IdCounterData> findByNameEq(ParameterContext<SearchDetails<IdCounterFindByNameEqDetails>> searchParameter) {
    SearchDetails<IdCounterFindByNameEqDetails> searchDetails = searchParameter.getValue();

    IdCounterFindByNameEqDetails details = searchDetails.getDetails();
  ...
  private SearchResult<IdCounterData> fetchData(ParameterContext<SearchDetails<?>> searchParameter, QueryBuilder queryBuilder) {
  ...
  private SearchResult<IdCounterData> fetchData(SearchDetails<?> searchDetails, Query countQuery, Query dataQuery) {
    Long total = 0L;
    if (!searchDetails.getSkipCount()) {
      ...
      if (total == 0 || searchDetails.getPaging().getRowsPerPage() == 0) {
        return SearchResult.success(Collections.<IdCounterData>emptyList(), total.intValue());
      }
    ...
    List<IdCounterData> resultDatas = (List<IdCounterData>) dataQuery.getResultList();
  ...
  @Override
  public <S extends IdCounterFindDetails> SearchResult<IdCounterData> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<IdCounterData> searchResult;
    ...
    Class<? extends IdCounterFindDetails> findDetailsClass = searchParameter.getValue().getDetails().getClass();
    try {
      if (finderMapping.containsKey(findDetailsClass)) {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
              findDetailsClass.getName() + " does not match a finder in IdCounterFinder.find()"
          );
        }

        searchResult = SearchResult.error(Diagnostic.error("utils", "idCounter", "invalidFinderException"));
      }
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Exception when processing " + findDetailsClass.getName() + " in IdCounterFinder.find() ", e
        );
      }

      searchResult = SearchResult.error(Diagnostic.error("utils", "idCounter", "finderException"));
    }
  ...
  private JPAQueryBuilder createQueryBuilder() {
    return new JPAQueryBuilder(IdCounterData.ENTITY_NAME, "name");
  }
