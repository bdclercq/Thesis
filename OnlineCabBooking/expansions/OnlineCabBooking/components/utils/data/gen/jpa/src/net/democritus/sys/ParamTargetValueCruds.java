...
@Local({ParamTargetValueCrudsLocal.class, ParamTargetValueCrudsInternal.class})
// @anchor:annotations:end
public class ParamTargetValueCruds /*@anchor:interfaces:start@*/implements ParamTargetValueCrudsLocal, ParamTargetValueCrudsInternal/*@anchor:interfaces:end@*/ {
  ...
  public static final String DISABLED_ERROR_MSG_KEY = "utils.paramTargetValue.alreadyDisabled";
  ...
  private ParamTargetValueFinderLocal paramTargetValueFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(ParamTargetValueCruds.class);
  ...
  private ParamTargetValueDataRefProjector dataRefProjector;
  private ParamTargetValueDetailsProjector detailsProjector;
  private ParamTargetValueInfoProjector infoProjector;

  private DataElementProjectorManager<ParamTargetValueData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    ParamTargetValueCrudsInternal paramTargetValueCrudsInternal = sessionContext.getBusinessObject(ParamTargetValueCrudsInternal.class);

    detailsProjector = new ParamTargetValueDetailsProjector(paramTargetValueCrudsInternal);
    infoProjector = new ParamTargetValueInfoProjector(paramTargetValueCrudsInternal);
    dataRefProjector = new ParamTargetValueDataRefProjector(paramTargetValueCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<ParamTargetValueData>();
  ...
  private void initFinder() {
    paramTargetValueFinder = new ParamTargetValueFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<ParamTargetValueDetails> detailsParameter) {
    ...
    ParamTargetValueData paramTargetValueData = new ParamTargetValueData();
    try {
      ...
      detailsProjector.toData(paramTargetValueData, detailsParameter);
    ...
    try {
      entityManager.persist(paramTargetValueData);

      // @anchor:implicitNameFieldOnly-afterPersist:start
      // @anchor:implicitNameFieldOnly-afterPersist:end

      if (paramTargetValueData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created ParamTargetValueCruds with { id : " + paramTargetValueData.getId() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(paramTargetValueData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<ParamTargetValueDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    ParamTargetValueDetails paramTargetValueDetails = detailsParameter.getValue();
    DataRef dataRef = paramTargetValueDetails.getDataRef();
    ...
    try {
      CrudsResult<ParamTargetValueData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      ParamTargetValueData paramTargetValueData = dataResult.getValue();
      ...
      if (paramTargetValueData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified ParamTargetValueCruds with { id : " + paramTargetValueData.getId() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ParamTargetValueCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof ParamTargetValueDetails) {
      ParamTargetValueDetails details = (ParamTargetValueDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(paramTargetValueData);
      // @anchor:delete:end
      ...
      if (paramTargetValueData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted ParamTargetValueCruds with { id : " + paramTargetValueData.getId() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<ParamTargetValueData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      ParamTargetValueData paramTargetValueData = entityManager.find(ParamTargetValueData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(paramTargetValueData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<ParamTargetValueData> getDataById(ParameterContext<Long> idParameter) {
    ParamTargetValueData data = entityManager.find(ParamTargetValueData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<ParamTargetValueData> getDataByName(ParameterContext<String> nameParameter) {
    return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  }

  public CrudsResult<ParamTargetValueData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<ParamTargetValueData> result;
  ...
  private CrudsResult<ParamTargetValueData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<ParamTargetValueData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends ParamTargetValueFindDetails> SearchResult<ParamTargetValueData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return paramTargetValueFinder.find(searchParameter);
  }

  public <S extends ParamTargetValueFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<ParamTargetValueData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<ParamTargetValueData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<ParamTargetValueData> fromItems, IDataElementProjector<ParamTargetValueData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<ParamTargetValueData> fromItems, IDataElementProjector<ParamTargetValueData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<ParamTargetValueData>> listParameter, String projection) {
    IDataElementProjector<ParamTargetValueData, P> elementProjector = getElementProjector(projection);
    List<ParamTargetValueData> list = listParameter.getValue();
  ...
  private <P> IDataElementProjector<ParamTargetValueData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<ParamTargetValueData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<ParamTargetValueData> dataParameter) {
  ...
  private <T> IDataElementProjector<ParamTargetValueData, T> getElementProjector(String projectionName) {
    IDataElementProjector<ParamTargetValueData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("utils", "ParamTargetValue"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<ParamTargetValueData> dataResult = getData(projectionRefParameter.construct(dataRef));
    ParamTargetValueData paramTargetValueData;
    if (dataResult.isSuccess()) {
      paramTargetValueData = dataResult.getValue();
    } else {
      paramTargetValueData = new ParamTargetValueData();
    }

    // anchor:custom-preProject:start
    // anchor:custom-preProject:end

    IDataElementProjector<ParamTargetValueData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(paramTargetValueData));

    if (paramTargetValueData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved ParamTargetValueCruds:" + projectionRef.getProjection() + " { id : " + paramTargetValueData.getId() + " }"
        );
      }
    }
  ...
  public CrudsResult<ParamTargetValueDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<ParamTargetValueData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    ParamTargetValueData paramTargetValueData = result.getValue();
    ParamTargetValueDetails paramTargetValueDetails = detailsProjector.project(dataRefParameter.construct(paramTargetValueData));

    return CrudsResult.success(paramTargetValueDetails);
  }

  public CrudsResult<List<ParamTargetValueDetails>> getDetailsListFromData(ParameterContext<Collection<ParamTargetValueData>> dataListParameter) {
    Collection<ParamTargetValueData> dataList = dataListParameter.getValue();
    List<ParamTargetValueDetails> list = new ArrayList<>(dataList.size());

    for (ParamTargetValueData paramTargetValueData: dataList) {
      ParamTargetValueDetails paramTargetValueDetails = detailsProjector.project(dataListParameter.construct(paramTargetValueData));
      list.add(paramTargetValueDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<ParamTargetValueData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    ParamTargetValueData data = dataResult.getValue();
