...
@Local({TagValuePairCrudsLocal.class, TagValuePairCrudsInternal.class})
// @anchor:annotations:end
public class TagValuePairCruds /*@anchor:interfaces:start@*/implements TagValuePairCrudsLocal, TagValuePairCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("utils", "TagValuePair");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "utils.tagValuePair.alreadyDisabled";
  ...
  private TagValuePairFinderLocal tagValuePairFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(TagValuePairCruds.class);
  ...
  private TagValuePairDataRefProjector dataRefProjector;
  private TagValuePairDetailsProjector detailsProjector;
  private TagValuePairInfoProjector infoProjector;

  private DataElementProjectorManager<TagValuePairData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    TagValuePairCrudsInternal tagValuePairCrudsInternal = sessionContext.getBusinessObject(TagValuePairCrudsInternal.class);

    detailsProjector = new TagValuePairDetailsProjector(tagValuePairCrudsInternal);
    infoProjector = new TagValuePairInfoProjector(tagValuePairCrudsInternal);
    dataRefProjector = new TagValuePairDataRefProjector(tagValuePairCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<TagValuePairData>();
  ...
  private void initFinder() {
    tagValuePairFinder = new TagValuePairFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<TagValuePairDetails> detailsParameter) {
    ...
    TagValuePairDetails details = detailsParameter.getValue();
    ...
    TagValuePairData tagValuePairData = new TagValuePairData();
    try {
      ...
      detailsProjector.toData(tagValuePairData, detailsParameter);
    ...
    try {
      entityManager.persist(tagValuePairData);

      // @anchor:implicitNameFieldOnly-afterPersist:start
      // @anchor:implicitNameFieldOnly-afterPersist:end

      if (tagValuePairData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created TagValuePairCruds with { id : " + tagValuePairData.getId() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(tagValuePairData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<TagValuePairDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    TagValuePairDetails tagValuePairDetails = detailsParameter.getValue();
    DataRef dataRef = tagValuePairDetails.getDataRef();
    ...
    try {
      CrudsResult<TagValuePairData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      TagValuePairData tagValuePairData = dataResult.getValue();
      ...
      if (tagValuePairData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified TagValuePairCruds with { id : " + tagValuePairData.getId() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "TagValuePairCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof TagValuePairDetails) {
      TagValuePairDetails details = (TagValuePairDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(tagValuePairData);
      // @anchor:delete:end
      ...
      if (tagValuePairData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted TagValuePairCruds with { id : " + tagValuePairData.getId() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<TagValuePairData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      TagValuePairData tagValuePairData = entityManager.find(TagValuePairData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(tagValuePairData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<TagValuePairData> getDataById(ParameterContext<Long> idParameter) {
    TagValuePairData data = entityManager.find(TagValuePairData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<TagValuePairData> getDataByName(ParameterContext<String> nameParameter) {
    return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  }

  public CrudsResult<TagValuePairData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<TagValuePairData> result;
  ...
  private CrudsResult<TagValuePairData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<TagValuePairData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends TagValuePairFindDetails> SearchResult<TagValuePairData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return tagValuePairFinder.find(searchParameter);
  }

  public <S extends TagValuePairFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<TagValuePairData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<TagValuePairData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<TagValuePairData> fromItems, IDataElementProjector<TagValuePairData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<TagValuePairData> fromItems, IDataElementProjector<TagValuePairData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<TagValuePairData>> listParameter, String projection) {
    IDataElementProjector<TagValuePairData, P> elementProjector = getElementProjector(projection);
    List<TagValuePairData> list = listParameter.getValue();
  ...
  private <P> IDataElementProjector<TagValuePairData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<TagValuePairData> dataParameter) {
    ...
    displayName = StringUtil.join(":",
      nameFromValue(data.getTag()),
      nameFromValue(data.getValue())
    );

    return displayName;
  }

  private Option<String> getCustomDisplayName(ParameterContext<TagValuePairData> dataParameter) {
  ...
  private <T> IDataElementProjector<TagValuePairData, T> getElementProjector(String projectionName) {
    IDataElementProjector<TagValuePairData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("utils", "TagValuePair"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<TagValuePairData> dataResult = getData(projectionRefParameter.construct(dataRef));
    TagValuePairData tagValuePairData;
    if (dataResult.isSuccess()) {
      tagValuePairData = dataResult.getValue();
    } else {
      tagValuePairData = new TagValuePairData();
    }

    // anchor:custom-preProject:start
    // anchor:custom-preProject:end

    IDataElementProjector<TagValuePairData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(tagValuePairData));

    if (tagValuePairData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved TagValuePairCruds:" + projectionRef.getProjection() + " { id : " + tagValuePairData.getId() + " }"
        );
      }
    }
  ...
  public CrudsResult<TagValuePairDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<TagValuePairData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    TagValuePairData tagValuePairData = result.getValue();
    TagValuePairDetails tagValuePairDetails = detailsProjector.project(dataRefParameter.construct(tagValuePairData));

    return CrudsResult.success(tagValuePairDetails);
  }

  public CrudsResult<List<TagValuePairDetails>> getDetailsListFromData(ParameterContext<Collection<TagValuePairData>> dataListParameter) {
    Collection<TagValuePairData> dataList = dataListParameter.getValue();
    List<TagValuePairDetails> list = new ArrayList<>(dataList.size());

    for (TagValuePairData tagValuePairData: dataList) {
      TagValuePairDetails tagValuePairDetails = detailsProjector.project(dataListParameter.construct(tagValuePairData));
      list.add(tagValuePairDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<TagValuePairData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    TagValuePairData data = dataResult.getValue();
