...
public class ParamTargetValueCsvImporterImpl implements IImporter {
  private static final Logger logger = LoggerFactory.getLogger(ParamTargetValueCsvImporterImpl.class);

  private ParamTargetValueMapper paramTargetValueMapper = new ParamTargetValueMapper();
  ...
  public ImportResult importFile(ParameterContext<ImportFile> parameter) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "paramTargetValue CSV import: start"
      );
    }
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "paramTargetValue CSV import: parsed"
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
          "paramTargetValue CSV import: imported"
  ...
  private void importInstance(ParameterContext<ImportLine> importLineParam, ImportResult importResult) {
    ...
    Result<ParamTargetValueDetails> mappingResult = paramTargetValueMapper.convertToDetails(importLineParam.construct(line.getValues()));
    if (mappingResult.isError()) {
      importResult.addError(new ImportError(line.getIndex(), mappingResult.getMessage()));
      return;
    }
    ParamTargetValueDetails paramTargetValueDetails = mappingResult.getValue();
    ...
    CrudsResult<DataRef> crudsResult = persistParamTargetValue(importLineParam.construct(paramTargetValueDetails));
  ...
  private CrudsResult<DataRef> persistParamTargetValue(ParameterContext<ParamTargetValueDetails> detailsParameter) {
    ParamTargetValueLocalAgent paramTargetValueAgent = ParamTargetValueLocalAgent.getParamTargetValueAgent(detailsParameter.getContext());
    ...
    return paramTargetValueAgent.createOrModify(paramTargetValueDetails);
  }
