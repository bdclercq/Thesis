...
import net.democritus.sys.ExecutionCsvImporterImpl;
...
@Remote(ExecutionRemote.class)
@Local(ExecutionLocal.class)
// @anchor:annotations:end
@DeclareRoles({
// anchor:custom-declare-roles:start
// anchor:custom-declare-roles:end
})
public class ExecutionBean /*@anchor:interfaces:start@*/implements ExecutionRemote, ExecutionLocal/*@anchor:interfaces:end@*/{

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("utils", "Execution");
  ...
  private static final Logger logger = LoggerFactory.getLogger(ExecutionBean.class);
  @Deprecated
  @PersistenceContext(unitName="OnlineCabBooking_utils")
  private EntityManager entityManager;

  @EJB
  ExecutionCrudsLocal executionCrudsLocal;
  ...
  public CrudsResult<DataRef> create(ParameterContext<ExecutionDetails> detailsParameter) {
    ...
    ExecutionDetails details = detailsParameter.getValue();
    ...
    CrudsResult<DataRef> result = executionCrudsLocal.create(context.withParameter(details));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<ExecutionDetails> detailsParameter) {
    ...
    CrudsResult<DataRef> result = executionCrudsLocal.modify(context.withParameter(details));
    if (result.isError()) {
      return result;
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    CrudsResult<DataRef> result = executionCrudsLocal.createOrModify(projectionParameter);
    if (result.isError()) {
      return result;
    }
  ...
  public CrudsResult<Void> delete(ParameterContext<Long> idParameter) {
    ...
    try {
      ...
      CrudsResult<Void> result = executionCrudsLocal.delete(context.withParameter(dataRef));
  ...
  public CrudsResult<DataRef> getId(ParameterContext<String> nameParameter) {
    return executionCrudsLocal.getDataRefFromName(nameParameter);
  }

  public CrudsResult<String> getName(ParameterContext<Long> idParameter) {
    // @anchor:before-getName:start
    // @anchor:before-getName:end
    CrudsResult<DataRef> dataRefResult = executionCrudsLocal.getDataRefFromId(idParameter);
  ...
  public DataRef getDataRef(Long id) {
    return executionCrudsLocal.getDataRefFromId(ParameterContext.create(null, id)).getValue();
  }

  @Deprecated
  public ExecutionInfo getInfo(Long id) {
    return getInfo(ParameterContext.create(null, id)).getValue();
  }

  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public ExecutionDetails getDetails(Long id) {
    return getDetailsFromDataRef(ParameterContext.create(null, idToDataRef(id))).getValue();
  }

  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public ExecutionDetails getDetails(DataRef dataRef) {
  ...
  public CrudsResult<DataRef> getDataRef(ParameterContext<Long> idParameter) {
    return executionCrudsLocal.getDataRefFromId(idParameter);
  }

  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    return executionCrudsLocal.resolveDataRef(dataRefParameter);
  }

  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<ExecutionInfo> getInfo(ParameterContext<Long> idParameter) {
    DataRef dataRef = idToDataRef(idParameter.getValue());
  ...
  public CrudsResult<ExecutionDetails> getDetails(ParameterContext<Long> idParameter) {
    ...
    CrudsResult<ExecutionDetails> result = getDetailsFromDataRef(idParameter.construct(dataRef));
  ...
  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  public CrudsResult<ExecutionDetails> getDetailsFromDataRef(ParameterContext<DataRef> dataRefParameter) {
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    result = executionCrudsLocal.getProjection(projectionRefParameter);
  ...
  public <S extends ExecutionFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    ...
    result = executionCrudsLocal.find(searchParameter);
  ...
  public ImportResult importFile(ParameterContext<ImportFile> parameter) {
    ...
    // @anchor:import-implementations:start
      ...
      importResult = new ExecutionCsvImporterImpl().importFile(parameter);
