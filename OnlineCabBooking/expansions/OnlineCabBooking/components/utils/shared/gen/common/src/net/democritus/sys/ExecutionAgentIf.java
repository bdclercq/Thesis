...
/**
 * interface for the entity bean ExecutionBean,
 * representing a known Execution
 */
public interface ExecutionAgentIf {
  ...
  CrudsResult<DataRef> create(ExecutionDetails executionDetails);
  CrudsResult<DataRef> modify(ExecutionDetails executionDetails);
  ...
  CrudsResult<ExecutionDetails> getDetails(DataRef dataRef);
  ...
  <S extends ExecutionFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<ExecutionDetails> getDetails(Long executionId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<ExecutionInfo> findAllInfos();
