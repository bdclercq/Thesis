...
public class ExecutionMapper implements IDataElementMapper<ExecutionDetails> {
  ...
  public Result<Map<String, String>> convertToMap(ParameterContext<ExecutionDetails> parameter) {
    ExecutionDetails executionDetails = parameter.getValue();
    try {
      Map<String, String> map = new HashMap<String, String>();

      // anchor:convert-fields-to-map:start
      if (executionDetails.getName() != null) {
        map.put("name", executionDetails.getName());
      }

      if (executionDetails.getComponent() != null) {
        map.put("component", executionDetails.getComponent());
      }

      if (executionDetails.getElement() != null) {
        map.put("element", executionDetails.getElement());
      }

      if (executionDetails.getPackageName() != null) {
        map.put("packageName", executionDetails.getPackageName());
      }
      ...
      if (logger.isErrorEnabled()) {
          logger.error(
          "Unexpected error while mapping instance " + executionDetails.getId() + " to map", e
          );
      }
  ...
  public Result<ExecutionDetails> convertToDetails(ParameterContext<Map<String, String>> parameter) {
    try {
      ...
      if (nameValueFromMap != null && !nameValueFromMap.isEmpty()) {
        executionDetails.setName(nameValueFromMap);
      }

      String componentValueFromMap = map.get("component");
      if (componentValueFromMap != null && !componentValueFromMap.isEmpty()) {
        executionDetails.setComponent(componentValueFromMap);
      }

      String elementValueFromMap = map.get("element");
      if (elementValueFromMap != null && !elementValueFromMap.isEmpty()) {
        executionDetails.setElement(elementValueFromMap);
      }

      String packageNameValueFromMap = map.get("packageName");
      if (packageNameValueFromMap != null && !packageNameValueFromMap.isEmpty()) {
        executionDetails.setPackageName(packageNameValueFromMap);
      }
