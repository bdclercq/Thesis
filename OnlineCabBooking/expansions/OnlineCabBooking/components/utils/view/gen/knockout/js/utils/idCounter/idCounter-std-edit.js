...
define(function (require) {
  ...
  var IdCounterProjector = require('utils/idCounter/idCounter-projector');
  var IdCounterForm = require('utils/idCounter/idCounter-instance-form');
  ...
  function defineEditDialog(options) {
    ...
    when(editRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to edit a(n) IdCounter element!")
    });

    function render() {
      var editIntent = IdCounterEvents.defineEditIntentListener();
      var showDialog = Trigger.defineTrigger();

      var constants = ko.observable();
      var selection = ko.observable();
      var formView = "utils/idCounter-form";
      var title = "Edit IdCounter";
      var layoutConfig = initLayoutConfig(constants)
      var formOptions = {
        resetOn: editIntent,
        action: IdCounterActions.editAction()
      };

      var detailsProjection = IdCounterProjector.defineProjector(selection, {
        projection: "details"
      });
      ...
      var form = IdCounterForm.defineForm({
  ...
  function initLayoutConfig(constants) {
    return {
      ...
      counter: {
          disabled: ko.pureComputed(function () {
            return utils.isDefined(constants().counter)
          })
      }
