define(function (require) {
   ...
   * }} params
   * @template I
   * @return {ParamTargetValueAction<I>}
   */
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "utils.paramTargetValue.create"}),
      updateEvent: ParamTargetValueEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(ParamTargetValueElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "utils.paramTargetValue.edit"}),
      updateEvent: ParamTargetValueEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(ParamTargetValueElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "utils.paramTargetValue.delete"}),
      updateEvent: ParamTargetValueEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(ParamTargetValueElement, "delete-json"),
          data: JSON.stringify({
            "paramTargetValueOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
