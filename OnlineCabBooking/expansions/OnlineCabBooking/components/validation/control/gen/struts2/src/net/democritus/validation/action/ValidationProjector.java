...
public class ValidationProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ValidationProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Validation.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Validation.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "validation", "net.democritus.validation", "Validation");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    ValidationAgent validationAgent = createValidationAgent();
    crudsResult = validationAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static ValidationAgent createValidationAgent() {
    return ValidationAgent.getValidationAgent(
