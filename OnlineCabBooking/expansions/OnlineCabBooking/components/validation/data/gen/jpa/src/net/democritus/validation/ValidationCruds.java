...
@Local({ValidationCrudsLocal.class, ValidationCrudsInternal.class})
// @anchor:annotations:end
public class ValidationCruds /*@anchor:interfaces:start@*/implements ValidationCrudsLocal, ValidationCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("validation", "Validation");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "validation.validation.alreadyDisabled";

  @PersistenceContext(unitName="OnlineCabBooking_validation")
  private EntityManager entityManager;

  @Resource
  private SessionContext sessionContext;

  private ValidationFinderLocal validationFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(ValidationCruds.class);
  ...
  private ValidationDataRefProjector dataRefProjector;
  private ValidationDetailsProjector detailsProjector;
  private ValidationInfoProjector infoProjector;

  private DataElementProjectorManager<ValidationData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    ValidationCrudsInternal validationCrudsInternal = sessionContext.getBusinessObject(ValidationCrudsInternal.class);

    detailsProjector = new ValidationDetailsProjector(validationCrudsInternal);
    infoProjector = new ValidationInfoProjector(validationCrudsInternal);
    dataRefProjector = new ValidationDataRefProjector(validationCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<ValidationData>();
  ...
  private void initFinder() {
    validationFinder = new ValidationFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<ValidationDetails> detailsParameter) {
    ...
    ValidationDetails details = detailsParameter.getValue();
    ...
    try {
      ...
      detailsProjector.toData(validationData, detailsParameter);
    ...
    try {
      entityManager.persist(validationData);
      ...
      if (validationData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created ValidationCruds with { id : " + validationData.getId() + ", name: " + validationData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(validationData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<ValidationDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    ValidationDetails validationDetails = detailsParameter.getValue();
    DataRef dataRef = validationDetails.getDataRef();
    ...
    try {
      CrudsResult<ValidationData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      ValidationData validationData = dataResult.getValue();
      ...
      if (validationData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified ValidationCruds with { id : " + validationData.getId() + ", name: " + validationData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "ValidationCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof ValidationDetails) {
      ValidationDetails details = (ValidationDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(validationData);
      // @anchor:delete:end
      ...
      if (validationData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted ValidationCruds with { id : " + validationData.getId() + ", name: " + validationData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<ValidationData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      ValidationData validationData = entityManager.find(ValidationData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(validationData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<ValidationData> getDataById(ParameterContext<Long> idParameter) {
    ValidationData data = entityManager.find(ValidationData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<ValidationData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<ValidationData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      ValidationFindByNameEqDetails findByNameEqDetails = new ValidationFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<ValidationFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<ValidationFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<ValidationData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<ValidationData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<ValidationData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<ValidationData> result;
  ...
  private CrudsResult<ValidationData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<ValidationData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends ValidationFindDetails> SearchResult<ValidationData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return validationFinder.find(searchParameter);
  }

  public <S extends ValidationFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<ValidationData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<ValidationData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<ValidationData> fromItems, IDataElementProjector<ValidationData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<ValidationData> fromItems, IDataElementProjector<ValidationData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<ValidationData>> listParameter, String projection) {
    IDataElementProjector<ValidationData, P> elementProjector = getElementProjector(projection);
    List<ValidationData> list = listParameter.getValue();
  ...
  private <P> IDataElementProjector<ValidationData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<ValidationData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<ValidationData> dataParameter) {
  ...
  private <T> IDataElementProjector<ValidationData, T> getElementProjector(String projectionName) {
    IDataElementProjector<ValidationData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("validation", "Validation"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<ValidationData> dataResult = getData(projectionRefParameter.construct(dataRef));
    ValidationData validationData;
    if (dataResult.isSuccess()) {
      validationData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<ValidationData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(validationData));

    if (validationData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved ValidationCruds:" + projectionRef.getProjection() + " { id : " + validationData.getId() + ", name: " + validationData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<ValidationDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<ValidationData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    ValidationData validationData = result.getValue();
    ValidationDetails validationDetails = detailsProjector.project(dataRefParameter.construct(validationData));

    return CrudsResult.success(validationDetails);
  }

  public CrudsResult<List<ValidationDetails>> getDetailsListFromData(ParameterContext<Collection<ValidationData>> dataListParameter) {
    Collection<ValidationData> dataList = dataListParameter.getValue();
    List<ValidationDetails> list = new ArrayList<>(dataList.size());

    for (ValidationData validationData: dataList) {
      ValidationDetails validationDetails = detailsProjector.project(dataListParameter.construct(validationData));
      list.add(validationDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<ValidationData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    ValidationData data = dataResult.getValue();
