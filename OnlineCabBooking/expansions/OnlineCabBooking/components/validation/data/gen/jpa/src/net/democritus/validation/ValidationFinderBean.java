...
public class ValidationFinderBean implements ValidationFinderLocal {

  private EntityManager entityManager;

  @SuppressWarnings("rawtypes")
  private static final Map<Class<? extends ValidationFindDetails>,
      BiFunction<ValidationFinderBean, ParameterContext, SearchResult<ValidationData>>> finderMapping = generateFinderMapping();

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(ValidationFinderBean.class);
  // @anchor:variables:end

  // anchor:custom-variables:start
  // anchor:custom-variables:end

  public ValidationFinderBean(EntityManager entityManager) {
    this.entityManager = entityManager;
  }

  @SuppressWarnings("rawtypes")
  private static Map<Class<? extends ValidationFindDetails>, BiFunction<ValidationFinderBean, ParameterContext, SearchResult<ValidationData>>> generateFinderMapping() {
    Map<Class<? extends ValidationFindDetails>, BiFunction<ValidationFinderBean, ParameterContext, SearchResult<ValidationData>>> finderMapping = new HashMap<>();
    // anchor:register-finders:start
    finderMapping.put(ValidationFindAllValidationsDetails.class, ValidationFinderBean::findAllValidations);
    finderMapping.put(ValidationFindByNameEqDetails.class, ValidationFinderBean::findByNameEq);
  ...
  public SearchResult<ValidationData> findAllValidations(ParameterContext<SearchDetails<ValidationFindAllValidationsDetails>> searchParameter) {
    SearchDetails<ValidationFindAllValidationsDetails> searchDetails = searchParameter.getValue();
  ...
  @SuppressWarnings({"unchecked", "rawtypes"})
  public SearchResult<ValidationData> findByNameEq(ParameterContext<SearchDetails<ValidationFindByNameEqDetails>> searchParameter) {
    SearchDetails<ValidationFindByNameEqDetails> searchDetails = searchParameter.getValue();

    ValidationFindByNameEqDetails details = searchDetails.getDetails();
  ...
  private SearchResult<ValidationData> fetchData(ParameterContext<SearchDetails<?>> searchParameter, QueryBuilder queryBuilder) {
  ...
  private SearchResult<ValidationData> fetchData(SearchDetails<?> searchDetails, Query countQuery, Query dataQuery) {
    Long total = 0L;
    if (!searchDetails.getSkipCount()) {
      ...
      if (total == 0 || searchDetails.getPaging().getRowsPerPage() == 0) {
        return SearchResult.success(Collections.<ValidationData>emptyList(), total.intValue());
      }
    ...
    List<ValidationData> resultDatas = (List<ValidationData>) dataQuery.getResultList();
  ...
  @Override
  public <S extends ValidationFindDetails> SearchResult<ValidationData> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<ValidationData> searchResult;
    ...
    Class<? extends ValidationFindDetails> findDetailsClass = searchParameter.getValue().getDetails().getClass();
    try {
      if (finderMapping.containsKey(findDetailsClass)) {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
              findDetailsClass.getName() + " does not match a finder in ValidationFinder.find()"
          );
        }

        searchResult = SearchResult.error(Diagnostic.error("validation", "validation", "invalidFinderException"));
      }
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "Exception when processing " + findDetailsClass.getName() + " in ValidationFinder.find() ", e
        );
      }

      searchResult = SearchResult.error(Diagnostic.error("validation", "validation", "finderException"));
    }
  ...
  private JPAQueryBuilder createQueryBuilder() {
    return new JPAQueryBuilder(ValidationData.ENTITY_NAME, "name");
  }
