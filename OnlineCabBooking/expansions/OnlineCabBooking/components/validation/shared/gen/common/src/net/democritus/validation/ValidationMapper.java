...
public class ValidationMapper implements IDataElementMapper<ValidationDetails> {
  ...
  public Result<Map<String, String>> convertToMap(ParameterContext<ValidationDetails> parameter) {
    ValidationDetails validationDetails = parameter.getValue();
    try {
      Map<String, String> map = new HashMap<String, String>();

      // anchor:convert-fields-to-map:start
      if (validationDetails.getName() != null) {
        map.put("name", validationDetails.getName());
      }
      ...
      if (logger.isErrorEnabled()) {
          logger.error(
          "Unexpected error while mapping instance " + validationDetails.getId() + " to map", e
          );
      }
  ...
  public Result<ValidationDetails> convertToDetails(ParameterContext<Map<String, String>> parameter) {
    try {
      ...
      if (nameValueFromMap != null && !nameValueFromMap.isEmpty()) {
        validationDetails.setName(nameValueFromMap);
      }
      ...
      return Result.success(validationDetails);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
          logger.error(
