...
public class EngineNodeDetailer extends ActionSupport implements Preparable {
  ...
  public CrudsResult<EngineNodeDetails> getCrudsResult() {
    return crudsResult;
  }

  public JsonResult<EngineNodeDetails> getJsonResult() {
  ...
  public String execute() throws Exception {
    ...
    if (engineNodeOid.length() > 0) {
      if (shouldClone(engineNodeOid)) {
        engineNodeId = new Long(engineNodeOid.substring(1));
        engineNodeOid = "";
      } else {
      ...
      if (crudsResult.isError()) {
        ...
        engineNodeDetails = crudsResult.getValue();
        engineNodeName = engineNodeDetails.getName();
      ...
      if (actionResult.equals(Action.SUCCESS)) {
        crudsResult = CrudsResult.success(engineNodeDetails);
      } else {
          Diagnostic diagnostic = Diagnostic.error("workflow", "EngineNode", parentField, "cannotSetParentField");
  ...
  private boolean shouldClone(String engineNodeOid) {
    return engineNodeOid.startsWith("c");
  }
  ...
  private String setParent(Long parentOid) {
    try {
        ...
        Method getParentDataRefMethod = EngineNodeAgent.class.getMethod(getParentDataRefMethodName, new Class[] {Long.class});

        DataRef parentRef = (DataRef) getParentDataRefMethod.invoke(engineNodeAgent, new Object[] {parentOid});

        String setParentMethodName = "set"+parentField;
        Method setParentMethod = EngineNodeDetails.class.getMethod(setParentMethodName, new Class[] {DataRef.class});

        setParentMethod.invoke(engineNodeDetails, new Object[] {parentRef});
