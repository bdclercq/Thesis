...
public class EngineNodeProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(EngineNodeProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "EngineNode.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "EngineNode.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "workflow", "net.democritus.wfe", "EngineNode");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    EngineNodeAgent engineNodeAgent = createEngineNodeAgent();
    crudsResult = engineNodeAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static EngineNodeAgent createEngineNodeAgent() {
    return EngineNodeAgent.getEngineNodeAgent(
