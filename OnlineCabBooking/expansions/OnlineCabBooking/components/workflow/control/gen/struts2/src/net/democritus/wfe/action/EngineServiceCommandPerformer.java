...
import net.democritus.wfe.EngineServiceCommand;
import net.democritus.wfe.EngineServiceCommandValidator;
...
public class EngineServiceCommandPerformer extends ActionSupport implements Preparable {
  ...
  public void prepare() throws Exception {
    ...
    Class<? extends ICommand> clazz = EngineServiceCommand.getCommandByName(commandName);
    if (clazz == null) {
    ...
    if (engineServiceAgent == null) {
      engineServiceAgent = EngineServiceAgent.getEngineServiceAgent(ContextRetriever.getContext());
    }
  ...
  @Override public void validate() {
    Option<ICommandValidator> optValidator = EngineServiceCommandValidator.getValidator(commandName);
  ...
  private void convertStrutsFieldErrors(Map<String, List<String>> fieldErrors) {
    ...
    for (Map.Entry<String, List<String>> entry : fieldErrors.entrySet()) {
      for (String errorMessage : entry.getValue()) {
        FieldError fieldError = new FieldError(entry.getKey(), "workflow.EngineService." + entry.getKey(), errorMessage);
