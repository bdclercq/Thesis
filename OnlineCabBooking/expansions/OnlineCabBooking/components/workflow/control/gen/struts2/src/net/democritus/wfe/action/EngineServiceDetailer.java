...
public class EngineServiceDetailer extends ActionSupport implements Preparable {

  private String parentOid = "";
  private String parentField = "";
  private CrudsResult<EngineServiceDetails> crudsResult;
  ...
  public CrudsResult<EngineServiceDetails> getCrudsResult() {
    return crudsResult;
  }

  public JsonResult<EngineServiceDetails> getJsonResult() {
  ...
  public String execute() throws Exception {
    ...
    if (engineServiceOid.length() > 0) {
      if (shouldClone(engineServiceOid)) {
        engineServiceId = new Long(engineServiceOid.substring(1));
        engineServiceOid = "";
      } else {
        engineServiceId = new Long(engineServiceOid);
      }

      crudsResult = engineServiceAgent.getDetails(engineServiceId);

      if (crudsResult.isError()) {
        actionResult = Action.INPUT;
      } else {
        engineServiceDetails = crudsResult.getValue();
        engineServiceName = engineServiceDetails.getName();
      ...
      if (actionResult.equals(Action.SUCCESS)) {
        crudsResult = CrudsResult.success(engineServiceDetails);
      } else {
          Diagnostic diagnostic = Diagnostic.error("workflow", "EngineService", parentField, "cannotSetParentField");
  ...
  private boolean shouldClone(String engineServiceOid) {
    return engineServiceOid.startsWith("c");
  }
  ...
  private String setParent(Long parentOid) {
    try {
        ...
        Method getParentDataRefMethod = EngineServiceAgent.class.getMethod(getParentDataRefMethodName, new Class[] {Long.class});

        DataRef parentRef = (DataRef) getParentDataRefMethod.invoke(engineServiceAgent, new Object[] {parentOid});

        String setParentMethodName = "set"+parentField;
        Method setParentMethod = EngineServiceDetails.class.getMethod(setParentMethodName, new Class[] {DataRef.class});

        setParentMethod.invoke(engineServiceDetails, new Object[] {parentRef});
