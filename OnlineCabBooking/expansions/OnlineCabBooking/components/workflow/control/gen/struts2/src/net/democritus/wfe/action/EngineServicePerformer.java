...
public class EngineServicePerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(EngineServicePerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      EngineServiceDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "EngineServicePerformer failed to perform task", e
        );
      }
  ...
  private void performTask(EngineServiceDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", EngineServiceDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(EngineServiceDetails details) throws Exception {
    Method setStatusMethod = EngineServiceDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    engineServiceAgent.modify(details);
  }
