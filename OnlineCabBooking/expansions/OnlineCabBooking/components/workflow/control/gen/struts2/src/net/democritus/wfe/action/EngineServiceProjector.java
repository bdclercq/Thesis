...
public class EngineServiceProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(EngineServiceProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "EngineService.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "EngineService.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "workflow", "net.democritus.wfe", "EngineService");
    ...
    crudsResult = engineServiceAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static EngineServiceAgent createEngineServiceAgent() {
    return EngineServiceAgent.getEngineServiceAgent(
