...
public class TimeWindowGroupPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(TimeWindowGroupPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    if (!timeWindowGroupOid.equals("")) {
      timeWindowGroupId = new Long(timeWindowGroupOid);
    } else {
      timeWindowGroupId = 0L;
    }
    ...
    try {
      TimeWindowGroupDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "TimeWindowGroupPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(TimeWindowGroupDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", TimeWindowGroupDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(TimeWindowGroupDetails details) throws Exception {
    Method setStatusMethod = TimeWindowGroupDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    timeWindowGroupAgent.modify(details);
  }
