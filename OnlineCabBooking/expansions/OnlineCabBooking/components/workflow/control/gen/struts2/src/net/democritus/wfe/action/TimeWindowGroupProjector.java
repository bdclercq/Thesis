...
public class TimeWindowGroupProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(TimeWindowGroupProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "TimeWindowGroup.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "TimeWindowGroup.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "workflow", "net.democritus.wfe", "TimeWindowGroup");
    ...
    crudsResult = timeWindowGroupAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static TimeWindowGroupAgent createTimeWindowGroupAgent() {
    return TimeWindowGroupAgent.getTimeWindowGroupAgent(
