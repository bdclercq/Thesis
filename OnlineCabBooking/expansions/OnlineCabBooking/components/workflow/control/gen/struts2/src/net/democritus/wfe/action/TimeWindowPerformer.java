...
public class TimeWindowPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(TimeWindowPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      TimeWindowDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "TimeWindowPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(TimeWindowDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", TimeWindowDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(TimeWindowDetails details) throws Exception {
    Method setStatusMethod = TimeWindowDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    timeWindowAgent.modify(details);
  }
