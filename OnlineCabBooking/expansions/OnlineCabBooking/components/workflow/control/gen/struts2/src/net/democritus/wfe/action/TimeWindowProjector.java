...
public class TimeWindowProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(TimeWindowProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "TimeWindow.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "TimeWindow.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "workflow", "net.democritus.wfe", "TimeWindow");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    TimeWindowAgent timeWindowAgent = createTimeWindowAgent();
    crudsResult = timeWindowAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static TimeWindowAgent createTimeWindowAgent() {
    return TimeWindowAgent.getTimeWindowAgent(
