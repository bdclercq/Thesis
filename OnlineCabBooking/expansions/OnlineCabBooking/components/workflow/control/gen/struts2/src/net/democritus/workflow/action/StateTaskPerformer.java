...
public class StateTaskPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(StateTaskPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      StateTaskDetails details = crudsResult.getValue();
      if (!taskName.equals("")) {
        performTask(details);
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "StateTaskPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(StateTaskDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", StateTaskDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(StateTaskDetails details) throws Exception {
    Method setStatusMethod = StateTaskDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    stateTaskAgent.modify(details);
  }
