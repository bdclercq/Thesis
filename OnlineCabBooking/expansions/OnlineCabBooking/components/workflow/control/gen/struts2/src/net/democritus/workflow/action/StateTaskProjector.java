...
public class StateTaskProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(StateTaskProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "StateTask.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "StateTask.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "workflow", "net.democritus.workflow", "StateTask");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    StateTaskAgent stateTaskAgent = createStateTaskAgent();
    crudsResult = stateTaskAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static StateTaskAgent createStateTaskAgent() {
    return StateTaskAgent.getStateTaskAgent(
