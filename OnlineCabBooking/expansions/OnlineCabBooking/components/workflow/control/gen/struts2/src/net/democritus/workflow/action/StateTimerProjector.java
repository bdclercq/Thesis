...
public class StateTimerProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(StateTimerProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "StateTimer.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "StateTimer.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "workflow", "net.democritus.workflow", "StateTimer");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    StateTimerAgent stateTimerAgent = createStateTimerAgent();
    crudsResult = stateTimerAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static StateTimerAgent createStateTimerAgent() {
    return StateTimerAgent.getStateTimerAgent(
