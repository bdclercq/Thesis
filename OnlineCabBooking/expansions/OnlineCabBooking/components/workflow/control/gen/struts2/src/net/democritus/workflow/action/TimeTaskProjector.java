...
public class TimeTaskProjector extends ActionSupport {
  ...
  private static final Logger logger = LoggerFactory.getLogger(TimeTaskProjector.class);
  ...
  public String execute() throws Exception {
    ...
    if (id == 0L) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "TimeTask.execute() failed: no id specified"
        );
      }
      return Action.ERROR;
    }
    ...
    if (projection.length() == 0) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "TimeTask.execute() failed: no projection specified"
        );
      }
      return Action.ERROR;
    }

    DataRef dataRef = new DataRef(id, null, "workflow", "net.democritus.workflow", "TimeTask");
    ProjectionRef projectionRef = new ProjectionRef(projection, dataRef);

    TimeTaskAgent timeTaskAgent = createTimeTaskAgent();
    crudsResult = timeTaskAgent.getProjection(projectionRef);

    return Action.SUCCESS;
  }

  private static TimeTaskAgent createTimeTaskAgent() {
    return TimeTaskAgent.getTimeTaskAgent(
