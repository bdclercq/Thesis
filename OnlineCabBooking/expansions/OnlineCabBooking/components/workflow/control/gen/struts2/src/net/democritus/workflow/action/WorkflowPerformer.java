...
public class WorkflowPerformer extends ActionSupport implements Preparable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(WorkflowPerformer.class);
  ...
  public String execute() throws Exception {
    ...
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "WorkflowPerformer failed to perform task", e
        );
      }
  ...
  private void performTask(WorkflowDetails details) throws Exception {
    ...
    Method performMethod = agentClass.getMethod("perform", WorkflowDetails.class, String.class);

    taskResult = (TaskResult<Void>) performMethod.invoke(agent, details, paramString);
  }

  public void setTaskStatus(WorkflowDetails details) throws Exception {
    Method setStatusMethod = WorkflowDetails.class.getMethod("setStatus", String.class);
    setStatusMethod.invoke(details, targetStatus);
    workflowAgent.modify(details);
  }
