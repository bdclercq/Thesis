...
public class EngineNodeDataRefProjector implements IDataElementProjector<EngineNodeData, DataRef>, IDataProjectorRequired<DataRef> {
  ...
  public EngineNodeDataRefProjector(EngineNodeCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  // anchor:project:start
  public DataRef project(ParameterContext<EngineNodeData> dataParameter) {

    DataRef projection = DataRef.withId(0L);
    projection.setComponentName("workflow");
    projection.setPackName("net.democritus.wfe");
    projection.setClassName("EngineNode");
  ...
  // anchor:toData:start
  public void toData(EngineNodeData data, ParameterContext<DataRef> projectionParameter) {
