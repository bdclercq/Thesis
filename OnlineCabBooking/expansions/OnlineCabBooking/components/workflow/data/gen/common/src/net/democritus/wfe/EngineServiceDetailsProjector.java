...
public class EngineServiceDetailsProjector implements IDataElementProjector<EngineServiceData, EngineServiceDetails>, IDataProjectorRequired<EngineServiceDetails> {
  ...
  public EngineServiceDetailsProjector(EngineServiceCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  public EngineServiceDetails project(ParameterContext<EngineServiceData> dataParameter) {

    EngineServiceDetails projection = new EngineServiceDetails();
    ...
    if (data != null) {
      ...
      projection.setEngineNodeServices(crudsInternal.getEngineNodeServices(dataParameter).getValueOrElse(EMPTY_DATA_REF_COLLECTION));
  ...
  public void toData(EngineServiceData data, ParameterContext<EngineServiceDetails> projectionParameter) {
    // don't set the data id
    EngineServiceDetails projection = projectionParameter.getValue();
  ...
  public DataRef getDataRef(EngineServiceDetails projection) {
  ...
  public Class<EngineServiceDetails> getProjectionClass() {
    return EngineServiceDetails.class;
  }
  ...
  public CrudsResult<Void> checkRequired(ParameterContext<EngineServiceDetails> projectionParameter) {
