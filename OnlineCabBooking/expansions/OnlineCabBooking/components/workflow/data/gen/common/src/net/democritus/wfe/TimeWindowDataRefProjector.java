...
public class TimeWindowDataRefProjector implements IDataElementProjector<TimeWindowData, DataRef>, IDataProjectorRequired<DataRef> {
  ...
  public TimeWindowDataRefProjector(TimeWindowCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  // anchor:project:start
  public DataRef project(ParameterContext<TimeWindowData> dataParameter) {

    DataRef projection = DataRef.withId(0L);
    projection.setComponentName("workflow");
    projection.setPackName("net.democritus.wfe");
    projection.setClassName("TimeWindow");
  ...
  // anchor:toData:start
  public void toData(TimeWindowData data, ParameterContext<DataRef> projectionParameter) {
