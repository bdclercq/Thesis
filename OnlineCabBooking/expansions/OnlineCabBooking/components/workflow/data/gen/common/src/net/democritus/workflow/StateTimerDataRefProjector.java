...
public class StateTimerDataRefProjector implements IDataElementProjector<StateTimerData, DataRef>, IDataProjectorRequired<DataRef> {
  ...
  public StateTimerDataRefProjector(StateTimerCrudsInternal crudsInternal) {
    this.crudsInternal = crudsInternal;
  }

  // anchor:project:start
  public DataRef project(ParameterContext<StateTimerData> dataParameter) {

    DataRef projection = DataRef.withId(0L);
    projection.setComponentName("workflow");
    projection.setPackName("net.democritus.workflow");
    projection.setClassName("StateTimer");
  ...
  // anchor:toData:start
  public void toData(StateTimerData data, ParameterContext<DataRef> projectionParameter) {
