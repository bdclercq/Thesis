...
@Local({StateTaskCrudsLocal.class, StateTaskCrudsInternal.class})
// @anchor:annotations:end
public class StateTaskCruds /*@anchor:interfaces:start@*/implements StateTaskCrudsLocal, StateTaskCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("workflow", "StateTask");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "workflow.stateTask.alreadyDisabled";
  ...
  private StateTaskFinderLocal stateTaskFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(StateTaskCruds.class);
  ...
  private StateTaskDataRefProjector dataRefProjector;
  private StateTaskDetailsProjector detailsProjector;
  private StateTaskInfoProjector infoProjector;

  private DataElementProjectorManager<StateTaskData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    StateTaskCrudsInternal stateTaskCrudsInternal = sessionContext.getBusinessObject(StateTaskCrudsInternal.class);

    detailsProjector = new StateTaskDetailsProjector(stateTaskCrudsInternal);
    infoProjector = new StateTaskInfoProjector(stateTaskCrudsInternal);
    dataRefProjector = new StateTaskDataRefProjector(stateTaskCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<StateTaskData>();
    ...
    elementProjectorManager.addProjector(new StateTaskDetailsWithoutRefsProjector(stateTaskCrudsInternal));
  ...
  private void initFinder() {
    stateTaskFinder = new StateTaskFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<StateTaskDetails> detailsParameter) {
    ...
    StateTaskDetails details = detailsParameter.getValue();
    ...
    StateTaskData stateTaskData = new StateTaskData();
    try {
      ...
      detailsProjector.toData(stateTaskData, detailsParameter);
    ...
    try {
      entityManager.persist(stateTaskData);

      // @anchor:implicitNameFieldOnly-afterPersist:start
      // @anchor:implicitNameFieldOnly-afterPersist:end

      if (stateTaskData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created StateTaskCruds with { id : " + stateTaskData.getId() + ", name: " + stateTaskData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(stateTaskData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<StateTaskDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    StateTaskDetails stateTaskDetails = detailsParameter.getValue();
    DataRef dataRef = stateTaskDetails.getDataRef();
    ...
    try {
      CrudsResult<StateTaskData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      StateTaskData stateTaskData = dataResult.getValue();
      ...
      if (stateTaskData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified StateTaskCruds with { id : " + stateTaskData.getId() + ", name: " + stateTaskData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "StateTaskCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof StateTaskDetails) {
      StateTaskDetails details = (StateTaskDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(stateTaskData);
      // @anchor:delete:end
      ...
      if (stateTaskData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted StateTaskCruds with { id : " + stateTaskData.getId() + ", name: " + stateTaskData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<StateTaskData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      StateTaskData stateTaskData = entityManager.find(StateTaskData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(stateTaskData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<StateTaskData> getDataById(ParameterContext<Long> idParameter) {
    StateTaskData data = entityManager.find(StateTaskData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<StateTaskData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<StateTaskData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      StateTaskFindByNameEqDetails findByNameEqDetails = new StateTaskFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<StateTaskFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<StateTaskFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<StateTaskData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<StateTaskData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<StateTaskData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<StateTaskData> result;
  ...
  private CrudsResult<StateTaskData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<StateTaskData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends StateTaskFindDetails> SearchResult<StateTaskData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return stateTaskFinder.find(searchParameter);
  }

  public <S extends StateTaskFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<StateTaskData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<StateTaskData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<StateTaskData> fromItems, IDataElementProjector<StateTaskData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<StateTaskData> fromItems, IDataElementProjector<StateTaskData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<StateTaskData>> listParameter, String projection) {
    IDataElementProjector<StateTaskData, P> elementProjector = getElementProjector(projection);
    List<StateTaskData> list = listParameter.getValue();
  ...
  @Override
  public void setWorkflow(StateTaskData stateTaskData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      stateTaskData.setWorkflow(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
    ...
    if (result.isSuccess()) {
      stateTaskData.setWorkflow(result.getValue().getId());
    } else {
  ...
  private <P> IDataElementProjector<StateTaskData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<StateTaskData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<StateTaskData> dataParameter) {
  ...
  private <T> IDataElementProjector<StateTaskData, T> getElementProjector(String projectionName) {
    IDataElementProjector<StateTaskData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("workflow", "StateTask"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<StateTaskData> dataResult = getData(projectionRefParameter.construct(dataRef));
    StateTaskData stateTaskData;
    if (dataResult.isSuccess()) {
      stateTaskData = dataResult.getValue();
    } else {
      stateTaskData = new StateTaskData();
    }

    // anchor:custom-preProject:start
    // anchor:custom-preProject:end

    IDataElementProjector<StateTaskData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(stateTaskData));

    if (stateTaskData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved StateTaskCruds:" + projectionRef.getProjection() + " { id : " + stateTaskData.getId() + ", name: " + stateTaskData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<StateTaskDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<StateTaskData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    StateTaskData stateTaskData = result.getValue();
    StateTaskDetails stateTaskDetails = detailsProjector.project(dataRefParameter.construct(stateTaskData));

    return CrudsResult.success(stateTaskDetails);
  }

  public CrudsResult<List<StateTaskDetails>> getDetailsListFromData(ParameterContext<Collection<StateTaskData>> dataListParameter) {
    Collection<StateTaskData> dataList = dataListParameter.getValue();
    List<StateTaskDetails> list = new ArrayList<>(dataList.size());

    for (StateTaskData stateTaskData: dataList) {
      StateTaskDetails stateTaskDetails = detailsProjector.project(dataListParameter.construct(stateTaskData));
      list.add(stateTaskDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<StateTaskData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    StateTaskData data = dataResult.getValue();
