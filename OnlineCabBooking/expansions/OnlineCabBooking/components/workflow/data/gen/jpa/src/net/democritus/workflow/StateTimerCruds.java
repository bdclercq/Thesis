...
@Local({StateTimerCrudsLocal.class, StateTimerCrudsInternal.class})
// @anchor:annotations:end
public class StateTimerCruds /*@anchor:interfaces:start@*/implements StateTimerCrudsLocal, StateTimerCrudsInternal/*@anchor:interfaces:end@*/ {

  private final DiagnosticHelper diagnosticHelper = new DiagnosticHelper("workflow", "StateTimer");
  private final DiagnosticFactory diagnosticFactory = diagnosticHelper.getDiagnosticFactory();

  public static final String DISABLED_ERROR_MSG_KEY = "workflow.stateTimer.alreadyDisabled";
  ...
  private StateTimerFinderLocal stateTimerFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(StateTimerCruds.class);
  ...
  private StateTimerDataRefProjector dataRefProjector;
  private StateTimerDetailsProjector detailsProjector;
  private StateTimerInfoProjector infoProjector;

  private DataElementProjectorManager<StateTimerData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    StateTimerCrudsInternal stateTimerCrudsInternal = sessionContext.getBusinessObject(StateTimerCrudsInternal.class);

    detailsProjector = new StateTimerDetailsProjector(stateTimerCrudsInternal);
    infoProjector = new StateTimerInfoProjector(stateTimerCrudsInternal);
    dataRefProjector = new StateTimerDataRefProjector(stateTimerCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<StateTimerData>();
  ...
  private void initFinder() {
    stateTimerFinder = new StateTimerFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<StateTimerDetails> detailsParameter) {
    ...
    StateTimerDetails details = detailsParameter.getValue();
    ...
    StateTimerData stateTimerData = new StateTimerData();
    try {
      ...
      detailsProjector.toData(stateTimerData, detailsParameter);
    ...
    try {
      entityManager.persist(stateTimerData);

      // @anchor:implicitNameFieldOnly-afterPersist:start
      // @anchor:implicitNameFieldOnly-afterPersist:end

      if (stateTimerData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created StateTimerCruds with { id : " + stateTimerData.getId() + ", name: " + stateTimerData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(stateTimerData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<StateTimerDetails> detailsParameter) {
    ...
    Context context = detailsParameter.getContext();
    StateTimerDetails stateTimerDetails = detailsParameter.getValue();
    DataRef dataRef = stateTimerDetails.getDataRef();
    ...
    try {
      CrudsResult<StateTimerData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      StateTimerData stateTimerData = dataResult.getValue();
      ...
      if (stateTimerData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified StateTimerCruds with { id : " + stateTimerData.getId() + ", name: " + stateTimerData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "StateTimerCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof StateTimerDetails) {
      StateTimerDetails details = (StateTimerDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(stateTimerData);
      // @anchor:delete:end
      ...
      if (stateTimerData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted StateTimerCruds with { id : " + stateTimerData.getId() + ", name: " + stateTimerData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<StateTimerData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      StateTimerData stateTimerData = entityManager.find(StateTimerData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(stateTimerData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<StateTimerData> getDataById(ParameterContext<Long> idParameter) {
    StateTimerData data = entityManager.find(StateTimerData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<StateTimerData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<StateTimerData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      StateTimerFindByNameEqDetails findByNameEqDetails = new StateTimerFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<StateTimerFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<StateTimerFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<StateTimerData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<StateTimerData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<StateTimerData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<StateTimerData> result;
  ...
  private CrudsResult<StateTimerData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<StateTimerData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends StateTimerFindDetails> SearchResult<StateTimerData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return stateTimerFinder.find(searchParameter);
  }

  public <S extends StateTimerFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<StateTimerData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<StateTimerData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<StateTimerData> fromItems, IDataElementProjector<StateTimerData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<StateTimerData> fromItems, IDataElementProjector<StateTimerData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<StateTimerData>> listParameter, String projection) {
    IDataElementProjector<StateTimerData, P> elementProjector = getElementProjector(projection);
    List<StateTimerData> list = listParameter.getValue();
  ...
  @Override
  public void setWorkflow(StateTimerData stateTimerData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      stateTimerData.setWorkflow(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
    ...
    if (result.isSuccess()) {
      stateTimerData.setWorkflow(result.getValue().getId());
    } else {
  ...
  private <P> IDataElementProjector<StateTimerData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<StateTimerData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<StateTimerData> dataParameter) {
  ...
  private <T> IDataElementProjector<StateTimerData, T> getElementProjector(String projectionName) {
    IDataElementProjector<StateTimerData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("workflow", "StateTimer"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<StateTimerData> dataResult = getData(projectionRefParameter.construct(dataRef));
    StateTimerData stateTimerData;
    if (dataResult.isSuccess()) {
      stateTimerData = dataResult.getValue();
    } else {
      stateTimerData = new StateTimerData();
    }

    // anchor:custom-preProject:start
    // anchor:custom-preProject:end

    IDataElementProjector<StateTimerData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(stateTimerData));

    if (stateTimerData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved StateTimerCruds:" + projectionRef.getProjection() + " { id : " + stateTimerData.getId() + ", name: " + stateTimerData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<StateTimerDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<StateTimerData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    StateTimerData stateTimerData = result.getValue();
    StateTimerDetails stateTimerDetails = detailsProjector.project(dataRefParameter.construct(stateTimerData));

    return CrudsResult.success(stateTimerDetails);
  }

  public CrudsResult<List<StateTimerDetails>> getDetailsListFromData(ParameterContext<Collection<StateTimerData>> dataListParameter) {
    Collection<StateTimerData> dataList = dataListParameter.getValue();
    List<StateTimerDetails> list = new ArrayList<>(dataList.size());

    for (StateTimerData stateTimerData: dataList) {
      StateTimerDetails stateTimerDetails = detailsProjector.project(dataListParameter.construct(stateTimerData));
      list.add(stateTimerDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<StateTimerData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    StateTimerData data = dataResult.getValue();
