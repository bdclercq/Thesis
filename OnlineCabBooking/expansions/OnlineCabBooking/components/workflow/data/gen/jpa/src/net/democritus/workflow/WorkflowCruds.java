...
@Local({WorkflowCrudsLocal.class, WorkflowCrudsInternal.class})
// @anchor:annotations:end
public class WorkflowCruds /*@anchor:interfaces:start@*/implements WorkflowCrudsLocal, WorkflowCrudsInternal/*@anchor:interfaces:end@*/ {
  ...
  public static final String DISABLED_ERROR_MSG_KEY = "workflow.workflow.alreadyDisabled";
  ...
  private WorkflowFinderLocal workflowFinder;
  ...
  private static final Logger logger = LoggerFactory.getLogger(WorkflowCruds.class);
  ...
  private WorkflowDataRefProjector dataRefProjector;
  private WorkflowDetailsProjector detailsProjector;
  private WorkflowInfoProjector infoProjector;

  private DataElementProjectorManager<WorkflowData> elementProjectorManager;
  ...
  private void initProjectorMapping() {
    WorkflowCrudsInternal workflowCrudsInternal = sessionContext.getBusinessObject(WorkflowCrudsInternal.class);

    detailsProjector = new WorkflowDetailsProjector(workflowCrudsInternal);
    infoProjector = new WorkflowInfoProjector(workflowCrudsInternal);
    dataRefProjector = new WorkflowDataRefProjector(workflowCrudsInternal);

    elementProjectorManager = new DataElementProjectorManager<WorkflowData>();
  ...
  private void initFinder() {
    workflowFinder = new WorkflowFinderBean(entityManager);
  }
  ...
  public CrudsResult<DataRef> create(ParameterContext<WorkflowDetails> detailsParameter) {
    ...
    try {
      ...
      detailsProjector.toData(workflowData, detailsParameter);
    ...
    try {
      entityManager.persist(workflowData);
      ...
      if (workflowData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Created WorkflowCruds with { id : " + workflowData.getId() + ", name: " + workflowData.getName() + " }"
          );
        }
      }
    } catch (Exception e) {
    ...
    CrudsResult<DataRef> result = getDataRefFromData(detailsParameter.construct(workflowData));
  ...
  public CrudsResult<DataRef> modify(ParameterContext<WorkflowDetails> detailsParameter) {
    ...
    DataRef dataRef = workflowDetails.getDataRef();
    ...
    try {
      CrudsResult<WorkflowData> dataResult = getData(context.withParameter(dataRef));
      if (dataResult.isError()) {
        return dataResult.convertError();
      }
      WorkflowData workflowData = dataResult.getValue();
      ...
      if (workflowData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Modified WorkflowCruds with { id : " + workflowData.getId() + ", name: " + workflowData.getName() + " }"
          );
        }
      }
    ...
    // @anchor:modify-exceptions:end
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "WorkflowCruds.modify() failed with ID = " + dataRef.getId(), e
        );
      }
      return getDiagnosticHelper().createCrudsError(MODIFY_ERROR_MSG_KEY);
    }
  ...
  public <P> CrudsResult<DataRef> createOrModify(ParameterContext<P> projectionParameter) {
    ...
    if (projection instanceof WorkflowDetails) {
      WorkflowDetails details = (WorkflowDetails) projection;
  ...
  public CrudsResult<Void> delete(ParameterContext<DataRef> dataRefParameter) {
    ...
    try {
      ...
      // @anchor:delete:start
      entityManager.remove(workflowData);
      // @anchor:delete:end
      ...
      if (workflowData != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Deleted WorkflowCruds with { id : " + workflowData.getId() + ", name: " + workflowData.getName() + " }"
          );
        }
      }
  ...
  public CrudsResult<DataRef> getDataRefFromData(ParameterContext<WorkflowData> dataParameter) {
  ...
  public CrudsResult<DataRef> getDataRefFromId(ParameterContext<Long> idParameter) {
    ...
    try {
      WorkflowData workflowData = entityManager.find(WorkflowData.class, id);
      ...
      return getDataRefFromData(idParameter.construct(workflowData));
    } catch (Exception e) {
      sessionContext.setRollbackOnly();
      if (logger.isErrorEnabled()) {
        logger.error(
  ...
  private CrudsResult<WorkflowData> getDataById(ParameterContext<Long> idParameter) {
    WorkflowData data = entityManager.find(WorkflowData.class, idParameter.getValue());
    if (data == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
  ...
  public CrudsResult<DataRef> getDataRefFromName(ParameterContext<String> nameParameter) {
    ...
    CrudsResult<WorkflowData> dataResult = getDataByName(nameParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
  ...
  private CrudsResult<WorkflowData> getDataByName(ParameterContext<String> nameParameter) {
    String name = nameParameter.getValue();
    if (name == null || name.trim().equals("")) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    try {
      WorkflowFindByNameEqDetails findByNameEqDetails = new WorkflowFindByNameEqDetails();
      findByNameEqDetails.setName(name);

      SearchDetails<WorkflowFindByNameEqDetails> searchDetails =
        new SearchDetails<>(findByNameEqDetails);

      ParameterContext<SearchDetails<WorkflowFindByNameEqDetails>> searchParameter =
        nameParameter.construct(searchDetails);

      SearchResult<WorkflowData> searchResult = findData(searchParameter);

      if (searchResult.isError()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }

      List<WorkflowData> list = searchResult.getResults();
      if (list.isEmpty()) {
        return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
      }
  ...
  public CrudsResult<WorkflowData> getData(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    if (dataRef == null) {
      return getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
    }
    ...
    CrudsResult<WorkflowData> result;
  ...
  private CrudsResult<WorkflowData> getDataFromStrategy(ParameterContext<DataRef> dataRefParameter) {
    DataRef dataRef = dataRefParameter.getValue();
    Long id = dataRef.getId();
    CrudsResult<WorkflowData> result = getDiagnosticHelper().createCrudsError(SEARCH_ERROR_MSG_KEY);
  ...
  public <S extends WorkflowFindDetails> SearchResult<WorkflowData> findData(ParameterContext<SearchDetails<S>> searchParameter) {
    return workflowFinder.find(searchParameter);
  }

  public <S extends WorkflowFindDetails, T> SearchResult<T> find(ParameterContext<SearchDetails<S>> searchParameter) {
    SearchResult<WorkflowData> dataResult = findData(searchParameter);
    ...
    IDataElementProjector<WorkflowData, T> projector = getElementProjector(projection);
   ...
   */
  private <T> List<T> project(Context context, List<WorkflowData> fromItems, IDataElementProjector<WorkflowData, T> projector) {
   ...
   */
  @Deprecated
  private <T> List<T> project(UserContext userContext, List<WorkflowData> fromItems, IDataElementProjector<WorkflowData, T> projector) {
    return project(Context.from(userContext), fromItems, projector);
  }

  @Override
  public <P> SearchResult<P> project(ParameterContext<List<WorkflowData>> listParameter, String projection) {
    IDataElementProjector<WorkflowData, P> elementProjector = getElementProjector(projection);
    List<WorkflowData> list = listParameter.getValue();
  ...
  public CrudsResult<DataRef> getResponsible(ParameterContext<Long> idParameter) {
    try {
  ...
  public CrudsResult<UserDetails> getResponsibleDetails(ParameterContext<DataRef> dataRefParameter) {
    try {
  ...
  @Override
  public void setResponsible(WorkflowData workflowData, ParameterContext<DataRef> dataRefParameter) {
    if (!DataRefValidation.isDataRefDefined(dataRefParameter.getValue())) {
      workflowData.setResponsible(0L);
      return;
    }
    CrudsResult<DataRef> result;
    try {
    ...
    if (result.isSuccess()) {
      workflowData.setResponsible(result.getValue().getId());
    } else {
  ...
  private <P> IDataElementProjector<WorkflowData, P> getElementProjector(P projection) {
  ...
  @Override
  public String getDisplayName(ParameterContext<WorkflowData> dataParameter) {
  ...
  private Option<String> getCustomDisplayName(ParameterContext<WorkflowData> dataParameter) {
  ...
  private <T> IDataElementProjector<WorkflowData, T> getElementProjector(String projectionName) {
    IDataElementProjector<WorkflowData, T> projector = elementProjectorManager.getProjector(projectionName);
    if (projector == null) {
      throw new InvalidProjectionException(new ElementRef("workflow", "Workflow"), projectionName);
    }
    return projector;
  }
  ...
  public <T> CrudsResult<T> getProjection(ParameterContext<ProjectionRef> projectionRefParameter) {
    ...
    CrudsResult<WorkflowData> dataResult = getData(projectionRefParameter.construct(dataRef));
    WorkflowData workflowData;
    if (dataResult.isSuccess()) {
      workflowData = dataResult.getValue();
    } else {
    ...
    IDataElementProjector<WorkflowData, T> projector = getElementProjector(projectionRef.getProjection());
    T projection = projector.project(projectionRefParameter.construct(workflowData));

    if (workflowData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Retrieved WorkflowCruds:" + projectionRef.getProjection() + " { id : " + workflowData.getId() + ", name: " + workflowData.getName() + " }"
        );
      }
    }
  ...
  public CrudsResult<WorkflowDetails> getDetails(ParameterContext<DataRef> dataRefParameter) {
    CrudsResult<WorkflowData> result = getData(dataRefParameter);
    if (result.isError()) {
      return result.convertError();
    }

    WorkflowData workflowData = result.getValue();
    WorkflowDetails workflowDetails = detailsProjector.project(dataRefParameter.construct(workflowData));

    return CrudsResult.success(workflowDetails);
  }

  public CrudsResult<List<WorkflowDetails>> getDetailsListFromData(ParameterContext<Collection<WorkflowData>> dataListParameter) {
    Collection<WorkflowData> dataList = dataListParameter.getValue();
    List<WorkflowDetails> list = new ArrayList<>(dataList.size());

    for (WorkflowData workflowData: dataList) {
      WorkflowDetails workflowDetails = detailsProjector.project(dataListParameter.construct(workflowData));
      list.add(workflowDetails);
    }
  ...
  public CrudsResult<DataRef> resolveDataRef(ParameterContext<DataRef> dataRefParameter) {
    ...
    CrudsResult<WorkflowData> dataResult = getData(dataRefParameter);
    if (dataResult.isError()) {
      return dataResult.convertError();
    }
    WorkflowData data = dataResult.getValue();
