...
public class StateTaskCsvImporterImpl implements IImporter {
  private static final Logger logger = LoggerFactory.getLogger(StateTaskCsvImporterImpl.class);

  private StateTaskMapper stateTaskMapper = new StateTaskMapper();
  ...
  private void importInstance(ParameterContext<ImportLine> importLineParam, ImportResult importResult) {
    ...
    Result<StateTaskDetails> mappingResult = stateTaskMapper.convertToDetails(importLineParam.construct(line.getValues()));
    if (mappingResult.isError()) {
      importResult.addError(new ImportError(line.getIndex(), mappingResult.getMessage()));
      return;
    }
    StateTaskDetails stateTaskDetails = mappingResult.getValue();
    ...
    CrudsResult<DataRef> crudsResult = persistStateTask(importLineParam.construct(stateTaskDetails));
  ...
  private CrudsResult<DataRef> persistStateTask(ParameterContext<StateTaskDetails> detailsParameter) {
    StateTaskLocalAgent stateTaskAgent = StateTaskLocalAgent.getStateTaskAgent(detailsParameter.getContext());
    ...
    return stateTaskAgent.createOrModify(stateTaskDetails);
  }
