...
/**
 * Stateless session bean encapsulating the implementation of the task RestoreEngines.
 */

@Stateless
@Remote(RestoreEnginesRemote.class)
@Local(RestoreEnginesLocal.class)
...
public class RestoreEnginesBean implements RestoreEnginesRemote, RestoreEnginesLocal {

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(RestoreEnginesBean.class);
  ...
  public TaskResult<Void> perform(ParameterContext<EngineNodeDetails> targetParameter) {
    ...
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "Failed to perform RestoreEngines on EngineNodeDetails", e
        );
      }
    }
