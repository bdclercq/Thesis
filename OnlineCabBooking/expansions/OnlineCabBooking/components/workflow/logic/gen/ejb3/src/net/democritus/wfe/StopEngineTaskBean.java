...
/**
 * Stateless session bean encapsulating the implementation of the task StopEngineTask.
 */

@Stateless
@Remote(StopEngineTaskRemote.class)
@Local(StopEngineTaskLocal.class)
...
public class StopEngineTaskBean implements StopEngineTaskRemote, StopEngineTaskLocal {

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(StopEngineTaskBean.class);
  ...
  public TaskResult<Void> perform(ParameterContext<EngineServiceDetails> targetParameter) {
    ...
    // anchor:custom-prePerform:start
        ...
        Integer result = timerHandlerLocal.stopTimer(targetParameter.getValue().getName());
        if(result== -1){
    ...
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
            "Failed to perform StopEngineTask on EngineServiceDetails", e
        );
      }
    }
