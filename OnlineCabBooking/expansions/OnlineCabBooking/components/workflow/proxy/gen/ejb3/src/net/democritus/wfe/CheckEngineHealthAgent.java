 ...
 */
public class CheckEngineHealthAgent {

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(CheckEngineHealthAgent.class);
  // @anchor:variables:end

  private static final CheckEngineHealthProxy checkEngineHealthProxy = CheckEngineHealthProxy.getCheckEngineHealthProxy();
  ...
  private CheckEngineHealthAgent(Context context, UserContext userContext) {
    this.mContext = context;
    this.mUserContext = userContext;
  }

  public static CheckEngineHealthAgent getCheckEngineHealthAgent(Context context) {
    UserContext userContext = context
        .getContext(UserContext.class)
        .orElse(UserContext.NO_USER_CONTEXT);
    return new CheckEngineHealthAgent(context, userContext);
  }

  public static CheckEngineHealthAgent getCheckEngineHealthAgent(UserContext userContext) {
    return new CheckEngineHealthAgent(Context.from(userContext), userContext);
  }
  ...
  private BasicProcessingContext getProcessingContext(String paramString) {
    return new BasicProcessingContext("net.democritus.wfe.CheckEngineHealthImpl", paramString);
  }
  ...
  public TaskResult<Void> perform(EngineNodeDetails engineNodeDetails, String paramString) {
    ...
    try {
      taskResult = checkEngineHealthProxy.perform(parameterContext);
    } catch (Exception e) {
