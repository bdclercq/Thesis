 ...
 */
public class StartEngineTaskAgent {

  // @anchor:variables:start
  private static final Logger logger = LoggerFactory.getLogger(StartEngineTaskAgent.class);
  // @anchor:variables:end

  private static final StartEngineTaskProxy startEngineTaskProxy = StartEngineTaskProxy.getStartEngineTaskProxy();
  ...
  private StartEngineTaskAgent(Context context, UserContext userContext) {
    this.mContext = context;
    this.mUserContext = userContext;
  }

  public static StartEngineTaskAgent getStartEngineTaskAgent(Context context) {
    UserContext userContext = context
        .getContext(UserContext.class)
        .orElse(UserContext.NO_USER_CONTEXT);
    return new StartEngineTaskAgent(context, userContext);
  }

  public static StartEngineTaskAgent getStartEngineTaskAgent(UserContext userContext) {
    return new StartEngineTaskAgent(Context.from(userContext), userContext);
  }
  ...
  private BasicProcessingContext getProcessingContext(String paramString) {
    return new BasicProcessingContext("net.democritus.wfe.StartEngineTaskImpl", paramString);
  }
  ...
  public TaskResult<Void> perform(EngineServiceDetails engineServiceDetails, String paramString) {
    ...
    try {
      taskResult = startEngineTaskProxy.perform(parameterContext);
    } catch (Exception e) {
