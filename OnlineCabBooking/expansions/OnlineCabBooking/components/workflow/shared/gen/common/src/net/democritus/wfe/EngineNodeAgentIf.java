...
/**
 * interface for the entity bean EngineNodeBean,
 * representing a known EngineNode
 */
public interface EngineNodeAgentIf {
  ...
  CrudsResult<DataRef> create(EngineNodeDetails engineNodeDetails);
  CrudsResult<DataRef> modify(EngineNodeDetails engineNodeDetails);
  ...
  CrudsResult<EngineNodeDetails> getDetails(DataRef dataRef);
  ...
  <S extends EngineNodeFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<EngineNodeDetails> getDetails(Long engineNodeId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<EngineNodeInfo> findAllInfos();
