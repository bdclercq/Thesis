...
/**
 * interface for the entity bean TimeWindowBean,
 * representing a known TimeWindow
 */
public interface TimeWindowAgentIf {
  ...
  CrudsResult<DataRef> create(TimeWindowDetails timeWindowDetails);
  CrudsResult<DataRef> modify(TimeWindowDetails timeWindowDetails);
  ...
  CrudsResult<TimeWindowDetails> getDetails(DataRef dataRef);
  ...
  <S extends TimeWindowFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<TimeWindowDetails> getDetails(Long timeWindowId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<TimeWindowInfo> findAllInfos();
