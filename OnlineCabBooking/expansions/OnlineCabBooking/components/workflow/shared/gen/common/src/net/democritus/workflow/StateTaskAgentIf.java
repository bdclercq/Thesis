...
/**
 * interface for the entity bean StateTaskBean,
 * representing a known StateTask
 */
public interface StateTaskAgentIf {
  ...
  CrudsResult<DataRef> create(StateTaskDetails stateTaskDetails);
  CrudsResult<DataRef> modify(StateTaskDetails stateTaskDetails);
  ...
  CrudsResult<StateTaskDetails> getDetails(DataRef dataRef);
  ...
  <S extends StateTaskFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<StateTaskDetails> getDetails(Long stateTaskId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<StateTaskInfo> findAllInfos();
