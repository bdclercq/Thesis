...
/**
 * interface for the entity bean TimeTaskBean,
 * representing a known TimeTask
 */
public interface TimeTaskAgentIf {
  ...
  CrudsResult<DataRef> create(TimeTaskDetails timeTaskDetails);
  CrudsResult<DataRef> modify(TimeTaskDetails timeTaskDetails);
  ...
  CrudsResult<TimeTaskDetails> getDetails(DataRef dataRef);
  ...
  <S extends TimeTaskFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<TimeTaskDetails> getDetails(Long timeTaskId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<TimeTaskInfo> findAllInfos();
