...
/**
 * interface for the entity bean WorkflowBean,
 * representing a known Workflow
 */
public interface WorkflowAgentIf {
  ...
  CrudsResult<DataRef> create(WorkflowDetails workflowDetails);
  CrudsResult<DataRef> modify(WorkflowDetails workflowDetails);
  ...
  CrudsResult<WorkflowDetails> getDetails(DataRef dataRef);
  ...
  <S extends WorkflowFindDetails, T> SearchResult<T> find(SearchDetails<S> searchDetails);
  // anchor:search-methods:end

  // anchor:data-only-methods:start
  CrudsResult<WorkflowDetails> getDetails(Long workflowId);
  CrudsResult<DataRef> getDataRef(String name);

  SearchResult<DataRef> findAllDataRefs();
  SearchResult<WorkflowInfo> findAllInfos();
