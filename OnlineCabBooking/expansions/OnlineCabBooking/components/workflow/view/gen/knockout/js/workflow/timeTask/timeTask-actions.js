define(function (require) {
  ...
  const TimeTaskEvents = require('workflow/timeTask/timeTask-events');
  const TimeTaskElement = require('nsx/nsx-application').getElement('workflow', 'timeTask');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.timeTask.create"}),
      updateEvent: TimeTaskEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(TimeTaskElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.timeTask.edit"}),
      updateEvent: TimeTaskEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(TimeTaskElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.timeTask.delete"}),
      updateEvent: TimeTaskEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(TimeTaskElement, "delete-json"),
          data: JSON.stringify({
            "timeTaskOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
