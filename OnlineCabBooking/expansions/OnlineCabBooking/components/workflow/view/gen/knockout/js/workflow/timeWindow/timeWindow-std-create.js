...
define(function (require) {
  ...
  var TimeWindowForm = require('workflow/timeWindow/timeWindow-instance-form');
  ...
  function defineCreateDialog(options) {
    ...
    when(createRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to create a(n) TimeWindow element!")
    });

    function render() {
      var createIntent = TimeWindowEvents.defineCreateIntentListener();

      var constants = ko.observable();
      var values = ko.observable();

      var formView = "workflow/timeWindow-form";
      var title = "TimeWindow";
      var layoutConfig = initLayoutConfig(constants);
      var defaultValues = { id: 0 };
      var formOptions = {
        resetOn: createIntent,
        action: TimeWindowActions.createAction()
      };
      ...
      var form = TimeWindowForm.defineForm({
        data: formData,
  ...
  function initLayoutConfig(constants) {
    return {
      ...
      startTime: {
          disabled: ko.pureComputed(function () {
            return utils.isDefined(constants().startTime)
          })
      },
      stopTime: {
          disabled: ko.pureComputed(function () {
            return utils.isDefined(constants().stopTime)
          })
      }
