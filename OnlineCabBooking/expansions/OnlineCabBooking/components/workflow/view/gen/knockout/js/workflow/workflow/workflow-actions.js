define(function (require) {
  ...
  const WorkflowEvents = require('workflow/workflow/workflow-events');
  const WorkflowElement = require('nsx/nsx-application').getElement('workflow', 'workflow');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.workflow.create"}),
      updateEvent: WorkflowEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(WorkflowElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.workflow.edit"}),
      updateEvent: WorkflowEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(WorkflowElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.workflow.delete"}),
      updateEvent: WorkflowEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(WorkflowElement, "delete-json"),
          data: JSON.stringify({
            "workflowOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
