define(function (require) {
  ...
  const DataAccessEvents = require('account/dataAccess/dataAccess-events');
  const DataAccessElement = require('nsx/nsx-application').getElement('account', 'dataAccess');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.dataAccess.create"}),
      updateEvent: DataAccessEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(DataAccessElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.dataAccess.edit"}),
      updateEvent: DataAccessEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(DataAccessElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.dataAccess.delete"}),
      updateEvent: DataAccessEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(DataAccessElement, "delete-json"),
          data: JSON.stringify({
            "dataAccessOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
