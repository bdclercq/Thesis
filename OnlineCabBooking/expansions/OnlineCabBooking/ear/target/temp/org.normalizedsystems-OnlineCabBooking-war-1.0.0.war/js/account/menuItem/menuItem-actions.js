define(function (require) {
  ...
  const MenuItemEvents = require('account/menuItem/menuItem-events');
  const MenuItemElement = require('nsx/nsx-application').getElement('account', 'menuItem');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.menuItem.create"}),
      updateEvent: MenuItemEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(MenuItemElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.menuItem.edit"}),
      updateEvent: MenuItemEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(MenuItemElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.menuItem.delete"}),
      updateEvent: MenuItemEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(MenuItemElement, "delete-json"),
          data: JSON.stringify({
            "menuItemOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
