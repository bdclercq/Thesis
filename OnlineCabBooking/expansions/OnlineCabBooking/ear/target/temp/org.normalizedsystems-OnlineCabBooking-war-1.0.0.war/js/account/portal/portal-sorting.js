...
define(function (require) {
  ...
  function defineSorting(input, options) {
    ...
    viewModel.version = SortableColumn.defineSortableColumn({
      fieldName: "version"
    }, {
      resetOn: Trigger.defineFilter({
        trigger: selectSorting,
        condition: function (sortField) {
          return sortField === null || sortField.fieldName !== "version";
        }
      })
    });

    viewModel.description = SortableColumn.defineSortableColumn({
      fieldName: "description"
    }, {
      resetOn: Trigger.defineFilter({
        trigger: selectSorting,
        condition: function (sortField) {
          return sortField === null || sortField.fieldName !== "description";
        }
      })
    });
    ...
    selectSorting.addTrigger(viewModel.version.select);
    selectSorting.addTrigger(viewModel.description.select);
