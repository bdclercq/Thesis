define(function (require) {
  ...
  const ScreenEvents = require('account/screen/screen-events');
  const ScreenElement = require('nsx/nsx-application').getElement('account', 'screen');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.screen.create"}),
      updateEvent: ScreenEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(ScreenElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.screen.edit"}),
      updateEvent: ScreenEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(ScreenElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.screen.delete"}),
      updateEvent: ScreenEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(ScreenElement, "delete-json"),
          data: JSON.stringify({
            "screenOid": target.id
          }),
          dataType: 'json',
