define(function (require) {
  ...
  const ScreenProfileEvents = require('account/screenProfile/screenProfile-events');
  const ScreenProfileElement = require('nsx/nsx-application').getElement('account', 'screenProfile');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.screenProfile.create"}),
      updateEvent: ScreenProfileEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(ScreenProfileElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.screenProfile.edit"}),
      updateEvent: ScreenProfileEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(ScreenProfileElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.screenProfile.delete"}),
      updateEvent: ScreenProfileEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(ScreenProfileElement, "delete-json"),
          data: JSON.stringify({
            "screenProfileOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
