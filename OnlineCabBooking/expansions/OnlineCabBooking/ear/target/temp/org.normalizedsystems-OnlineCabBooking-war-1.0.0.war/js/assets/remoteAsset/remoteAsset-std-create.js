...
define(function (require) {
  ...
  var RemoteAssetForm = require('assets/remoteAsset/remoteAsset-instance-form');
  var RemoteAssetActions = require('assets/remoteAsset/remoteAsset-actions');
  ...
  function defineCreateDialog(options) {
    ...
    when(createRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to create a(n) RemoteAsset element!")
    });

    function render() {
      var createIntent = RemoteAssetEvents.defineCreateIntentListener();

      var constants = ko.observable();
      var values = ko.observable();

      var formView = "assets/remoteAsset-form";
      var title = "RemoteAsset";
      var layoutConfig = initLayoutConfig(constants);
      var defaultValues = { id: 0 };
      var formOptions = {
        resetOn: createIntent,
        action: RemoteAssetActions.createAction()
      };
      ...
      var form = RemoteAssetForm.defineForm({
        data: formData,
  ...
  function initLayoutConfig(constants) {
    return {
      ...
      url: {
          disabled: ko.pureComputed(function () {
            return utils.isDefined(constants().url)
          })
      }
