...
define(function (require) {
  ...
  var CarTypeForm = require('onlineCabBookingComp/carType/carType-instance-form');
  var CarTypeActions = require('onlineCabBookingComp/carType/carType-actions');
  ...
  function defineCreateDialog(options) {
    ...
    when(createRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to create a(n) CarType element!")
    });

    function render() {
      var createIntent = CarTypeEvents.defineCreateIntentListener();

      var constants = ko.observable();
      var values = ko.observable();

      var formView = "onlineCabBookingComp/carType-form";
      var title = "CarType";
      var layoutConfig = initLayoutConfig(constants);
      var defaultValues = { id: 0 };
      var formOptions = {
        resetOn: createIntent,
        action: CarTypeActions.createAction()
      };
      ...
      var form = CarTypeForm.defineForm({
        data: formData,
