define(function (require) {
  ...
  const CustomerEvents = require('onlineCabBookingComp/customer/customer-events');
  const CustomerElement = require('nsx/nsx-application').getElement('onlineCabBookingComp', 'customer');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.customer.create"}),
      updateEvent: CustomerEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(CustomerElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.customer.edit"}),
      updateEvent: CustomerEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(CustomerElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.customer.delete"}),
      updateEvent: CustomerEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(CustomerElement, "delete-json"),
          data: JSON.stringify({
            "customerOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
