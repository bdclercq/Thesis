define(function (require) {
  ...
  const TripBookingEvents = require('onlineCabBookingComp/tripBooking/tripBooking-events');
  const TripBookingElement = require('nsx/nsx-application').getElement('onlineCabBookingComp', 'tripBooking');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.tripBooking.create"}),
      updateEvent: TripBookingEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(TripBookingElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.tripBooking.edit"}),
      updateEvent: TripBookingEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(TripBookingElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.tripBooking.delete"}),
      updateEvent: TripBookingEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(TripBookingElement, "delete-json"),
          data: JSON.stringify({
            "tripBookingOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
