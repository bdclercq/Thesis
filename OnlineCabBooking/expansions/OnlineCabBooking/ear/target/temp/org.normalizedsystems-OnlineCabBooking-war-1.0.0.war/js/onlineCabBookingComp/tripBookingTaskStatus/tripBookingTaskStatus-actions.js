define(function (require) {
  ...
  const TripBookingTaskStatusEvents = require('onlineCabBookingComp/tripBookingTaskStatus/tripBookingTaskStatus-events');
  const TripBookingTaskStatusElement = require('nsx/nsx-application').getElement('onlineCabBookingComp', 'tripBookingTaskStatus');
   ...
   * }} params
   * @template I
   * @return {TripBookingTaskStatusAction<I>}
   */
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.tripBookingTaskStatus.create"}),
      updateEvent: TripBookingTaskStatusEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(TripBookingTaskStatusElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.tripBookingTaskStatus.edit"}),
      updateEvent: TripBookingTaskStatusEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(TripBookingTaskStatusElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.tripBookingTaskStatus.delete"}),
      updateEvent: TripBookingTaskStatusEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(TripBookingTaskStatusElement, "delete-json"),
          data: JSON.stringify({
            "tripBookingTaskStatusOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
