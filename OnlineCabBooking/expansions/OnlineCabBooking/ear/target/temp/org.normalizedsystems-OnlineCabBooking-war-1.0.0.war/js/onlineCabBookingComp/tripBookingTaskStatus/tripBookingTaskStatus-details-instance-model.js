  ...
  function defineInstanceView(input, options) {
    ...
    function initViewmodel() {
      return {
        ...
        finishedAt: ko.observable(),
        status: ko.observable(),
        stateTask: defineLink(StateTaskEvents),
    ...
    function updateViewmodel(tripBookingTaskStatusDetails) {
      if (!!tripBookingTaskStatusDetails) {
        viewmodel.id(tripBookingTaskStatusDetails.id);
        // anchor:update-viewModel:start
        viewmodel.name(tripBookingTaskStatusDetails.name);
        viewmodel.startedAt(tripBookingTaskStatusDetails.startedAt);
        viewmodel.finishedAt(tripBookingTaskStatusDetails.finishedAt);
        viewmodel.status(tripBookingTaskStatusDetails.status);
        viewmodel.stateTask.value(tripBookingTaskStatusDetails.stateTask);
        viewmodel.tripBooking.value(tripBookingTaskStatusDetails.tripBooking);
