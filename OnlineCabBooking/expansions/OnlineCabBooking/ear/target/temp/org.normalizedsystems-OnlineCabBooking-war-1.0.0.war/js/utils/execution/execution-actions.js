define(function (require) {
  ...
  const ExecutionEvents = require('utils/execution/execution-events');
  const ExecutionElement = require('nsx/nsx-application').getElement('utils', 'execution');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "utils.execution.create"}),
      updateEvent: ExecutionEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(ExecutionElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "utils.execution.edit"}),
      updateEvent: ExecutionEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(ExecutionElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "utils.execution.delete"}),
      updateEvent: ExecutionEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(ExecutionElement, "delete-json"),
          data: JSON.stringify({
            "executionOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
