...
define(function (require) {
  ...
  function defineSorting(input, options) {
    ...
    viewModel.param = SortableColumn.defineSortableColumn({
      fieldName: "param"
    }, {
      resetOn: Trigger.defineFilter({
        trigger: selectSorting,
        condition: function (sortField) {
          return sortField === null || sortField.fieldName !== "param";
        }
      })
    });
    ...
    viewModel.value = SortableColumn.defineSortableColumn({
      fieldName: "value"
    }, {
      resetOn: Trigger.defineFilter({
        trigger: selectSorting,
        condition: function (sortField) {
          return sortField === null || sortField.fieldName !== "value";
        }
      })
    });
    ...
    selectSorting.addTrigger(viewModel.param.select);
    selectSorting.addTrigger(viewModel.target.select);
    selectSorting.addTrigger(viewModel.value.select);
