define(function (require) {
  ...
  const EngineNodeServiceEvents = require('workflow/engineNodeService/engineNodeService-events');
  const EngineNodeServiceElement = require('nsx/nsx-application').getElement('workflow', 'engineNodeService');
   ...
   * }} params
   * @template I
   * @return {EngineNodeServiceAction<I>}
   */
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.engineNodeService.create"}),
      updateEvent: EngineNodeServiceEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(EngineNodeServiceElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.engineNodeService.edit"}),
      updateEvent: EngineNodeServiceEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(EngineNodeServiceElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.engineNodeService.delete"}),
      updateEvent: EngineNodeServiceEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(EngineNodeServiceElement, "delete-json"),
          data: JSON.stringify({
            "engineNodeServiceOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
