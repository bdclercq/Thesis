  ...
  function defineInstanceView(input, options) {
    ...
    function initViewmodel() {
      return {
        ...
        engineNode: defineLink(EngineNodeEvents),
        engineService: defineLink(EngineServiceEvents),
        lastRunAt: ko.observable(),
    ...
    function updateViewmodel(engineNodeServiceDetails) {
      if (!!engineNodeServiceDetails) {
        viewmodel.id(engineNodeServiceDetails.id);
        // anchor:update-viewModel:start
        viewmodel.name(engineNodeServiceDetails.name);
        viewmodel.status(engineNodeServiceDetails.status);
        viewmodel.engineNode.value(engineNodeServiceDetails.engineNode);
        viewmodel.engineService.value(engineNodeServiceDetails.engineService);
        viewmodel.lastRunAt(engineNodeServiceDetails.lastRunAt);
        viewmodel.nextRun(engineNodeServiceDetails.nextRun);
