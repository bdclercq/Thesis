  ...
  function defineInstanceView(input, options) {
    ...
    function initViewmodel() {
      return {
        ...
        timeWindowGroup: defineLink(TimeWindowGroupEvents),
        lastRunAt: { value: ko.observable() },
        batchSize: ko.observable(),
        maximumNumberOfNodes: ko.observable(),
        engineNodeServices: ko.observable()
    ...
    function updateViewmodel(engineServiceDetails) {
      if (!!engineServiceDetails) {
        viewmodel.id(engineServiceDetails.id);
        // anchor:update-viewModel:start
        viewmodel.name(engineServiceDetails.name);
        viewmodel.status(engineServiceDetails.status);
        viewmodel.changed(engineServiceDetails.changed);
        viewmodel.busy(engineServiceDetails.busy);
        viewmodel.waitTime(engineServiceDetails.waitTime);
        viewmodel.collector(engineServiceDetails.collector);
        viewmodel.workflow.value(engineServiceDetails.workflow);
        viewmodel.timeWindowGroup.value(engineServiceDetails.timeWindowGroup);
        if (engineServiceDetails.lastRunAt) {
          viewmodel.lastRunAt.value(engineServiceDetails.lastRunAt.value);
        } else {
          viewmodel.lastRunAt.value(null);
        }
        viewmodel.batchSize(engineServiceDetails.batchSize);
        viewmodel.maximumNumberOfNodes(engineServiceDetails.maximumNumberOfNodes);
        viewmodel.engineNodeServices(engineServiceDetails.engineNodeServices);
