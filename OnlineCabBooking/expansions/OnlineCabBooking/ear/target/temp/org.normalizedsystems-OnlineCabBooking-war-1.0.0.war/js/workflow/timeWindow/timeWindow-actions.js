define(function (require) {
  ...
  const TimeWindowEvents = require('workflow/timeWindow/timeWindow-events');
  const TimeWindowElement = require('nsx/nsx-application').getElement('workflow', 'timeWindow');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.timeWindow.create"}),
      updateEvent: TimeWindowEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(TimeWindowElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.timeWindow.edit"}),
      updateEvent: TimeWindowEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(TimeWindowElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.timeWindow.delete"}),
      updateEvent: TimeWindowEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(TimeWindowElement, "delete-json"),
          data: JSON.stringify({
            "timeWindowOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
