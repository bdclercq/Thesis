...
define(function (require) {
  ...
  function defineSorting(input, options) {
    ...
    viewModel.startTime = SortableColumn.defineSortableColumn({
      fieldName: "startTime"
    }, {
      resetOn: Trigger.defineFilter({
        trigger: selectSorting,
        condition: function (sortField) {
          return sortField === null || sortField.fieldName !== "startTime";
        }
      })
    });

    viewModel.stopTime = SortableColumn.defineSortableColumn({
      fieldName: "stopTime"
    }, {
      resetOn: Trigger.defineFilter({
        trigger: selectSorting,
        condition: function (sortField) {
          return sortField === null || sortField.fieldName !== "stopTime";
        }
      })
    });
    ...
    selectSorting.addTrigger(viewModel.startTime.select);
    selectSorting.addTrigger(viewModel.stopTime.select);
