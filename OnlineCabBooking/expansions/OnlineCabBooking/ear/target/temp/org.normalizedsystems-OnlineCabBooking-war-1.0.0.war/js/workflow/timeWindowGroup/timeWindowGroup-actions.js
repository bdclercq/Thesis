define(function (require) {
  ...
  const TimeWindowGroupEvents = require('workflow/timeWindowGroup/timeWindowGroup-events');
  const TimeWindowGroupElement = require('nsx/nsx-application').getElement('workflow', 'timeWindowGroup');
   ...
   * }} params
   * @template I
   * @return {TimeWindowGroupAction<I>}
   */
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.timeWindowGroup.create"}),
      updateEvent: TimeWindowGroupEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(TimeWindowGroupElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.timeWindowGroup.edit"}),
      updateEvent: TimeWindowGroupEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(TimeWindowGroupElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.timeWindowGroup.delete"}),
      updateEvent: TimeWindowGroupEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(TimeWindowGroupElement, "delete-json"),
          data: JSON.stringify({
            "timeWindowGroupOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
