...
define(function (require) {
  ...
  var TimeWindowGroupForm = require('workflow/timeWindowGroup/timeWindowGroup-instance-form');
  var TimeWindowGroupActions = require('workflow/timeWindowGroup/timeWindowGroup-actions');
  ...
  function defineCreateDialog(options) {
    ...
    when(createRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to create a(n) TimeWindowGroup element!")
    });

    function render() {
      var createIntent = TimeWindowGroupEvents.defineCreateIntentListener();

      var constants = ko.observable();
      var values = ko.observable();

      var formView = "workflow/timeWindowGroup-form";
      var title = "TimeWindowGroup";
      var layoutConfig = initLayoutConfig(constants);
      var defaultValues = { id: 0 };
      var formOptions = {
        resetOn: createIntent,
        action: TimeWindowGroupActions.createAction()
      };
      ...
      var form = TimeWindowGroupForm.defineForm({
        data: formData,
  ...
  function initLayoutConfig(constants) {
    return {
      ...
      visible: {
          disabled: ko.pureComputed(function () {
            return utils.isDefined(constants().visible)
          })
      },
      timeWindows: {
          disabled: ko.pureComputed(function () {
            return utils.isDefined(constants().timeWindows)
          })
      }
