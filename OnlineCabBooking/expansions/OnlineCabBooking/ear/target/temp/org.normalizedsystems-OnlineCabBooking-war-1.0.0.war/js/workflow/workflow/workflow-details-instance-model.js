  ...
  function defineInstanceView(input, options) {
    ...
    function initViewmodel() {
      return {
        ...
        responsible: defineLink(UserEvents),
        sequencingStrategy: ko.observable()
    ...
    function updateViewmodel(workflowDetails) {
      if (!!workflowDetails) {
        viewmodel.id(workflowDetails.id);
        // anchor:update-viewModel:start
        viewmodel.name(workflowDetails.name);
        viewmodel.target(workflowDetails.target);
        viewmodel.componentName(workflowDetails.componentName);
        viewmodel.className(workflowDetails.className);
        viewmodel.responsible.value(workflowDetails.responsible);
        viewmodel.sequencingStrategy(workflowDetails.sequencingStrategy);
