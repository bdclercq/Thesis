...
define(function (require) {
  ...
  var WorkflowSorting = require('workflow/workflow/workflow-sorting');
  var WorkflowInfoModel = require('workflow/workflow/workflow-info-instance-model');
  ...
  function defineTable(page, input) {
    var sorting = WorkflowSorting.defineSorting();
    var dataUpdate = WorkflowEvents.defineUpdateListener();
    ...
    var listView = ListView.defineListView({
      page: listModel.page,
      instanceView: WorkflowInfoModel
    });

    var infoListView = "workflow/workflow-info-list"
    var viewModel = {
      heading: sorting,
      list: listView
    };
    ...
    listView.items.subscribe(function (items) {
      if (items.length > 0 && !listView.selection()) {
        var item = items[0];
        item.select.trigger();
      }
    });
