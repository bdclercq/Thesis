define(function (require) {
  ...
  const HelpInfoEvents = require('account/helpInfo/helpInfo-events');
  const HelpInfoElement = require('nsx/nsx-application').getElement('account', 'helpInfo');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.helpInfo.create"}),
      updateEvent: HelpInfoEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(HelpInfoElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.helpInfo.edit"}),
      updateEvent: HelpInfoEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(HelpInfoElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.helpInfo.delete"}),
      updateEvent: HelpInfoEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(HelpInfoElement, "delete-json"),
          data: JSON.stringify({
            "helpInfoOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
