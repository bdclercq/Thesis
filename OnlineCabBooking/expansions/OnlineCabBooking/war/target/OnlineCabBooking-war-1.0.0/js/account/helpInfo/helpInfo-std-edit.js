...
define(function (require) {
  ...
  var HelpInfoProjector = require('account/helpInfo/helpInfo-projector');
  var HelpInfoForm = require('account/helpInfo/helpInfo-instance-form');
  var HelpInfoActions = require('account/helpInfo/helpInfo-actions');
  ...
  function defineEditDialog(options) {
    ...
    when(editRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to edit a(n) HelpInfo element!")
    });

    function render() {
      var editIntent = HelpInfoEvents.defineEditIntentListener();
      var showDialog = Trigger.defineTrigger();

      var constants = ko.observable();
      var selection = ko.observable();
      var formView = "account/helpInfo-form";
      var title = "Edit HelpInfo";
      var layoutConfig = initLayoutConfig(constants)
      var formOptions = {
        resetOn: editIntent,
        action: HelpInfoActions.editAction()
      };

      var detailsProjection = HelpInfoProjector.defineProjector(selection, {
        projection: "details"
      });
      ...
      var form = HelpInfoForm.defineForm({
  ...
  function initLayoutConfig(constants) {
    return {
      ...
      description: {
          disabled: ko.pureComputed(function () {
            return utils.isDefined(constants().description)
          })
      }
