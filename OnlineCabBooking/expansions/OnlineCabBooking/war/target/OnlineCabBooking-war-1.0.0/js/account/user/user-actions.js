define(function (require) {
  ...
  const UserEvents = require('account/user/user-events');
  const UserElement = require('nsx/nsx-application').getElement('account', 'user');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.user.create"}),
      updateEvent: UserEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(UserElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.user.edit"}),
      updateEvent: UserEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(UserElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "account.user.delete"}),
      updateEvent: UserEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(UserElement, "delete-json"),
          data: JSON.stringify({
            "userOid": target.id
          }),
          dataType: 'json',
