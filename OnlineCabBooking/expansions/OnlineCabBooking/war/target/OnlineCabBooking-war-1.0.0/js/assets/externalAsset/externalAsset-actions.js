define(function (require) {
  ...
  const ExternalAssetEvents = require('assets/externalAsset/externalAsset-events');
  const ExternalAssetElement = require('nsx/nsx-application').getElement('assets', 'externalAsset');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "assets.externalAsset.create"}),
      updateEvent: ExternalAssetEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(ExternalAssetElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "assets.externalAsset.edit"}),
      updateEvent: ExternalAssetEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(ExternalAssetElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "assets.externalAsset.delete"}),
      updateEvent: ExternalAssetEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(ExternalAssetElement, "delete-json"),
          data: JSON.stringify({
            "externalAssetOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
