...
define(function (require) {
  ...
  function defineCreateDialog(options) {
    ...
    when(createRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to create a(n) ExternalAsset element!")
    });

    function render() {
      var createIntent = ExternalAssetEvents.defineCreateIntentListener();

      var constants = ko.observable();
      var values = ko.observable();

      var formView = "assets/externalAsset-form";
      var title = "ExternalAsset";
      var layoutConfig = initLayoutConfig(constants);
      var defaultValues = { id: 0 };
      var formOptions = {
        resetOn: createIntent,
        action: ExternalAssetActions.createAction()
      };
