define(function (require) {
  ...
  const InternalAssetEvents = require('assets/internalAsset/internalAsset-events');
  const InternalAssetElement = require('nsx/nsx-application').getElement('assets', 'internalAsset');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "assets.internalAsset.create"}),
      updateEvent: InternalAssetEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(InternalAssetElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "assets.internalAsset.edit"}),
      updateEvent: InternalAssetEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(InternalAssetElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "assets.internalAsset.delete"}),
      updateEvent: InternalAssetEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(InternalAssetElement, "delete-json"),
          data: JSON.stringify({
            "internalAssetOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
