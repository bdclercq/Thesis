define(function (require) {
  ...
  const InternalAssetChunkEvents = require('assets/internalAssetChunk/internalAssetChunk-events');
  const InternalAssetChunkElement = require('nsx/nsx-application').getElement('assets', 'internalAssetChunk');
   ...
   * }} params
   * @template I
   * @return {InternalAssetChunkAction<I>}
   */
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "assets.internalAssetChunk.create"}),
      updateEvent: InternalAssetChunkEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(InternalAssetChunkElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "assets.internalAssetChunk.edit"}),
      updateEvent: InternalAssetChunkEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(InternalAssetChunkElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "assets.internalAssetChunk.delete"}),
      updateEvent: InternalAssetChunkEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(InternalAssetChunkElement, "delete-json"),
          data: JSON.stringify({
            "internalAssetChunkOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
