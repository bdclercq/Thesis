define(function (require) {
  ...
  const AddressEvents = require('onlineCabBookingComp/address/address-events');
  const AddressElement = require('nsx/nsx-application').getElement('onlineCabBookingComp', 'address');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.address.create"}),
      updateEvent: AddressEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(AddressElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.address.edit"}),
      updateEvent: AddressEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(AddressElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.address.delete"}),
      updateEvent: AddressEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(AddressElement, "delete-json"),
          data: JSON.stringify({
            "addressOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
