define(function (require) {
  ...
  const CabEvents = require('onlineCabBookingComp/cab/cab-events');
  const CabElement = require('nsx/nsx-application').getElement('onlineCabBookingComp', 'cab');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.cab.create"}),
      updateEvent: CabEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(CabElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.cab.edit"}),
      updateEvent: CabEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(CabElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.cab.delete"}),
      updateEvent: CabEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(CabElement, "delete-json"),
          data: JSON.stringify({
            "cabOid": target.id
          }),
          dataType: 'json',
