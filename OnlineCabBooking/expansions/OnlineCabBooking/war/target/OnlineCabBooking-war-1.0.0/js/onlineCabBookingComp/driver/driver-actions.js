define(function (require) {
  ...
  const DriverEvents = require('onlineCabBookingComp/driver/driver-events');
  const DriverElement = require('nsx/nsx-application').getElement('onlineCabBookingComp', 'driver');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.driver.create"}),
      updateEvent: DriverEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(DriverElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.driver.edit"}),
      updateEvent: DriverEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(DriverElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.driver.delete"}),
      updateEvent: DriverEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(DriverElement, "delete-json"),
          data: JSON.stringify({
            "driverOid": target.id
          }),
          dataType: 'json',
