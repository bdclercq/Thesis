define(function (require) {
  ...
  const InvoiceEvents = require('onlineCabBookingComp/invoice/invoice-events');
  const InvoiceElement = require('nsx/nsx-application').getElement('onlineCabBookingComp', 'invoice');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.invoice.create"}),
      updateEvent: InvoiceEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(InvoiceElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.invoice.edit"}),
      updateEvent: InvoiceEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(InvoiceElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.invoice.delete"}),
      updateEvent: InvoiceEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(InvoiceElement, "delete-json"),
          data: JSON.stringify({
            "invoiceOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
