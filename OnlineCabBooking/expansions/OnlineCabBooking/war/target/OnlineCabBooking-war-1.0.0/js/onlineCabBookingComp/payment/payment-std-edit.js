...
define(function (require) {
  ...
  var PaymentProjector = require('onlineCabBookingComp/payment/payment-projector');
  var PaymentForm = require('onlineCabBookingComp/payment/payment-instance-form');
  var PaymentActions = require('onlineCabBookingComp/payment/payment-actions');
  ...
  function defineEditDialog(options) {
    ...
    when(editRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to edit a(n) Payment element!")
    });

    function render() {
      var editIntent = PaymentEvents.defineEditIntentListener();
      var showDialog = Trigger.defineTrigger();

      var constants = ko.observable();
      var selection = ko.observable();
      var formView = "onlineCabBookingComp/payment-form";
      var title = "Edit Payment";
      var layoutConfig = initLayoutConfig(constants)
      var formOptions = {
        resetOn: editIntent,
        action: PaymentActions.editAction()
      };

      var detailsProjection = PaymentProjector.defineProjector(selection, {
        projection: "details"
      });
      ...
      var form = PaymentForm.defineForm({
