define(function (require) {
  ...
  const PersonEvents = require('onlineCabBookingComp/person/person-events');
  const PersonElement = require('nsx/nsx-application').getElement('onlineCabBookingComp', 'person');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.person.create"}),
      updateEvent: PersonEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(PersonElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.person.edit"}),
      updateEvent: PersonEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(PersonElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "onlineCabBookingComp.person.delete"}),
      updateEvent: PersonEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(PersonElement, "delete-json"),
          data: JSON.stringify({
            "personOid": target.id
          }),
          dataType: 'json',
