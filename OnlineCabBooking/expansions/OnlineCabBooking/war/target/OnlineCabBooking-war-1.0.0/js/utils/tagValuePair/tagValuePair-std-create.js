...
define(function (require) {
  ...
  function defineCreateDialog(options) {
    ...
    when(createRightsRequest.refused).thenDo(function () {
      throw new Error("You do not have the rights to create a(n) TagValuePair element!")
    });

    function render() {
      var createIntent = TagValuePairEvents.defineCreateIntentListener();

      var constants = ko.observable();
      var values = ko.observable();

      var formView = "utils/tagValuePair-form";
      var title = "TagValuePair";
      var layoutConfig = initLayoutConfig(constants);
      var defaultValues = { id: 0 };
      var formOptions = {
        resetOn: createIntent,
        action: TagValuePairActions.createAction()
      };
