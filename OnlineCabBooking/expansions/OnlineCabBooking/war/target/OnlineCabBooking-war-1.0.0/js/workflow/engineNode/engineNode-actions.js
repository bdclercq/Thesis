define(function (require) {
  ...
  const EngineNodeEvents = require('workflow/engineNode/engineNode-events');
  const EngineNodeElement = require('nsx/nsx-application').getElement('workflow', 'engineNode');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.engineNode.create"}),
      updateEvent: EngineNodeEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(EngineNodeElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.engineNode.edit"}),
      updateEvent: EngineNodeEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(EngineNodeElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.engineNode.delete"}),
      updateEvent: EngineNodeEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(EngineNodeElement, "delete-json"),
          data: JSON.stringify({
            "engineNodeOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
