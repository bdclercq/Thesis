define(function (require) {
  ...
  const StateTaskEvents = require('workflow/stateTask/stateTask-events');
  const StateTaskElement = require('nsx/nsx-application').getElement('workflow', 'stateTask');
  ...
  function createAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.stateTask.create"}),
      updateEvent: StateTaskEvents.defineCreateUpdateTrigger(),
      submit: (formData, callback) => {
        formData.id = 0;
        var requestOptions = {
          url: nsxActions.getElementAction(StateTaskElement, "saveDetails-json"),
  ...
  function editAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.stateTask.edit"}),
      updateEvent: StateTaskEvents.defineEditUpdateTrigger(),
      submit: (formData, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(StateTaskElement, "saveDetails-json"),
          data: JSON.stringify(formData),
  ...
  function deleteAction() {
    return newAction({
      errorNotifications: ErrorTrigger.defineTrigger({subject: "workflow.stateTask.delete"}),
      updateEvent: StateTaskEvents.defineDeleteUpdateTrigger(),
      submit: (target, callback) => {
        var requestOptions = {
          url: nsxActions.getElementAction(StateTaskElement, "delete-json"),
          data: JSON.stringify({
            "stateTaskOid": target.id
          }),
          dataType: 'json',
          contentType: 'application/json; charset=UTF-8'
        };
